...
void
test_null_key (void *server_, void *server_mon_, char *my_endpoint_,
               char *server_public_, char *client_public_,
               char *client_secret_)
{
  expect_new_client_curve_bounce_fail (server_public_, client_public_,
                                       client_secret_, my_endpoint_, server_);

  int handshake_failed_encryption_event_count = expect_monitor_event_multiple (
      server_mon_, ZMQ_EVENT_HANDSHAKE_FAILED_PROTOCOL,
...
test_curve_security_with_bogus_client_credentials ()
{
  ...
  expect_new_client_curve_bounce_fail (valid_server_public, bogus_public,
                                       bogus_secret, my_endpoint, server, NULL,
...
void
expect_zmtp_mechanism_mismatch (void *client_, char *my_endpoint_,
                                void *server_, void *server_mon_)
{
...
void
send_all (fd_t fd_, const char *data_, socket_size_t size_)
{
  while (size_ > 0)
    {
...
template <size_t N>
void
send (fd_t fd_, const char (&data_)[N])
{
...
template <size_t N>
void
send (fd_t fd_, const uint8_t (&data_)[N])
{
...
// same as htonll, which is only available on few platforms (recent Windows,
// but not on Linux, e.g.(
static uint64_t
host_to_network (uint64_t value_)
{
  // The answer is 42
...
template <size_t N>
void
send_command (fd_t s_, char (&command_)[N])
{
  if (N < 256)
    {
      send (s_, "\x04");
...
void
recv_all (fd_t fd_, uint8_t *data_, socket_size_t len_)
{
...
fd_t
connect_exchange_greeting_and_send_hello (char *my_endpoint_,
...
fd_t
connect_exchange_greeting_and_hello_welcome (char *my_endpoint_,
                                             void *server_mon_, int timeout_,
                                             zmq::curve_client_tools_t &tools_)
{
...
void
test_curve_security_invalid_keysize (void *ctx_)
{
...
main (void)
{
  ...
  // TODO this requires a deviating test setup, must be moved to a separate
  // executable/fixture
