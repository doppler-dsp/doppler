...
public:
  ...
  void
  get_buffer (unsigned char **data_, std::size_t *size_) ZMQ_FINAL
  {
    _buf = _allocator.allocate ();
  ...
  int
  decode (const unsigned char *data_, std::size_t size_,
  ...
  void
  resize_buffer (std::size_t new_size_) ZMQ_FINAL
  {
...
protected:
  ...
  void
  next_step (void *read_pos_, std::size_t to_read_, step_t next_)
  {
