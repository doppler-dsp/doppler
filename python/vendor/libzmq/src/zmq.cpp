...
void
zmq_version (int *major_, int *minor_, int *patch_)
{
...
int
zmq_ctx_set (void *ctx_, int option_, int optval_)
{
  return zmq_ctx_set_ext (ctx_, option_, &optval_, sizeof (int));
}

int
zmq_ctx_set_ext (void *ctx_, int option_, const void *optval_,
                 size_t optvallen_)
{
  if (!ctx_ || !(static_cast<zmq::ctx_t *> (ctx_))->check_tag ())
    {
...
int
zmq_ctx_get (void *ctx_, int option_)
{
  if (!ctx_ || !(static_cast<zmq::ctx_t *> (ctx_))->check_tag ())
    {
      errno = EFAULT;
...
int
zmq_ctx_get_ext (void *ctx_, int option_, void *optval_, size_t *optvallen_)
{
  if (!ctx_ || !(static_cast<zmq::ctx_t *> (ctx_))->check_tag ())
    {
      errno = EFAULT;
...
static zmq::socket_base_t *
as_socket_base_t (void *s_)
{
...
void *
zmq_socket (void *ctx_, int type_)
{
  if (!ctx_ || !(static_cast<zmq::ctx_t *> (ctx_))->check_tag ())
    {
      errno = EFAULT;
...
int
zmq_socket_monitor_versioned (void *s_, const char *addr_, uint64_t events_,
...
int
zmq_socket_monitor (void *s_, const char *addr_, int events_)
{
  return zmq_socket_monitor_versioned (s_, addr_, events_, 1, ZMQ_PAIR);
}

int
zmq_join (void *s_, const char *group_)
{
  zmq::socket_base_t *s = as_socket_base_t (s_);
  if (!s)
    return -1;
...
int
zmq_leave (void *s_, const char *group_)
{
  zmq::socket_base_t *s = as_socket_base_t (s_);
  if (!s)
    return -1;
...
int
zmq_bind (void *s_, const char *addr_)
{
  zmq::socket_base_t *s = as_socket_base_t (s_);
  if (!s)
    return -1;
...
int
zmq_connect (void *s_, const char *addr_)
{
  zmq::socket_base_t *s = as_socket_base_t (s_);
  if (!s)
    return -1;
...
uint32_t
zmq_connect_peer (void *s_, const char *addr_)
{
...
int
zmq_unbind (void *s_, const char *addr_)
{
  zmq::socket_base_t *s = as_socket_base_t (s_);
  if (!s)
    return -1;
...
int
zmq_disconnect (void *s_, const char *addr_)
{
  zmq::socket_base_t *s = as_socket_base_t (s_);
  if (!s)
    return -1;
...
int
zmq_sendmsg (void *s_, zmq_msg_t *msg_, int flags_)
{
  return zmq_msg_send (msg_, s_, flags_);
}

int
zmq_send (void *s_, const void *buf_, size_t len_, int flags_)
{
  zmq::socket_base_t *s = as_socket_base_t (s_);
  if (!s)
    return -1;
...
//
int
zmq_sendiov (void *s_, iovec *a_, size_t count_, int flags_)
{
  zmq::socket_base_t *s = as_socket_base_t (s_);
  if (!s)
    return -1;
...
int
zmq_recvmsg (void *s_, zmq_msg_t *msg_, int flags_)
{
  return zmq_msg_recv (msg_, s_, flags_);
}

int
zmq_recv (void *s_, void *buf_, size_t len_, int flags_)
{
  zmq::socket_base_t *s = as_socket_base_t (s_);
  if (!s)
    return -1;
...
//
int
zmq_recviov (void *s_, iovec *a_, size_t *count_, int flags_)
{
  zmq::socket_base_t *s = as_socket_base_t (s_);
  if (!s)
    return -1;
...
int
zmq_msg_init (zmq_msg_t *msg_)
{
  return (reinterpret_cast<zmq::msg_t *> (msg_))->init ();
}

int
zmq_msg_init_size (zmq_msg_t *msg_, size_t size_)
{
...
int
zmq_msg_recv (zmq_msg_t *msg_, void *s_, int flags_)
{
  zmq::socket_base_t *s = as_socket_base_t (s_);
  if (!s)
    return -1;
...
int
zmq_msg_close (zmq_msg_t *msg_)
{
  return (reinterpret_cast<zmq::msg_t *> (msg_))->close ();
}

int
zmq_msg_move (zmq_msg_t *dest_, zmq_msg_t *src_)
{
...
int
zmq_msg_copy (zmq_msg_t *dest_, zmq_msg_t *src_)
{
  return (reinterpret_cast<zmq::msg_t *> (dest_))
...
void *
zmq_msg_data (zmq_msg_t *msg_)
{
  return (reinterpret_cast<zmq::msg_t *> (msg_))->data ();
}

size_t
zmq_msg_size (const zmq_msg_t *msg_)
{
  return ((zmq::msg_t *)msg_)->size ();
}

int
zmq_msg_more (const zmq_msg_t *msg_)
{
  return zmq_msg_get (msg_, ZMQ_MORE);
}

int
zmq_msg_get (const zmq_msg_t *msg_, int property_)
{
...
const char *
zmq_msg_gets (const zmq_msg_t *msg_, const char *property_)
{
...
#if defined ZMQ_HAVE_POLLER
static int
zmq_poller_poll (zmq_pollitem_t *items_, int nitems_, long timeout_)
{
  ...
  //  If no sockets are repeated (likely), the two are still co-ordered, so
  //  step through the items checking for matches only on the first event. If
  //  there are repeat items, they cannot be assumed to be co-ordered, so each
  //  pollitem must check fired events from the beginning.
...
int
zmq_poll (zmq_pollitem_t *items_, int nitems_, long timeout_)
{
#if defined ZMQ_HAVE_POLLER
  ...
  for (int i = 0; i != nitems_; i++)
    {
      ...
      //  events to normal POLLIN/POLLOUT for poll ().
      else
        {
          ...
          pollfds[i].events = (items_[i].events & ZMQ_POLLIN ? POLLIN : 0)
  ...
  //  file descriptors.
  //  TODO since this function is called by a client, we could return errno
  //  EINVAL/ENOMEM/... here
...
#if defined ZMQ_HAVE_WINDOWS
          int rc = select (0, inset.get (), outset.get (), errset.get (),
                           ptimeout);
...
#ifdef ZMQ_HAVE_PPOLL
// return values of 0 or -1 should be returned from zmq_poll; return value 1
// means items passed checks
int
zmq_poll_check_items_ (zmq_pollitem_t *items_, int nitems_, long timeout_)
{
  if (unlikely (nitems_ < 0))
    {
      errno = EINVAL;
...
struct zmq_poll_select_fds_t_
{
  explicit zmq_poll_select_fds_t_ (int nitems_)
      : pollset_in (nitems_), pollset_out (nitems_), pollset_err (nitems_),
        inset (nitems_), outset (nitems_), errset (nitems_), maxfd (0)
  {
    FD_ZERO (pollset_in.get ());
    FD_ZERO (pollset_out.get ());
    FD_ZERO (pollset_err.get ());
...
timeval *
zmq_poll_select_set_timeout_ (long timeout_, bool first_pass, uint64_t now,
                              uint64_t end, timeval &timeout)
{
  timeval *ptimeout;
  if (first_pass)
    {
...
int
zmq_poll_select_check_events_ (zmq_pollitem_t *items_, int nitems_,
                               zmq_poll_select_fds_t_ &fds, int &nevents)
{
  //  Check for the events.
  for (int i = 0; i != nitems_; i++)
    {
...
bool
zmq_poll_must_break_loop_ (long timeout_, int nevents, bool &first_pass,
                           zmq::clock_t &clock, uint64_t &now, uint64_t &end)
{
  //  If timeout is zero, exit immediately whether there are events or not.
  if (timeout_ == 0)
    return true;
...
#if !defined _WIN32
int
zmq_ppoll (zmq_pollitem_t *items_, int nitems_, long timeout_,
  ...
  while (true)
    {
      ...
      if (zmq_poll_must_break_loop_ (timeout_, nevents, first_pass, clock, now,
                                     end))
        {
          break;
        }
...
int
zmq_poller_destroy (void **poller_p_)
{
  if (poller_p_)
    {
...
static int
check_poller (void *const poller_)
{
  if (!poller_
...
static int
check_events (const short events_)
{
...
static int
check_poller_registration_args (void *const poller_, void *const s_)
{
...
static int
check_poller_fd_registration_args (void *const poller_, const zmq::fd_t fd_)
{
  if (-1 == check_poller (poller_))
    return -1;
...
int
zmq_poller_add (void *poller_, void *s_, void *user_data_, short events_)
{
...
int
zmq_poller_add_fd (void *poller_, zmq::fd_t fd_, void *user_data_,
                   short events_)
{
...
int
zmq_poller_modify (void *poller_, void *s_, short events_)
{
  if (-1 == check_poller_registration_args (poller_, s_)
      || -1 == check_events (events_))
    return -1;
...
int
zmq_poller_modify_fd (void *poller_, zmq::fd_t fd_, short events_)
{
  if (-1 == check_poller_fd_registration_args (poller_, fd_)
      || -1 == check_events (events_))
    return -1;
...
int
zmq_poller_remove (void *poller_, void *s_)
{
  if (-1 == check_poller_registration_args (poller_, s_))
    return -1;
...
int
zmq_poller_remove_fd (void *poller_, zmq::fd_t fd_)
{
  if (-1 == check_poller_fd_registration_args (poller_, fd_))
    return -1;
...
int
zmq_poller_wait (void *poller_, zmq_poller_event_t *event_, long timeout_)
{
...
int
zmq_poller_wait_all (void *poller_, zmq_poller_event_t *events_, int n_events_,
                     long timeout_)
{
  if (-1 == check_poller (poller_))
    return -1;
  ...
  const int rc
      = (static_cast<zmq::socket_poller_t *> (poller_))
            ->wait (
                reinterpret_cast<zmq::socket_poller_t::event_t *> (events_),
                n_events_, timeout_);
...
int
zmq_poller_fd (void *poller_, zmq_fd_t *fd_)
{
  if (!poller_
      || !(static_cast<zmq::socket_poller_t *> (poller_)->check_tag ()))
    {
...
int
zmq_socket_get_peer_state (void *s_, const void *routing_id_,
...
int
zmq_timers_destroy (void **timers_p_)
{
...
int
zmq_timers_add (void *timers_, size_t interval_, zmq_timer_fn handler_,
                void *arg_)
{
...
int
zmq_timers_cancel (void *timers_, int timer_id_)
{
  if (!timers_ || !(static_cast<zmq::timers_t *> (timers_))->check_tag ())
    {
      errno = EFAULT;
...
int
zmq_timers_set_interval (void *timers_, int timer_id_, size_t interval_)
{
  if (!timers_ || !(static_cast<zmq::timers_t *> (timers_))->check_tag ())
    {
      errno = EFAULT;
...
int
zmq_timers_reset (void *timers_, int timer_id_)
{
  if (!timers_ || !(static_cast<zmq::timers_t *> (timers_))->check_tag ())
    {
      errno = EFAULT;
...
long
zmq_timers_timeout (void *timers_)
{
  if (!timers_ || !(static_cast<zmq::timers_t *> (timers_))->check_tag ())
    {
      errno = EFAULT;
...
int
zmq_timers_execute (void *timers_)
{
  if (!timers_ || !(static_cast<zmq::timers_t *> (timers_))->check_tag ())
    {
      errno = EFAULT;
...
int
zmq_proxy (void *frontend_, void *backend_, void *capture_)
{
...
int
zmq_proxy_steerable (void *frontend_, void *backend_, void *capture_,
                     void *control_)
{
  if (!frontend_ || !backend_)
    {
...
int
zmq_device (int /* type */, void *frontend_, void *backend_)
{
  return zmq::proxy (static_cast<zmq::socket_base_t *> (frontend_),
...
int
zmq_has (const char *capability_)
{
...
int
zmq_socket_monitor_pipes_stats (void *s_)
{
  zmq::socket_base_t *s = as_socket_base_t (s_);
  if (!s)
    return -1;
