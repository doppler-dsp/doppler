...
dp_nprmeas_state_t *
dp_nprmeas_create (size_t n, double fs, double full_scale, size_t bits,
                   double dynamic_range_db)
{
  if (n < 2 || fs <= 0.0)
    return NULL;
  dp_nprmeas_state_t *s = (dp_nprmeas_state_t *)calloc (1, sizeof (*s));
  if (!s)
    return NULL;
...
void
dp_nprmeas_destroy (dp_nprmeas_state_t *state)
{
  if (!state)
    return;
  if (state->psd)
...
void
dp_nprmeas_reset (dp_nprmeas_state_t *state)
{
  (void)state;
}

npr_meas_t
dp_nprmeas_analyze (dp_nprmeas_state_t *s, const float *x, size_t n_in,
...
size_t
dp_nprmeas_spectrum_dbfs_max_out (dp_nprmeas_state_t *state)
{
  return state->nfft;
}

size_t
dp_nprmeas_spectrum_dbfs (dp_nprmeas_state_t *state, const float *x,
