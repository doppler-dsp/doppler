...
#define WFM_FRAME_MAX_STAGES 6
  ...
  /**
   * @brief The frame knobs a FACE offers, before they become a description.
   *
   * Both directions of a link answer the same question -- "which fields, and
   * which stage covers what" -- from the same handful of choices, and the
   * answer is @ref wfm_frame_desc_of. A generator fills this from its scene;
   * a receiver fills it from its own configuration. They cannot disagree
   * about the layout because neither one computes it.
   *
   * A field is a literal the caller supplies. On the RECEIVE side the payload
   * is exactly what is not known yet, so @p payload may be NULL and only
   * @p n_payload matters -- the description is a geometry, and a receiver
   * needs the geometry, not the contents.
   *
   * The two CCSDS numbers arrive as numbers rather than as a dependency:
   * `wfm_frame.c` cannot call `ccsds_tm` (ccsds_tm depends on it), which is
   * the same reason the stage KERNELS arrive as a table. Each caller passes
  ...
  typedef struct
  {
    ...
    size_t         n_marker;
    /** Preamble, repeated @p preamble_reps times. A DSSS burst leaves this
        NULL: its preamble is unmodulated and unspread, so it is not part of
        the frame at all (see @ref wfm_dsss_desc_chips). */
    ...
    const uint8_t *sync;
    size_t         n_sync;
    /** The payload. NULL on the receive side; @p n_payload always matters. */
  ...
  /**
   * @brief Turn the knobs into a description: fields in wire order, and the
   *        span each stage covers.
   *
   * The ONE place a set of frame choices becomes a layout, so a generator and
   * a receiver holding the same choices hold the same frame. Which stage
   * covers what is the whole content of it:
   *
   ...
   *
   * The middle row is CCSDS 131.0-B-6 10.3.4's rule generalised: the
   * randomiser does not cover the ASM, and the reason given — a marker a
   * receiver correlates against must not vary between frames — is exactly as
   * true of a preamble and a sync word.
   *
   * @param s  the knobs.
   * @param d  receives the description.
   * @return 0, or -1 on NULL, or if the geometry needs more fields or stages
   *         than a description holds.
   */
