...
print(f"  {'idx':>3}  {'in.real':>8}  {'in.imag':>8}  {'out.real':>9}  {'out.imag':>9}")
for i in range(8):
    print(f"  {i:>3}  {x[i].real:>8.3f}  {x[i].imag:>8.3f}"
...
t1 = np.arange(8)
t2 = np.arange(8, 16)
tone = lambda t: np.exp(2j * np.pi * 0.05 * t).astype(np.complex64)
