...
NCO_init (NCOObject *self, PyObject *args, PyObject *kwds)
{
  ...
  self->_steps_u32_buf
      = malloc (nco_steps_u32_max_out (self->handle) * sizeof (uint32_t));
  ...
  self->_steps_u32_ovf_buf
      = malloc (nco_steps_u32_ovf_max_out (self->handle) * sizeof (uint32_t));
  ...
  self->_steps_u32_ovf_buf_1
      = malloc (nco_steps_u32_ovf_max_out (self->handle) * sizeof (uint8_t));
...
static PyObject *
NCO_steps_u32_scaled (NCOObject *self, PyObject *args)
{
  if (!self->handle)
    {
      PyErr_SetString (PyExc_RuntimeError, "destroyed");
...
static PyObject *
NCO_steps_u32_ovf (NCOObject *self, PyObject *args)
{
  if (!self->handle)
    {
      PyErr_SetString (PyExc_RuntimeError, "destroyed");
...
static PyGetSetDef NCO_getset[]
    = { { "norm_freq", (getter)NCO_getprop_norm_freq,
          (setter)NCO_setprop_norm_freq, NULL, NULL },
        { "phase", (getter)NCO_getprop_phase, (setter)NCO_setprop_phase, NULL,
          NULL },
...
static PyMethodDef NCO_methods[]
    = { { "reset", (PyCFunction)NCO_reset, METH_NOARGS,
          "Reset state to post-create defaults." },

        { "steps_u32", (PyCFunction)NCO_steps_u32, METH_VARARGS,
          ...
          "    >>> y.dtype\n"
          "    dtype('uint32')\n" },
        { "steps_u32_scaled", (PyCFunction)NCO_steps_u32_scaled, METH_VARARGS,
          ...
          "    >>> y.dtype\n"
          "    dtype('uint32')\n" },
        { "steps_u32_ovf", (PyCFunction)NCO_steps_u32_ovf, METH_VARARGS,
          ...
          "    >>> y[0].dtype\n"
          "    dtype('uint32')\n" },
        { "destroy", (PyCFunction)NCO_destroy, METH_NOARGS,
          "Release resources." },
...
static PyObject *
LO_new (PyTypeObject *type, PyObject *args, PyObject *kwds)
{
...
static int
LO_init (LOObject *self, PyObject *args, PyObject *kwds)
{
  ...
  self->_steps_buf
      = malloc (lo_steps_max_out (self->handle) * sizeof (float complex));
  if (!self->_steps_buf)
    {
      PyErr_NoMemory ();
      return -1;
  ...
  self->_steps_ctrl_buf
      = malloc (lo_steps_ctrl_max_out (self->handle) * sizeof (float complex));
...
static PyObject *
LO_reset (LOObject *self, PyObject *Py_UNUSED (ignored))
{
  if (!self->handle)
    {
      PyErr_SetString (PyExc_RuntimeError, "destroyed");
...
static PyObject *
LO_steps (LOObject *self, PyObject *args)
{
  if (!self->handle)
    {
      PyErr_SetString (PyExc_RuntimeError, "destroyed");
...
static PyObject *
LO_steps_ctrl (LOObject *self, PyObject *args)
{
  if (!self->handle)
    {
      PyErr_SetString (PyExc_RuntimeError, "destroyed");
...
static PyGetSetDef LO_getset[]
    = { { "norm_freq", (getter)LO_getprop_norm_freq,
          (setter)LO_setprop_norm_freq, NULL, NULL },
        { "phase", (getter)LO_getprop_phase, (setter)LO_setprop_phase, NULL,
          NULL },
        { "phase_inc", (getter)LO_getprop_phase_inc, NULL, NULL, NULL },
        { NULL } };

static PyObject *
LO_destroy (LOObject *self, PyObject *Py_UNUSED (ignored))
{
  if (self->handle)
    {
      lo_destroy (self->handle);
...
static PyObject *
LO_enter (LOObject *self, PyObject *Py_UNUSED (ignored))
{
  Py_INCREF (self);
  return (PyObject *)self;
}
...
static PyObject *
LO_exit (LOObject *self, PyObject *args)
{
  (void)args;
  if (self->handle)
    {
...
static PyMethodDef LO_methods[]
    = { { "reset", (PyCFunction)LO_reset, METH_NOARGS,
          "Reset state to post-create defaults." },

        { "steps", (PyCFunction)LO_steps, METH_VARARGS,
          "steps(n=1) -> ndarray\n"
          "\n"
          "Zero-copy view into pre-allocated output buffer.\n"
          "\n"
          ...
          "    >>> y.dtype\n"
          "    dtype('complex64')\n" },
        { "steps_ctrl", (PyCFunction)LO_steps_ctrl, METH_VARARGS,
          ...
          "    >>> y.dtype\n"
          "    dtype('complex64')\n" },
        { "destroy", (PyCFunction)LO_destroy, METH_NOARGS,
          "Release resources." },
        { "__enter__", (PyCFunction)LO_enter, METH_NOARGS, NULL },
        { "__exit__", (PyCFunction)LO_exit, METH_VARARGS, NULL },
        { NULL } };

static PyTypeObject LOType = {
  PyVarObject_HEAD_INIT (NULL, 0).tp_name = "source.LO",
  .tp_basicsize = sizeof (LOObject),
  .tp_dealloc = (destructor)LO_dealloc,
  .tp_flags = Py_TPFLAGS_DEFAULT,
  .tp_doc = "LO type.",
  .tp_methods = LO_methods,
  .tp_getset = LO_getset,
  .tp_new = LO_new,
  .tp_init = (initproc)LO_init,
};
...
static PyModuleDef source_moduledef = {
  PyModuleDef_HEAD_INIT, .m_name = "source", .m_doc = "Source module.",
...
PyInit_source (void)
{
  ...
  Py_INCREF (&LOType);
  if (PyModule_AddObject (m, "LO", (PyObject *)&LOType) < 0)
    {
      Py_DECREF (&LOType);
      Py_DECREF (m);
      return NULL;
