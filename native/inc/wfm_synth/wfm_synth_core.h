...
enum {
    ...
    WFM_SYNTH_BITS = 5,  /* user bit pattern, oversampled + cycled    */
};
 ...
 *              5=bits.  The Python binding accepts strings "tone"|"noise"|
 *              "pn"|"bpsk"|"qpsk"|"bits".  For "bits" attach the pattern with
 *              wfm_synth_set_bits() after create().
