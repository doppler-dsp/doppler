...
class Reader:
    ...
    path : str | os.PathLike
        ...
        DETACHED BLUE capture this is normally the HEADER file -- `<base>.tmp`
        or `<base>.prm` per BLUE 3.1.1.4 (this library's own writer emits
        `<base>.hdr`) -- whose HCB `detached` field points at the collocated
        `<base>.det` payload; the extension does not decide, `detached` does.
    ...
    @property
    def t0_source(self) -> Literal["none", "timecode", "sigmf"]:
    ...
    def keywords(self) -> dict[str, str | int | float | list[int] | list[float]]:
        ...
        empty when the capture carries no extended header. Values follow the
        keyword type: a str for A, an int/float for a single-element numeric
        keyword, a list for a multi-element one. For a detached capture these
        come from the HEADER file.
        """
