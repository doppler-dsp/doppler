...
int
zmq::timers_t::add (size_t interval_, timers_timer_fn handler_, void *arg_)
{
  if (handler_ == NULL)
    {
      errno = EFAULT;
...
struct zmq::timers_t::match_by_id
{
  ...
  bool
  operator() (timersmap_t::value_type const &entry_) const
  {
...
int
zmq::timers_t::cancel (int timer_id_)
{
...
int
zmq::timers_t::set_interval (int timer_id_, size_t interval_)
{
...
int
zmq::timers_t::reset (int timer_id_)
{
  const timersmap_t::iterator end = _timers.end ();
  const timersmap_t::iterator it
      = std::find_if (_timers.begin (), end, match_by_id (timer_id_));
