...
void
zmq::socks_greeting_encoder_t::encode (const socks_greeting_t &greeting_)
{
  uint8_t *ptr = _buf;
...
int
zmq::socks_greeting_encoder_t::output (fd_t fd_)
{
  const int rc = tcp_write (fd_, _buf + _bytes_written,
                            _bytes_encoded - _bytes_written);
  if (rc > 0)
...
int
zmq::socks_choice_decoder_t::input (fd_t fd_)
{
...
int
zmq::socks_basic_auth_request_encoder_t::output (fd_t fd_)
{
  const int rc = tcp_write (fd_, _buf + _bytes_written,
                            _bytes_encoded - _bytes_written);
  if (rc > 0)
...
int
zmq::socks_auth_response_decoder_t::input (fd_t fd_)
{
  zmq_assert (_bytes_read < 2);
  const int rc = tcp_read (fd_, _buf + _bytes_read, 2 - _bytes_read);
  if (rc > 0)
...
zmq::socks_request_t::socks_request_t (uint8_t command_, std::string hostname_,
                                       uint16_t port_)
...
void
zmq::socks_request_encoder_t::encode (const socks_request_t &req_)
{
...
int
zmq::socks_request_encoder_t::output (fd_t fd_)
{
  const int rc = tcp_write (fd_, _buf + _bytes_written,
                            _bytes_encoded - _bytes_written);
  if (rc > 0)
...
int
zmq::socks_response_decoder_t::input (fd_t fd_)
{
  size_t n = 0;
