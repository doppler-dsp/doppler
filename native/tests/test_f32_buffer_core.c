...
main (void)
{
  dp_f32_buffer_state_t *ab = dp_f32_create (1024);
  DP_REQUIRE (ab != NULL);
  ...
  DP_CHECK (dp_f32_peek_view (ab, N) == NULL);

  /* N ELEMENTS in is N samples, not N scalars. */
  DP_CHECK (dp_f32_write_some_view (ab, x, N) == N);
  DP_CHECK (dp_f32_available (ab) == N);
  ...
  for (int k = 0; k < N; k++)
    {
      DP_CHECK (v[k] == (float _Complex) (k + 1));
      DP_CHECK (raw[2 * k] == (float)(k + 1) && raw[2 * k + 1] == 0.0f);
    }

  /* wait_view is peek_view when the samples are already there. */
  DP_CHECK (dp_f32_wait_view (ab, N) == v);
  dp_f32_consume (ab, N);
  ...
  DP_REQUIRE (big != NULL);
  DP_CHECK (!dp_f32_write_view (ab, big, cap + 1));
  DP_CHECK (ab->dropped == cap + 1);
  DP_CHECK (dp_f32_write_view (ab, big, cap));
  DP_CHECK (dp_f32_space (ab) == 0);
  free (big);

  DP_CHECK (dp_f32_buffer_get_capacity (ab) == cap);
  DP_CHECK (dp_f32_buffer_get_available (ab) == cap);
  DP_CHECK (dp_f32_buffer_get_space (ab) == 0);
  DP_CHECK (dp_f32_buffer_get_dropped (ab) == cap + 1);
  DP_CHECK (!dp_f32_buffer_get_closed (ab));
  dp_f32_close (ab);
  DP_CHECK (dp_f32_buffer_get_closed (ab));

  dp_f32_destroy (ab);
  DP_TEST_END ("test_f32_buffer_core");
}
