...
int
get_subscription_count (void *skt)
{
  int num_subs = 0;
...
test_nested_topic_prefixes ()
{
  ...
  // Even if the subscriptions are nested one into the other, the number of
  // subscriptions received on the subscriber/publisher socket will be 3:
  TEST_ASSERT_EQUAL_INT (get_subscription_count (subscriber), 3);
  TEST_ASSERT_EQUAL_INT (get_subscription_count (publisher), 3);
