...
int
zmq::vmci_listener_t::set_local_address (const char *addr_)
{
  ...
  _s = open_socket (this->get_ctx ()->get_vmci_socket_family (), SOCK_STREAM,
                    0);
...
zmq::vmci_listener_t::accept ()
{
  ...
  if (sock == INVALID_SOCKET)
    {
      wsa_assert (WSAGetLastError () == WSdpWOULDBLOCK
                  || WSAGetLastError () == WSdpCONNRESET
                  || WSAGetLastError () == WSdpMFILE
                  || WSAGetLastError () == WSdpNOBUFS);
      return retired_fd;
    }
#if !defined _WIN32_WCE
  //  On Windows, preventing sockets to be inherited by child processes.
