...
its NCO drifts on the quantised aid (§12.22), and closed on that cell the
loop holds the emitter at its own jitter with no truth in the loop (§12.23). What the hold cannot tell apart is
...
| the searcher's cell as the code tracker    | in the window the surface reads the phase as the loop does; under data the coasting DLL on the raw block does; closed on its own cell it holds the emitter at the loop's jitter for 25 s at both C/N0s | §12.20–23             |
