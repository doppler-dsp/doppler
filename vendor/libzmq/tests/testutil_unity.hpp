...
int test_assert_success_message_errno_helper (int rc_, const char *msg_,
                                              const char *expr_, int line);

int test_assert_success_message_raw_errno_helper (int rc_, const char *msg_,
                                                  const char *expr_, int line,
                                                  bool zero_ = false);
...
int test_assert_failure_message_raw_errno_helper (int rc_, int expected_errno_,
                                                  const char *msg_,
                                                  const char *expr_, int line);
...
void
send_array_expect_success (void *socket_, const uint8_t (&array_)[SIZE],
                           int flags_)
{
...
template <size_t SIZE>
void
recv_array_expect_success (void *socket_, const uint8_t (&array_)[SIZE],
                           int flags_)
{
  char buffer[255];
...
// test_context_socket_close or test_context_socket_close_zero_linger, and
// generates a warning otherwise.
void teardown_test_context ();

// Creates a libzmq socket on the test context, and tracks its lifecycle.
// You MUST use test_context_socket_close or
// test_context_socket_close_zero_linger to close a socket created via this
// function, otherwise undefined behaviour will result. CAUTION: this function
// is not thread-safe, and may only be used from the main thread.
...
void test_bind (void *socket_, const char *bind_address_, char *my_endpoint_,
                size_t len_);
...
typedef void (*bind_function_t) (void *socket_, char *my_endpoint_,
                                 size_t len_);
