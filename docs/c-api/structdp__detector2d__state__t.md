...
# Struct dp\_detector2d\_state\_t



[**ClassList**](annotated.md) **>** [**dp\_detector2d\_state\_t**](structdp__detector2d__state__t.md)
...
Allocate with [**dp\_detector2d\_create()**](detector2d__core_8h.md#function-dp_detector2d_create); never stack-allocate. 
...
```C++
int dp_detector2d_state_t::_last_corr_valid;
```
...
```C++
dp_corr2d_state_t* dp_detector2d_state_t::corr;
```
...
```C++
float* dp_detector2d_state_t::mag_buf;
```
...
```C++
size_t dp_detector2d_state_t::n;
```
...
```C++
float dp_detector2d_state_t::noise_est;
```
...
```C++
size_t dp_detector2d_state_t::noise_hi;
```
...
```C++
size_t dp_detector2d_state_t::noise_lo;
```
...
```C++
det_noise_mode_t dp_detector2d_state_t::noise_mode;
```
...
```C++
float* dp_detector2d_state_t::noise_scratch;
```
...
```C++
size_t dp_detector2d_state_t::nx;
```
...
```C++
size_t dp_detector2d_state_t::ny;
```
...
```C++
float _Complex* dp_detector2d_state_t::out_buf;
```
...
```C++
size_t dp_detector2d_state_t::peak_col;
```
...
```C++
float dp_detector2d_state_t::peak_mag;
```
...
```C++
size_t dp_detector2d_state_t::peak_row;
```
...
```C++
dp_f32_t* dp_detector2d_state_t::ring;
```
...
```C++
size_t dp_detector2d_state_t::ring_cap;
```
...
```C++
float dp_detector2d_state_t::test_stat;
```
...
```C++
float dp_detector2d_state_t::threshold;
```
