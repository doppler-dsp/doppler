...
static int encode_base64 (const unsigned char *in_, int in_len_, char *out_,
                          int out_len_);

static void compute_accept_key (char *key_,
                                unsigned char hash_[SHA_DIGEST_LENGTH]);

zmq::ws_engine_t::ws_engine_t (fd_t fd_, const options_t &options_,
                               const endpoint_uri_pair_t &endpoint_uri_pair_,
                               const ws_address_t &address_, bool client_)
    : stream_engine_base_t (fd_, options_, endpoint_uri_pair_, true),
      _client (client_), _address (address_),
      _client_handshake_state (client_handshake_initial),
      _server_handshake_state (handshake_initial), _header_name_position (0),
      _header_value_position (0), _header_upgrade_websocket (false),
      _header_connection_upgrade (false), _heartbeat_timeout (0)
{
...
zmq::ws_engine_t::~ws_engine_t () { _close_msg.close (); }
  ...
  if (_client)
    {
      ...
      // The nonce doesn't have to be secure one, it is just use to avoid proxy
      // cache
...
int
zmq::ws_engine_t::routing_id_msg (msg_t *msg_)
{
...
int
zmq::ws_engine_t::process_routing_id_msg (msg_t *msg_)
{
...
bool
zmq::ws_engine_t::select_protocol (const char *protocol_)
{
  ...
  if (_options.mechanism == ZMQ_NULL && strcmp ("ZWS2.0/NULL", protocol_) == 0)
    {
          ...
          else if (c == '\r')
            {
              ...
              if (strcasecmp ("upgrade", _header_name) == 0)
                       ...
                       == 0)
                {
                  ...
                  // Sec-WebSocket-Protocol can appear multiple times or be a
                  // comma separated list if _websocket_protocol is already set
                  // we skip the check
  ...
  while (_insize > 0)
    {
      ...
      switch (_client_handshake_state)
        ...
        case response_line_HTTP_slash_1_dot_1:
          if (c == ' ')
            _client_handshake_state = response_line_HTTP_slash_1_dot_1_space;
          else
            _client_handshake_state = client_handshake_error;
          break;
        ...
        case client_header_field_value_trailing_space:
          if (c == '\n')
            ...
            _client_handshake_state = client_header_field_value_trailing_space;
          else
            {
              _header_value[0] = c;
              _header_value_position = 1;
        ...
        case client_header_field_value:
          if (c == '\n')
          ...
          else if (c == '\r')
            {
              ...
              else if (strcasecmp ("Sec-WebSocket-Accept", _header_name) == 0)
...
int
zmq::ws_engine_t::decode_and_push (msg_t *msg_)
{
  zmq_assert (_mechanism != NULL);

  //  with WS engine, ping and pong commands are control messages and should
  //  not go through any mechanism
  ...
  if (msg_->flags () & msg_t::command && !msg_->is_ping () && !msg_->is_pong ()
      && !msg_->is_close_cmd ())
...
int
zmq::ws_engine_t::produce_close_message (msg_t *msg_)
{
...
int
zmq::ws_engine_t::produce_no_msg_after_close (msg_t *msg_)
{
  LIBZMQ_UNUSED (msg_);
  _next_msg = static_cast<int (stream_engine_base_t::*) (msg_t *)> (
...
int
zmq::ws_engine_t::close_connection_after_close (msg_t *msg_)
{
  LIBZMQ_UNUSED (msg_);
...
int
zmq::ws_engine_t::produce_ping_message (msg_t *msg_)
{
  int rc = msg_->init ();
  errno_assert (rc == 0);
...
int
zmq::ws_engine_t::produce_pong_message (msg_t *msg_)
{
  int rc = msg_->init ();
  errno_assert (rc == 0);
...
int
zmq::ws_engine_t::process_command_message (msg_t *msg_)
{
  if (msg_->is_ping ())
    {
      _next_msg = static_cast<int (stream_engine_base_t::*) (msg_t *)> (
...
static void
compute_accept_key (char *key_, unsigned char *hash_)
{
