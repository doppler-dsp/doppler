...
int
getlen (const zmq::generic_mtrie_t<int>::prefix_t &data_)
{
...
void
mtrie_count (int *pipe_, int *count_)
{
  LIBZMQ_UNUSED (pipe_);
  ++*count_;
}
...
void
test_add_and_rm_other (const char *add_name_, const char *rm_name_)
{
  int addpipe, rmpipe;
  zmq::generic_mtrie_t<int> mtrie;
...
void
add_indexed_expect_unique (zmq::generic_mtrie_t<int> &mtrie_, int *pipes_,
                           const char **names_, size_t i_)
{
  ...
  TEST_ASSERT_EQUAL (zmq::generic_mtrie_t<int>::last_value_removed,
                     res); // FIXME asserting equality between enum and bool? I
                           // think first arg for macro should be "true"
}
...
template <size_t N>
void
add_entries (zmq::generic_mtrie_t<int> &mtrie_, int (&pipes_)[N],
             const char *(&names_)[N])
{
...
template <size_t N>
void
add_and_rm_entries (const char *(&names_)[N])
{
  int pipes[N];
  zmq::generic_mtrie_t<int> mtrie;
...
void
check_name (zmq::generic_mtrie_t<int>::prefix_t data_, size_t len_,
            const char *name_)
{
...
template <size_t N>
void
add_entries_rm_pipes_unique (const char *(&names_)[N])
{
  int pipes[N];
  zmq::generic_mtrie_t<int> mtrie;
  add_entries (mtrie, pipes, names_);
...
void
check_count (zmq::generic_mtrie_t<int>::prefix_t data_, size_t len_,
             int *count_)
{
  LIBZMQ_UNUSED (data_);
  LIBZMQ_UNUSED (len_);
...
void
add_duplicate_entry (zmq::generic_mtrie_t<int> &mtrie_, int (&pipes_)[2])
{
  const char *name = "foo";
