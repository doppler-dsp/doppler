/* bench_i16_buffer_core.c — the element-typed face of the i16 ring.
 *
...
main (void)
{
  ...
  for (int r = 0; r < ITERATIONS; r++)
    {
      ...
      for (int k = 0; k < BENCH_N / FRAME; k++)
        {
          dp_i16_write_some_view (ab, x, FRAME);
          dp_iq16_t *v = dp_i16_peek_view (ab, FRAME);
          sink += (v != NULL);
          dp_i16_consume (ab, FRAME);
        }
  ...
  jm_bench_write_json (&_bench, "i16_buffer");
  dp_i16_destroy (ab);
  return sink == 0;
}
