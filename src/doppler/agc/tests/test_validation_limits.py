"""The AGC report's certified envelope, as tests that actually run.
...
This module is `agc`'s copy of the fix, and it is deliberately thin: it
...
**The AGC's limits are the binding's, not the algorithm's.** doppler is
...
from doppler.agc.tests.validation.agc import validate as agc_validate
from doppler.tests._validation_common import assert_renders

OBJECTS = {"agc": agc_validate}
