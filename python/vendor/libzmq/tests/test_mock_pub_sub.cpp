...
static void
recv_with_retry (fd_t fd_, char *buffer_, int bytes_)
{
  int received = 0;
  while (true)
    {
...
static void
mock_handshake (fd_t fd_, bool sub_command, bool mock_pub)
{
  char buffer[128];
...
static void
prep_server_socket (void **server_out_, void **mon_out_, char *endpoint_,
                    size_t ep_length_, int socket_type)
{
...
static void
test_mock_pub_sub (bool sub_command_, bool mock_pub_)
{
  int rc;
  ...
  if (mock_pub_)
    {
      ...
      //  Because zeromq attach the pipe after the handshake, we need more time
      //  here before we can run the state-machine
      msleep (1);
  ...
  else
    {
      if (sub_command_)
        {
          const uint8_t sub[13]
              = { 4, 11, 9, 'S', 'U', 'B', 'S', 'C', 'R', 'I', 'B', 'E', 'A' };
