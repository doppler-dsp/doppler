...
* `sequential` True: test for a detection after EVERY block (the per-block CFAR ratio threshold  see \_ratio\_threshold() in carrier\_acq\_core.c  tightens as more looks accumulate), stopping the moment one fires or max\_n\_blocks is reached. False: accumulate silently and test once, at dwell\_target. 
