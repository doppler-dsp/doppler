/*
 * bench_telemetry_core.c — what instrumentation actually costs.
 *
 ...
 * to size a capture (tlm_capture.h) rather than run into the ring's ceiling.
 *
...
#undef ARM
  ...
  jm_bench_write_json (&_bench, "telemetry");
  free (sink);
  dp_tlm_destroy (t);
  return 0;
}
