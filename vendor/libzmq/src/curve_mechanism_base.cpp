...
//  libsodium added crypto_box_easy_afternm and crypto_box_open_easy_afternm
//  with https:
//  //github.com/jedisct1/libsodium/commit/aaf5fbf2e53a33b18d8ea9bdf2c6f73d7acc8c3e
...
int
zmq::curve_mechanism_base_t::encode (msg_t *msg_)
{
  return curve_encoding_t::encode (msg_);
}

int
zmq::curve_mechanism_base_t::decode (msg_t *msg_)
{
...
zmq::curve_encoding_t::curve_encoding_t (const char *encode_nonce_prefix_,
    ...
    : _encode_nonce_prefix (encode_nonce_prefix_),
      _decode_nonce_prefix (decode_nonce_prefix_), _cn_nonce (1),
      _cn_peer_nonce (1), _downgrade_sub (downgrade_sub_)
{
}
...
int
zmq::curve_encoding_t::check_validity (msg_t *msg_, int *error_event_code_)
{
...
int
zmq::curve_encoding_t::encode (msg_t *msg_)
{
  ...
  if (msg_->is_subscribe () || msg_->is_cancel ())
    {
      if (_downgrade_sub)
        ...
        sub_cancel_len = msg_->is_cancel () ? zmq::msg_t::cancel_cmd_name_size
  ...
  // For backward compatibility subscribe/cancel command messages are not
  // stored with the message flags, and are encoded in the encoder, so that
  // messages for < 3.0 peers can be encoded in the "old" 0/1 way rather than
  // as commands.
  if (sub_cancel_len == 1)
...
#ifdef ZMQ_HAVE_CRYPTO_BOX_EASY_FNS
  msg_t msg_box;
  int rc = msg_box.init_size (message_header_len + mlen + crypto_box_MACBYTES);
  zmq_assert (rc == 0);
  ...
  int rc = crypto_box_afternm (&message_box[0],
                               &message_plaintext_with_zerobytes[0], mlen,
...
int
zmq::curve_encoding_t::decode (msg_t *msg_, int *error_event_code_)
{
  ...
  if (rc == 0)
    {
      ...
      if (msg_->size () > 0)
        {
          memcpy (msg_->data (), &message_plaintext[flags_len], msg_->size ());
        }
#endif
