/* test_ddc_symbols.c -- generated by just-makeit (gh-1361).
 *
 * The address of every function the Python binding calls. This
 * file is linked into the C test, so a function that is declared
 * and never defined fails the build here, naming it -- rather than
 * at import, where a shared object's undefined symbol surfaces.
 * Regenerated by `jm apply`; do not edit.
 */
#include "doppler/ddc/ddc_core.h"

typedef void (*jm_any_fn)(void);

extern const jm_any_fn jm_bound_symbols_ddc[];
/* `used` keeps the compiler from dropping the table; `retain` keeps
 * the linker's --gc-sections from dropping it (measured: without it
 * a -Wl,--gc-sections build links a missing definition cleanly). On
 * Mach-O, `used` alone already marks it no-dead-strip. */
#if defined(__has_attribute)
#if __has_attribute(retain)
__attribute__((used, retain))
#elif __has_attribute(used)
__attribute__((used))
#endif
#endif
const jm_any_fn jm_bound_symbols_ddc[] = {
    (jm_any_fn)dp_ddc_create,
    (jm_any_fn)ddc_create_matched,
    (jm_any_fn)dp_ddc_destroy,
    (jm_any_fn)dp_ddc_reset,
    (jm_any_fn)dp_ddc_get_norm_freq,
    (jm_any_fn)dp_ddc_set_norm_freq,
    (jm_any_fn)dp_ddc_get_rate,
    (jm_any_fn)dp_ddc_execute,
    (jm_any_fn)dp_ddc_execute_ctrl,
    (jm_any_fn)dp_ddc_execute_ctrl_push,
    (jm_any_fn)dp_ddc_get_narrow_pulse,
    (jm_any_fn)dp_ddc_get_clipped,
    (jm_any_fn)dp_ddc_execute_max_out,
    (jm_any_fn)dp_ddc_state_bytes,
    (jm_any_fn)dp_ddc_get_state,
    (jm_any_fn)dp_ddc_set_state,
    (jm_any_fn)dp_ddc_execute_ctrl_max_out,
    (jm_any_fn)dp_ddc_execute_ctrl_push_max_out,
};
