...
zmq::pgm_sender_t::pgm_sender_t (io_thread_t *parent_,
    ...
    : io_object_t (parent_), has_tx_timer (false), has_rx_timer (false),
      session (NULL), encoder (0), more_flag (false),
      pgm_socket (false, options_), options (options_),
      handle (static_cast<handle_t> (NULL)),
      uplink_handle (static_cast<handle_t> (NULL)),
      rdata_notify_handle (static_cast<handle_t> (NULL)),
      pending_notify_handle (static_cast<handle_t> (NULL)), out_buffer (NULL),
      out_buffer_size (0), write_size (0)
{
  int rc = msg.init ();
  errno_assert (rc == 0);
}
...
int
zmq::pgm_sender_t::init (bool udp_encapsulation_, const char *network_)
{
...
void
zmq::pgm_sender_t::plug (io_thread_t *io_thread_, session_base_t *session_)
{
...
void
zmq::pgm_sender_t::restart_output ()
{
  set_pollout (handle);
  out_event ();
}
...
bool
zmq::pgm_sender_t::restart_input ()
{
  zmq_assert (false);
  return true;
}
...
zmq::pgm_sender_t::get_endpoint () const
{
...
void
zmq::pgm_sender_t::timer_event (int token)
{
