...
namespace zmq
{
//  Decoder for Web socket framing protocol. Converts data stream into
//  messages. The class has to inherit from shared_message_memory_allocator
//  because the base class calls allocate in its constructor.
class ws_decoder_t ZMQ_FINAL
    : public decoder_base_t<ws_decoder_t, shared_message_memory_allocator>
{
public:
  ws_decoder_t (size_t bufsize_, int64_t maxmsgsize_, bool zero_copy_,
                bool must_mask_);
  ~ws_decoder_t ();
