...
| **2-D roll** (2-D code FFT × signal FFT → 2-D IFFT) | `spectral.Corr2D` / `CorrDetector2D` (manual composition); natively via `acquire.BurstAcquisition`'s wideband fallback when `doppler_uncertainty` exceeds the native span, or unconditionally via `acquire.Acquisition` (continuous mode always uses this mechanism) | `1/nx` / `±1/2`                       | **1 epoch**            | wide Δf, few reps, enough SNR — whole grid in one 2-D op |
