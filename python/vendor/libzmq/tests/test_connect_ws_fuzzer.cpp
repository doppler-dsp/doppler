...
// Test that the ZMTP WebSocket engine handles invalid handshake when
// connecting https://rfc.zeromq.org/spec/45/
extern "C" int
...
test_connect_ws_fuzzer ()
{
  ...
  if (fuzzer_corpus_encode (
          "tests/libzmq-fuzz-corpora/test_connect_ws_fuzzer_seed_corpus",
          &data, &len, &num_cases)
      != 0)
    exit (77);
