...
int
test_defaults (int send_hwm_, int msg_cnt_, const char *endpoint_)
{
...
int
receive (void *socket_, int *is_termination_)
{
  int recv_count = 0;
  *is_termination_ = 0;
...
int
test_blocking (int send_hwm_, int msg_cnt_, const char *endpoint_)
{
  char pub_endpoint[SOCKET_STRING_LEN];
  ...
  // dequeue SUB socket again, to make sure XPUB has space to send the
  // termination message
  recv_count += receive (sub_socket, &is_termination);
...
void
test_defaults_large (const char *bind_endpoint_)
{
...
void
test_defaults_small (const char *bind_endpoint_)
{
...
void
test_blocking (const char *bind_endpoint_)
{
...
#define DEFINE_REGULAR_TEST_CASES(name, bind_endpoint)                        \
  void test_defaults_large_##name () { test_defaults_large (bind_endpoint); } \
                                                                              \
  void test_defaults_small_##name () { test_defaults_small (bind_endpoint); } \
                                                                              \
