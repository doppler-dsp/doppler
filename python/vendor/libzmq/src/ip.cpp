...
zmq::fd_t
zmq::open_socket (int domain_, int type_, int protocol_)
{
  int rc;
...
void
zmq::enable_ipv4_mapping (fd_t s_)
{
  LIBZMQ_UNUSED (s_);
...
int
zmq::get_peer_ip_address (fd_t sockfd_, std::string &ip_addr_)
{
  ...
  if (addrlen == 0)
    {
      ...
      wsa_assert (last_error != WSANOTINITIALISED && last_error != WSdpFAULT
                  && last_error != WSdpINPROGRESS
                  && last_error != WSdpNOTSOCK);
...
void
zmq::set_ip_type_of_service (fd_t s_, int iptos_)
{
...
void
zmq::set_socket_priority (fd_t s_, int priority_)
{
...
int
zmq::bind_to_device (fd_t s_, const std::string &bound_device_)
{
#ifdef ZMQ_HAVE_SO_BINDTODEVICE
  int rc = setsockopt (s_, SOL_SOCKET, SO_BINDTODEVICE, bound_device_.c_str (),
                       bound_device_.length ());
  if (rc != 0)
    {
...
#if defined ZMQ_HAVE_WINDOWS
static void
tune_socket (const SOCKET socket_)
{
  BOOL tcp_nodelay = 1;
...
static int
make_fdpair_tcpip (zmq::fd_t *r_, zmq::fd_t *w_)
{
  ...
  //  Use problematic Event implementation for compatibility if using old port
  //  5905. Otherwise use Mutex implementation.
...
#else
      sync = CreateEventW (NULL, FALSE, TRUE,
      ...
      while (still_to_send || still_to_recv)
        {
          ...
          if (still_to_send > 0)
            {
              nbytes = ::send (*w_,
                               reinterpret_cast<char *> (dummy + dummy_size
                                                         - still_to_send),
                               still_to_send, 0);
...
int
zmq::make_fdpair (fd_t *r_, fd_t *w_)
{
#if defined ZMQ_HAVE_EVENTFD
  int flags = 0;
  ...
  *w_ = *r_ = fd;
  ...
  if (listener == retired_fd)
    {
      //  This may happen if the library was built on a system supporting
      //  AF_UNIX, but the system running doesn't support it.
      goto try_tcpip;
    }
...
try_tcpip:
  ...
  *r_ = accept (listener, NULL, NULL);
  ...
  int on = 1;
  int rc = setsockopt (listener, IPPROTO_TCP, TCP_NODELAY, (char *)&on,
                       sizeof on);
  errno_assert (rc != -1);
...
void
zmq::make_socket_noninheritable (fd_t sock_)
{
...
void
zmq::assert_success_or_recoverable (zmq::fd_t s_, int rc_)
{
#ifdef ZMQ_HAVE_WINDOWS
  if (rc_ != SOCKET_ERROR)
    {
  ...
  if (err != 0)
    {
      wsa_assert (err == WSdpCONNREFUSED || err == WSdpCONNRESET
                  || err == WSdpCONNABORTED || err == WSdpINTR
                  || err == WSdpTIMEDOUT || err == WSdpHOSTUNREACH
                  || err == WSdpNETUNREACH || err == WSdpNETDOWN
                  || err == WSdpNETRESET || err == WSdpACCES
                  || err == WSdpINVAL || err == WSdpADDRINUSE);
    }
#else
...
#if defined ZMQ_HAVE_WINDOWS
char *
widechar_to_utf8 (const wchar_t *widestring)
{
  int nch, n;
  char *utf8 = 0;
...
int
zmq::create_ipc_wildcard_address (std::string &path_, std::string &file_)
{
#if defined ZMQ_HAVE_WINDOWS
