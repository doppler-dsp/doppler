

# Struct dp\_specan\_state\_t



[**ClassList**](annotated.md) **>** [**dp\_specan\_state\_t**](structdp__specan__state__t.md)



_Specan state. Allocate with_ [_**dp\_specan\_create()**_](specan__core_8h.md#function-dp_specan_create) _._

* `#include <specan_core.h>`





















## Public Attributes

| Type | Name |
| ---: | :--- |
|  double | [**beta**](#variable-beta)  <br> |
|  double | [**center**](#variable-center)  <br> |
|  [**dp\_ddc\_state\_t**](ddc__core_8h.md#typedef-dp_ddc_state_t) \* | [**ddc**](#variable-ddc)  <br> |
|  size\_t | [**disp\_lo**](#variable-disp_lo)  <br> |
|  size\_t | [**disp\_n**](#variable-disp_n)  <br> |
|  double | [**fs\_in**](#variable-fs_in)  <br> |
|  double | [**fs\_out**](#variable-fs_out)  <br> |
|  size\_t | [**n**](#variable-n)  <br> |
|  size\_t | [**navg**](#variable-navg)  <br> |
|  size\_t | [**nfft**](#variable-nfft)  <br> |
|  double | [**offset\_db**](#variable-offset_db)  <br> |
|  float \_Complex \* | [**pend**](#variable-pend)  <br> |
|  size\_t | [**pend\_cap**](#variable-pend_cap)  <br> |
|  size\_t | [**pend\_len**](#variable-pend_len)  <br> |
|  [**dp\_psd\_state\_t**](structdp__psd__state__t.md) \* | [**psd**](#variable-psd)  <br> |
|  float \* | [**pwr**](#variable-pwr)  <br> |
|  double | [**rbw**](#variable-rbw)  <br> |
|  float \_Complex \* | [**scratch**](#variable-scratch)  <br> |
|  size\_t | [**scratch\_cap**](#variable-scratch_cap)  <br> |
|  double | [**span**](#variable-span)  <br> |
|  double | [**src\_center**](#variable-src_center)  <br> |












































## Public Attributes Documentation




### variable beta 

```C++
double dp_specan_state_t::beta;
```



Kaiser beta realising [**rbw**](structdp__specan__state__t.md#variable-rbw). 
 


        

<hr>



### variable center 

```C++
double dp_specan_state_t::center;
```



Display center frequency, Hz. 
 


        

<hr>



### variable ddc 

```C++
dp_ddc_state_t* dp_specan_state_t::ddc;
```



Tuner + decimator (mix to DC, resample). 
 


        

<hr>



### variable disp\_lo 

```C++
size_t dp_specan_state_t::disp_lo;
```



First display bin in the DC-centred array. 


        

<hr>



### variable disp\_n 

```C++
size_t dp_specan_state_t::disp_n;
```



Display band length (cropped bins). 
 


        

<hr>



### variable fs\_in 

```C++
double dp_specan_state_t::fs_in;
```



Input sample rate, Hz. 
 


        

<hr>



### variable fs\_out 

```C++
double dp_specan_state_t::fs_out;
```



Decimated rate, Hz (= span·1.28, ≤ fs\_in). 


        

<hr>



### variable n 

```C++
size_t dp_specan_state_t::n;
```



Segment / window length (samples). 
 


        

<hr>



### variable navg 

```C++
size_t dp_specan_state_t::navg;
```



Segments averaged per emitted frame. 
 


        

<hr>



### variable nfft 

```C++
size_t dp_specan_state_t::nfft;
```



Zero-padded transform length. 
 


        

<hr>



### variable offset\_db 

```C++
double dp_specan_state_t::offset_db;
```



Additive dB offset on the display (dBm cal). 


        

<hr>



### variable pend 

```C++
float _Complex* dp_specan_state_t::pend;
```



Decimated samples awaiting a frame. 
 


        

<hr>



### variable pend\_cap 

```C++
size_t dp_specan_state_t::pend_cap;
```



Elements allocated in [**pend**](structdp__specan__state__t.md#variable-pend). 
 


        

<hr>



### variable pend\_len 

```C++
size_t dp_specan_state_t::pend_len;
```



Valid samples in [**pend**](structdp__specan__state__t.md#variable-pend). 
 


        

<hr>



### variable psd 

```C++
dp_psd_state_t* dp_specan_state_t::psd;
```



Averaging PSD at the decimated rate. 
 


        

<hr>



### variable pwr 

```C++
float* dp_specan_state_t::pwr;
```



Two-sided linear power scratch, length nfft. 


        

<hr>



### variable rbw 

```C++
double dp_specan_state_t::rbw;
```



Requested resolution bandwidth, Hz. 
 


        

<hr>



### variable scratch 

```C++
float _Complex* dp_specan_state_t::scratch;
```



Ddc output scratch, capacity scratch\_cap. 
 


        

<hr>



### variable scratch\_cap 

```C++
size_t dp_specan_state_t::scratch_cap;
```



Elements allocated in [**scratch**](structdp__specan__state__t.md#variable-scratch). 
 


        

<hr>



### variable span 

```C++
double dp_specan_state_t::span;
```



Display span, Hz. 
 


        

<hr>



### variable src\_center 

```C++
double dp_specan_state_t::src_center;
```



Source center frequency, Hz. 
 


        

<hr>

------------------------------
The documentation for this class was generated from the following file `native/inc/doppler/specan/specan_core.h`

