...
{
public:
  stream_engine_base_t (fd_t fd_, const options_t &options_,
...
protected:
  ...
  virtual int
  process_heartbeat_message (msg_t *msg_)
  {
    LIBZMQ_UNUSED (msg_);
    return -1;
  };
  virtual int
  produce_pong_message (msg_t *msg_)
  {
    LIBZMQ_UNUSED (msg_);
    return -1;
  };
...
private:
  ...
  //  Indicate if engine has an handshake stage, if it does, engine must call
  //  session.engine_ready when handshake is completed.
