...
- **Status.** **CERTIFIED** — all 43 limits hold.
- **Findings.** 12 recorded, 6 still open: F4, F5, F6, F7, F8, F9.

**What a caller most needs to know**

- **Seed it.** A residual inside `|df| <= bn/M` is locked within `2/bn` samples, and inside that window the settling time does not depend on the offset at all — constant, not merely bounded (worst case measured 1.42 in units of `bn/M`, 0.85 at 6 dB Es/N0). So a coarse acquisition in front of this loop needs a frequency bin no wider than `2*bn/M` (§2.5).
- **Its worst failure is silent, and reproducible.** Every `k/M` is a stable lock of the same width as the true one: the loop sits there reporting a lock statistic of +1.0000 with the tracked frequency wrong by exactly `1/M`. Nothing self-referenced detects it — it takes an external reference, a sync word, or a search bounded to `|df| < 1/(2M)` (§2.3).
- **`lock` and `locked` are telemetry, never control.** They describe the M-th power, not whether the loop is working: of 18 Es/N0 points swept, 14 track the carrier and only 8 declare, and an RRC-shaped input at M >= 4 tracks to 1e-6 while reading `locked = False`. A receiver gated on `locked` will refuse a working loop (§2.2).
- **Pull-in stops at a frequency, not at a multiple of `bn`.** The arm window sets an absolute ceiling — 0.03066 cycles/sample at the shipped geometry, M = 4 — which does not move with `bn` and is 4.1x tighter than the `1/(2M)` aliasing ceiling. Widening the arm tightens it further (§2.4).
- **The input level cannot reach the loop.** Eight decades of amplitude move the tracked carrier by at most 8.5e-09 cycles/sample: the discriminator divides out its own `|z|^M`, so there is no AGC in this object and none is wanted (§2.9).
- **What `n` costs is the table in §2.6, not the header's closed form.** `1/2 + 1/(M+1)` does not reproduce — measured 0.9267 / 0.7416 / 0.5870 for the half-symbol arm against 0.8333 / 0.7000 / 0.6111 predicted (gh-733).

## 1. The object — design and expectations

`dp_carrier_nda_state_t` is **carrier recovery that needs neither data nor symbol timing**. Per sample it de-rotates the input with an integer-phase NCO, slides a free-running I/Q boxcar of `sps/n` samples, and runs an M-th-power discriminator on the window average — raising the arm sample to the Mth power strips the M-PSK modulation, leaving M times the carrier phase. That is what lets it acquire a bare carrier, or a modulated one before timing lock, and it is why the same mechanism costs an M-fold ambiguity in phase **and in frequency**.

### Where the design lives — this report does not restate it

| source | holds |
|---|---|
| [`native/inc/doppler/carrier_nda/carrier_nda_core.h`](../../../../../../native/inc/doppler/carrier_nda/carrier_nda_core.h) | the contract: the acquisition bounds and which one is silent, what the arm window costs, the threshold chain, and why the verify count is not derived from it |
