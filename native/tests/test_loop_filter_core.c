...
main (void)
{
   ...
   * ------------------------------------------------------------------ */
  {
    dp_loop_filter_state_t *lf = dp_loop_filter_create (bn, zeta, t);
    DP_CHECK (lf != NULL);
    if (!lf)
      return 1;
    ...
    dp_loop_filter_state_t *edge = dp_loop_filter_create (0.0, zeta, t);
    DP_CHECK (edge != NULL); /* bn = 0 is IN the domain — see §10 */
    dp_loop_filter_destroy (edge);

    DP_CHECK (dp_loop_filter_create (-1e-9, zeta, t) == NULL);
    DP_CHECK (dp_loop_filter_create (bn, zeta, 0.0) == NULL);
    DP_CHECK (dp_loop_filter_create (bn, zeta, -1.0) == NULL);
    DP_CHECK (dp_loop_filter_create (bn, 0.0, t) == NULL);
    DP_CHECK (dp_loop_filter_create (bn, -1.0, t) == NULL);
    /* Non-finite in any argument: t = INFINITY used to give NaN gains, and
       a NaN gain poisons every later update permanently. */
    DP_CHECK (dp_loop_filter_create (INFINITY, zeta, t) == NULL);
    DP_CHECK (dp_loop_filter_create (bn, INFINITY, t) == NULL);
    DP_CHECK (dp_loop_filter_create (bn, zeta, INFINITY) == NULL);
    DP_CHECK (dp_loop_filter_create (NAN, zeta, t) == NULL);
    DP_CHECK (dp_loop_filter_create (bn, NAN, t) == NULL);
    DP_CHECK (dp_loop_filter_create (bn, zeta, NAN) == NULL);
  }
   ...
   * ------------------------------------------------------------------ */
  {
    ...
    double ctl = dp_loop_filter_step (lf, 1.0);
    ...
    for (int i = 0; i < 9; i++)
      (void)dp_loop_filter_step (lf, 1.0);
    ...
    lf->integ  = 0.0;
    double neg = dp_loop_filter_step (lf, -1.0);
   ...
   * ------------------------------------------------------------------ */
  {
    ...
    dp_loop_filter_state_t *blk = dp_loop_filter_create (bn, zeta, t);
    dp_loop_filter_state_t *ref = dp_loop_filter_create (bn, zeta, t);

    /* Split the block in two so the carry ACROSS calls is exercised, not
       just the carry within one. */
    dp_loop_filter_steps (blk, x, got, 20);
    dp_loop_filter_steps (blk, x + 20, got + 20, (size_t)(N - 20));

    for (int i = 0; i < N; i++)
      {
        double want = dp_loop_filter_step (ref, x[i]);
    ...
    dp_loop_filter_steps (blk, NULL, NULL, 0);
    DP_CHECK (blk->integ == before);

    dp_loop_filter_destroy (blk);
    dp_loop_filter_destroy (ref);
  }
   ...
   * ------------------------------------------------------------------ */
  {
    ...
    dp_loop_filter_state_t *a = dp_loop_filter_create (bn, zeta, t);
    dp_loop_filter_state_t *b = dp_loop_filter_create (bn, zeta, t);

    dp_loop_filter_steps (a, x, sep, N); /* separate buffers */

    memcpy (both, x, sizeof both);
    dp_loop_filter_steps (b, both, both, N); /* fully aliased */

    for (int i = 0; i < N; i++)
      DP_CHECK_NEAR (both[i], sep[i], 1e-12);

    dp_loop_filter_destroy (a);
    dp_loop_filter_destroy (b);
  }
   ...
   * ------------------------------------------------------------------ */
  {
    ...
    z.integ = 0.75;
    for (int i = 0; i < 100; i++)
      DP_CHECK (dp_loop_filter_step (&z, 1.0) == 0.75);
   ...
   * ------------------------------------------------------------------ */
  {
    dp_loop_filter_state_t *a = dp_loop_filter_create (0.01, 0.707, 1.0);
    for (int i = 0; i < 30; i++)
      (void)dp_loop_filter_step (a, 0.1);

    unsigned char blob[64];
    DP_CHECK (dp_loop_filter_state_bytes (a) <= sizeof blob);
    dp_loop_filter_get_state (a, blob);

    double refv = 0.0;
    for (int i = 0; i < 10; i++)
      refv = dp_loop_filter_step (a, 0.1); /* reference continuation */

    dp_loop_filter_state_t *b = dp_loop_filter_create (0.01, 0.707, 1.0);
    DP_CHECK (dp_loop_filter_set_state (b, blob) == DP_OK);
    ...
    DP_CHECK (dp_loop_filter_set_state (b, blob) == DP_ERR_INVALID);
    DP_CHECK (b->integ == integ_after_restore);
    blob[0] ^= (unsigned char)0xFF;

    double gotv = 0.0;
    for (int i = 0; i < 10; i++)
      gotv = dp_loop_filter_step (b, 0.1);
    ...
    dp_loop_filter_state_t *c = dp_loop_filter_create (0.05, 0.5, 4.0);
    DP_CHECK (c->bn == 0.05);
    DP_CHECK (dp_loop_filter_set_state (c, blob) == DP_OK);
    DP_CHECK (c->bn == 0.01);
    DP_CHECK (c->zeta == 0.707);
    DP_CHECK (c->t == 1.0);
    DP_CHECK (c->kp == a->kp && c->ki == a->ki);
    dp_loop_filter_destroy (c);
