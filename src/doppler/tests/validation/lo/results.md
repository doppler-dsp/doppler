...
> **There is no `docs/design/lo.md`.** The LO's rationale lives entirely in the header; the conversion's rationale lives in `nco_core.h`, whose own design page is still stranded in PR #647 (see the NCO report's provenance note). Nothing was moved or restated here to paper over that.
...
| F4 | GAP | both max_out doc lines are stale. lo_core.h calls steps_max_out() 'maximum samples per call' and lo_core.c says 'calling with n > 65536 overflows the buffer and is undefined behaviour'. Measured: steps(70000) returns 70000 correct samples and steps_ctrl the same — pass_capacity (jm gh-138) made the caller's capacity the bound and the Python binding grows its buffer. 65536 is a pre-allocation hint; the sentences describe the pre-gh-138 contract. |
...
- **9 findings**, 5 of them gaps or confirmed defects: F2, F3, F4, F5, F6
- **26/26 limits** hold
