...
zmq::shared_message_memory_allocator::shared_message_memory_allocator (
    ...
    : _buf (NULL), _buf_size (0), _max_size (bufsize_), _msg_content (NULL),
      _max_counters ((_max_size + msg_t::max_vsm_size - 1)
                     / msg_t::max_vsm_size)
{
}
      ...
      if (c->sub (1))
        {
          ...
          // new one release pointer because we are going to create a new
          // buffer
          release ();
        }
    }
...
void
zmq::shared_message_memory_allocator::call_dec_ref (void *, void *hint_)
{
  zmq_assert (hint_);
