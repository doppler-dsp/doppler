...
`wfm`'s copy of the campaign's limits gate, and the module's first. Each
object under `validation/` ends its report with a **Limits** section
headed "Claims a caller may rely on. A failure here is a regression, not
a new finding" -- and that sentence is only true if something executes
it.
...
would. `write=False` suppresses `results.md`, the plots and the CSVs -- a
test must not write into the repo -- while every measurement still
executes.
...
`doppler.wfm`, and a shared collector would import every module's
