...
static PyMethodDef AGCObj_methods[] = {
  ...
  { "set_telemetry", (PyCFunction)(void *)AGCObj_set_telemetry,
    ...
    ">>> gain = recs[recs[\"probe\"] == "
    "tlm.probe_id(\"agc.gain_db\")][\"value\"]\n"
    ">>> lvl = recs[recs[\"probe\"] == "
    "tlm.probe_id(\"agc.level_db\")][\"value\"]\n"
...
static PyTypeObject AGCObjType = {
  ...
  = "Construct a log-domain feedback AGC and return its heap state. The loop\n"
    ...
    "    Loop noise bandwidth in cycles/sample. The FILTER's time constant "
    "is\n"
    "    1/(4*loop_bw) samples; the object settles more slowly than that on "
    "a\n"
    "    quiet input, because the detector is inside the loop and measures "
    "in\n"
    "    power (see the Linear-in-dB note above — measured 1.7x to 2.2x at "
    "-40\n"
