...
void
test__zmq_z85_encode__invalid__failure (size_t size_)
{
  errno = 0;
...
void
test__zmq_z85_decode__invalid__failure (const char (&encoded_)[SIZE])
{
...
// call zmq_z85_encode, then zmq_z85_decode, and compare the results with the
// original
template <size_t SIZE>
void
...
#define def_test__zmq_z85_decode__invalid__failure(name, param)               \
  def_test__zmq_z85_basename (decode__invalid__failure, name, param)
    ...
    def_test__zmq_z85_decode__invalid__failure (indivisble_by_5_multiple_chars,
                                                "01234567")
        def_test__zmq_z85_decode__invalid__failure (indivisble_by_5_one_char,
                                                    "0")
    ...
    // decode invalid data with an invalid character just below the range of
    // valid characters
    def_test__zmq_z85_decode__invalid__failure (char_adjacent_below,
                                                "####\0200")

    // decode invalid data with an invalid character just above the range of
    // valid characters
    def_test__zmq_z85_decode__invalid__failure (char_adjacent_above,
                                                "####\0037")
