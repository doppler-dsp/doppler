...
int main(void)
{
    ...
    /* ── steps() matches per-sample loop ─────────────────────────────── */
    {
        ...
        CHECK(a && b);
        float input[32]; uint64_t bulk[32], loop[32];
        for (int i = 0; i < 32; i++) input[i] = -1.5f + 3.0f * i / 31.0f;
        f32_to_i16u64_steps(a, input, bulk, 32);
        for (int i = 0; i < 32; i++) loop[i] = f32_to_i16u64_step(b, input[i]);
        CHECK(memcmp(bulk, loop, 32 * sizeof(uint64_t)) == 0);
        f32_to_i16u64_destroy(a); f32_to_i16u64_destroy(b);
    }
