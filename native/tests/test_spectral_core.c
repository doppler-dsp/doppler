...
main (void)
{
  ...
  {
    static float bh[N];
    dp_blackman_harris_window (bh, N);
    for (size_t i = 0; i < N; i++)
      {
    ...
    dp_hann_window (w, N);
    DP_CHECK (dp_kaiser_enbw (bh, N) > dp_kaiser_enbw (w, N));
  }
  ...
  /* ── kaiser_window / kaiser_enbw ────────────────────────────────── */
  {
    ...
    DP_CHECK (fabs ((double)dp_kaiser_enbw (w, N) - 1.0) < 1e-5);
    ...
    for (float beta = 0.0f; beta <= 12.0f; beta += 1.0f)
      {
        dp_kaiser_window (w, N, beta);
        const double e = (double)dp_kaiser_enbw (w, N);
    ...
    dp_hann_window (w, 8);
    DP_CHECK (fabs ((double)dp_kaiser_enbw (w, 8) - 1.7143) < 1e-3);
  }
  ...
  /* ── kaiser_beta_for_sidelobe: inverse of the Kaiser design rule ─── */
  {
    ...
    for (double a = 21.0; a <= 120.0; a += 5.0)
      {
        const double b = dp_kaiser_beta_for_sidelobe (a);
        DP_CHECK (b >= prev);
        prev = b;
      }
    /* Below 21 dB a rectangular window already meets the target. */
    DP_CHECK (dp_kaiser_beta_for_sidelobe (10.0) <= 1e-9);
    ...
    dp_kaiser_window (w40, 256, (float)dp_kaiser_beta_for_sidelobe (40.0));
    dp_kaiser_window (w80, 256, (float)dp_kaiser_beta_for_sidelobe (80.0));
    DP_CHECK (dp_kaiser_enbw (w80, 256) > dp_kaiser_enbw (w40, 256));
  }
  ...
  /* ── magnitude_db_cf32 / cf64: the same answer, two input widths ── */
  {
    ...
    dp_magnitude_db_cf32 (x32, 4, o32, 1e-12f, 0.0f);
    ...
    dp_magnitude_db_cf32 (x32, 4, o64, 1e-12f, -13.5f);
    for (int i = 0; i < 4; i++)
    ...
    dp_magnitude_db_cf64 (x64, 4, o64, 1e-12, 0.0f);
    for (int i = 0; i < 3; i++)
  ...
  /* ── find_peaks_f32: the header's example, then the ordering ────── */
  {
    ...
    size_t n = dp_find_peaks_f32 (db, 32, 2, -30.0f, pk);
  ...
  /* ── noise_floor_db: a MEDIAN, so tones must not move it ────────── */
  {
    ...
    DP_CHECK (fabs (dp_noise_floor_db (db, 101) - (-70.0)) < 1e-6);
    ...
    for (int i = 0; i < 101; i++)
      db[i] = (float)(-100 + i);
    DP_CHECK (fabs (dp_noise_floor_db (db, 101) - (-50.0)) < 1e-6);

    DP_CHECK (dp_noise_floor_db (db, 0) == 0.0);
  }
  ...
  /* ── obw_from_power: (1-frac)/2 excluded at EACH end ─────────────── */
  {
    ...
    for (int i = 0; i < 100; i++)
      pwr[i] = 1.0;
    DP_CHECK (fabs (dp_obw_from_power (pwr, 100, fs, 0.90) - 900.0) < 20.0);
    DP_CHECK (fabs (dp_obw_from_power (pwr, 100, fs, 0.50) - 500.0) < 20.0);

    /* Monotone in frac: asking for more power cannot need less band. */
    double prev = -1.0;
    for (double f = 0.1; f <= 0.99; f += 0.1)
      {
        const double b = dp_obw_from_power (pwr, 100, fs, f);
    ...
    DP_CHECK (dp_obw_from_power (pwr, 100, fs, 0.99) < 200.0);
    ...
    DP_CHECK (fabs (dp_obw_from_power (scaled, 100, fs, 0.99)
                    - dp_obw_from_power (pwr, 100, fs, 0.99))
              < 1e-9);

    /* Degenerate inputs return 0 rather than a plausible bandwidth. */
    DP_CHECK (dp_obw_from_power (pwr, 0, fs, 0.99) == 0.0);
    for (int i = 0; i < 100; i++)
      pwr[i] = 0.0;
