 ...
 *   5. ctrl-port FM shift — dp_lo_steps_ctrl deviates frequency per sample
...
main (void)
{
   ...
   * ---------------------------------------------------------------- */
  {
    ...
    if (!lo)
      return 1;
    dp_lo_reset (lo);
    DP_CHECK (dp_lo_get_phase (lo) == 0u);
    dp_lo_destroy (lo);
  }
   ...
   * ---------------------------------------------------------------- */
  {
    dp_lo_state_t *a = dp_lo_create (0.1);
    dp_lo_state_t *b = dp_lo_create (0.1);
    float _Complex ref[16], blk[8];
    dp_lo_steps (a, 16, ref, 16);
    dp_lo_steps (b, 8, blk, 8);
    for (int i = 0; i < 8; i++)
   ...
   *
   * dp_lo_steps_ctrl with a constant ctrl offset of 0.25 should produce
   * the same output as dp_lo_steps at norm_freq + 0.25 — but without
   ...
   * ---------------------------------------------------------------- */
  {
    dp_lo_state_t *lo_ctrl = dp_lo_create (0.0);
    dp_lo_state_t *lo_ref  = dp_lo_create (0.25);
    ...
    dp_lo_steps_ctrl (lo_ctrl, ctrl, 8, out_ctrl, 8);
    dp_lo_steps (lo_ref, 8, out_ref, 8);

    for (int i = 0; i < 8; i++)
      DP_CHECK (near_c (out_ctrl[i], out_ref[i]));

    /* Base norm_freq is unchanged after steps_ctrl. */
    DP_CHECK (dp_lo_get_norm_freq (lo_ctrl) == 0.0);

    dp_lo_destroy (lo_ctrl);
    dp_lo_destroy (lo_ref);
  }
  ...
  {
    dp_lo_state_t *lo = dp_lo_create (0.25);
    DP_CHECK (dp_lo_get_norm_freq (lo) == 0.25);
    DP_CHECK (dp_lo_get_phase (lo) == 0u);
    DP_CHECK (dp_lo_get_phase_inc (lo) == 0x40000000u);

    dp_lo_set_phase (lo, 0x80000000u);
    DP_CHECK (dp_lo_get_phase (lo) == 0x80000000u);

    dp_lo_set_norm_freq (lo, 0.5);
    DP_CHECK (dp_lo_get_phase_inc (lo) == 0x80000000u);
    DP_CHECK (dp_lo_get_phase (lo) == 0x80000000u); /* unchanged */

    dp_lo_reset (lo);
    DP_CHECK (dp_lo_get_phase (lo) == 0u);
    DP_CHECK (dp_lo_get_phase_inc (lo) == 0x80000000u); /* unchanged */

    dp_lo_destroy (lo);
  }

  /* ----------------------------------------------------------------
   * 8. lo_step (inline single-sample) == dp_lo_steps (block), bit-exact
   *
   ...
   * ---------------------------------------------------------------- */
  {
    ...
    DP_CHECK (dp_lo_get_phase (&stp) == dp_lo_get_phase (blk));
    dp_lo_destroy (blk);
  }
   ...
   * (dp_lo_set_norm_freq) and nudging the phase (dp_lo_set_phase).  Changing
   ...
   * ---------------------------------------------------------------- */
  {
    ...
    dp_lo_set_norm_freq (&s, 0.2);                      /* retune */
    DP_CHECK (dp_lo_get_phase (&s) == ph_before);       /* no jump      */
    DP_CHECK (dp_lo_get_phase_inc (&s) == 0x33333333u); /* 0.2 * 2^32   */
    (void)lo_step (&s);
    DP_CHECK (dp_lo_get_phase (&s) == ph_before + 0x33333333u); /* new inc  */

    /* exact integer phase nudge (proportional term of a loop) */
    uint32_t ph2 = dp_lo_get_phase (&s);
    dp_lo_set_phase (&s, ph2 + 0x10000000u);
    DP_CHECK (dp_lo_get_phase (&s) == ph2 + 0x10000000u);
  }
   ...
   * fractional part, and lo_step must match dp_lo_steps for them too.
   * ---------------------------------------------------------------- */
  {
   ...
   * ---------------------------------------------------------------- */
  {
    ...
    dp_lo_state_t *ref = dp_lo_create (0.123456);
    float _Complex full[256];
    dp_lo_steps (ref, N + M, full, N + M);
    dp_lo_destroy (ref);

    dp_lo_state_t *a = dp_lo_create (0.123456);
    float _Complex tmp[256];
    dp_lo_steps (a, N, tmp, N); /* advance N */

    DP_CHECK (dp_lo_state_bytes (a)
              == sizeof (dp_state_hdr_t) + sizeof (uint32_t));
    unsigned char blob[32];
    dp_lo_get_state (a, blob);
    dp_lo_destroy (a);

    dp_lo_state_t *b
        = dp_lo_create (0.123456); /* fresh, from the descriptor */
    DP_CHECK (dp_lo_set_state (b, blob) == DP_OK);
    float _Complex resumed[256];
    dp_lo_steps (b, M, resumed, M);
    dp_lo_destroy (b);
  ...
  {
    dp_lo_state_t *a = dp_lo_create (0.2);
    dp_lo_state_t *b = dp_lo_create (0.2);
    dp_lo_steps (a, 37, (float _Complex[37]){ 0 }, 37);
    DP_STATE_ROUNDTRIP_TEST (dp_lo, a, b);
    dp_lo_destroy (a);
    dp_lo_destroy (b);
  }
   ...
   * ---------------------------------------------------------------- */
  {
    ...
    dp_lo_state_t *lo50 = dp_lo_create (50.0 / fs);
    DP_CHECK (dp_lo_get_phase_inc (lo50) == 10226u);
    double actual50 = (double)dp_lo_get_phase_inc (lo50) / 4294967296.0 * fs;
    ...
    dp_lo_state_t *lo51 = dp_lo_create (51.0 / fs);
    DP_CHECK (dp_lo_get_phase_inc (lo51) == 10430u);
    double actual51 = (double)dp_lo_get_phase_inc (lo51) / 4294967296.0 * fs;
  ...
  /* ── pass_capacity: emission stops at max_out (jm gh-138) ────────── */
  {
    ...
    dp_lo_state_t *lo  = dp_lo_create (0.01);
    dp_lo_state_t *ref = dp_lo_create (0.01);
    float _Complex out[16];
    float _Complex expect[5];
    for (int i = 0; i < 16; i++)
      out[i] = 42.0f + 42.0f * I;

    DP_CHECK (dp_lo_steps (lo, 16, out, 5) == 5);
    for (int i = 5; i < 16; i++)
      DP_CHECK (out[i] == 42.0f + 42.0f * I); /* tail untouched */
    dp_lo_steps (ref, 5, expect, 5);
    for (int i = 0; i < 5; i++)
      DP_CHECK (out[i] == expect[i]);
    DP_CHECK (dp_lo_get_phase (lo)
              == dp_lo_get_phase (ref)); /* advanced by 5, not 16 */

    /* Zero capacity emits nothing and does not advance the phase. */
    uint32_t before = dp_lo_get_phase (lo);
    DP_CHECK (dp_lo_steps (lo, 16, out, 0) == 0);
    DP_CHECK (dp_lo_get_phase (lo) == before);
    ...
    DP_CHECK (dp_lo_steps_ctrl (lo, ctrl, 8, cout, 3) == 3);
    for (int i = 3; i < 8; i++)
   ...
   * ---------------------------------------------------------------- */
  {
    ...
    DP_CHECK (exact);
    DP_CHECK (dp_lo_get_phase (&plain) == dp_lo_get_phase (&zeroed));
    ...
    for (int i = 0; i < 64; i++)
      if (crealf (a[i]) != crealf (b[i]) || cimagf (a[i]) != cimagf (b[i]))
        exact = 0;
    DP_CHECK (exact);
    DP_CHECK (dp_lo_get_phase (&down) == dp_lo_get_phase (quarter));
    dp_lo_destroy (quarter);
    ...
    DP_CHECK (dp_lo_get_phase (&f0) != 0u); /* not the vacuous comparison */
    DP_CHECK (dp_lo_get_phase (&f1) == dp_lo_get_phase (&f0));
    DP_CHECK (dp_lo_get_phase (&f2) == dp_lo_get_phase (&f0));
    ...
    DP_CHECK (dp_lo_get_norm_freq (&keep) == 0.1);
    uint32_t ph_after_steer = dp_lo_get_phase (&keep);
    (void)lo_step_ctrl (&keep, 0.0);
    DP_CHECK (dp_lo_get_phase (&keep) == ph_after_steer + inc0);
  ...
  /* ----------------------------------------------------------------
   * 17. dp_lo_steps_ctrl is exactly a loop over lo_step_ctrl.
   *
   ...
   * ---------------------------------------------------------------- */
  {
    ...
    for (size_t i = 0; i < N; i++)
      if (crealf (blk[i]) != crealf (one[i])
        ...
        exact = 0;
    DP_CHECK (exact);
    DP_CHECK (dp_lo_get_phase (bs) == dp_lo_get_phase (&ss));
    /* the block form must not persist the control either */
    DP_CHECK (dp_lo_get_phase_inc (bs) == dp_lo_get_phase_inc (&ss));
    DP_CHECK (dp_lo_get_norm_freq (bs) == 0.077);
    dp_lo_destroy (bs);
  }

  /* ----------------------------------------------------------------
   * 18. dp_lo_destroy(NULL) is a no-op.
   *
   ...
   * ---------------------------------------------------------------- */
  {
    DP_CHECK (dp_lo_steps_max_out (NULL) == 65536u);
    DP_CHECK (dp_lo_steps_ctrl_max_out (NULL) == 65536u);
    ...
    if (big)
      {
        dp_lo_state_t *lo = dp_lo_create (0.013);
        DP_CHECK (dp_lo_steps (lo, BIG, big, BIG) == BIG);
        /* every sample really was written: the phase advanced by all of
         * them, and the tail is on the unit circle rather than zeroed */
        dp_lo_state_t *chk = dp_lo_create (0.013);
        DP_CHECK (dp_lo_get_phase (lo)
                  == (uint32_t)((uint64_t)dp_lo_get_phase_inc (chk)
                                * (uint64_t)BIG));
   ...
   * ---------------------------------------------------------------- */
  {
    ...
    dp_lo_state_t *sweep = dp_lo_create (0.10000000017);
    float _Complex buf[4096];
    float worst_m = 0.0f;
    for (int blk = 0; blk < 64; blk++)
      {
        dp_lo_steps (sweep, 4096, buf, 4096);
   ...
   *     conversion.
   *
   * Section 15 pins truncation on the CONFIGURE path (dp_lo_create /
   * dp_lo_set_norm_freq).  The control port is the other way into the same
   ...
   * ---------------------------------------------------------------- */
  {
    ...
    dp_lo_state_t *cfg = dp_lo_create (f51);
    ...
    dp_lo_state_t *bsteer = dp_lo_create (0.0);
    const double   c1[1]  = { f51 };
    float _Complex o1[1];
    dp_lo_steps_ctrl (bsteer, c1, 1, o1, 1);
    ...
    DP_CHECK (dp_lo_get_phase (bsteer) <= 10430u + 4u);
    DP_CHECK (dp_lo_get_phase (bsteer) + 4u >= 10430u);

    dp_lo_destroy (cfg);
    dp_lo_destroy (bsteer);
  }
