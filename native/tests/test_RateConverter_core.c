...
static inline int _near(double a, double b, double tol)
    { return fabs(a - b) <= tol; }

static float _mean_amp(const float complex *v, size_t n)
{
    double s = 0.0;
...
static float complex *_dc_block(size_t n)
{
...
static void test_invalid_rate(void)
{
...
static void test_stage_labels(void)
{
    char buf[64];
...
static void test_dc_gain(void)
{
    ...
    for (size_t r = 0; r < n_rates; r++) {
        ...
        size_t out_cap = (rates[r] > 1.0)
                         ? (size_t)(N_IN * rates[r] + 4) : N_IN;
        ...
        if (n2 > 0) {
            ...
            if (!ok) {
                fprintf(stderr,
                    "FAIL dc_gain rate=%.6g amp=%.4f n_out=%zu\n",
                    rates[r], amp, n2);
                _fails++;
            }
        }
...
static void test_reset_reproducible(void)
{
    enum { N = 256 };
    float complex *in   = _dc_block(N);
...
static void test_execute_max_out(void)
{
