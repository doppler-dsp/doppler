...
zmq::null_mechanism_t::null_mechanism_t (session_base_t *session_,
    ...
    : mechanism_base_t (session_, options_),
      ...
      _ready_command_sent (false), _error_command_sent (false),
      _ready_command_received (false), _error_command_received (false),
...
int
zmq::null_mechanism_t::next_handshake_command (msg_t *msg_)
{
      ...
      if (status_code != "300")
        {
          ...
          const int rc
              = msg_->init_size (error_command_name_len + error_reason_len_size
                                 + status_code_len);
          zmq_assert (rc == 0);
...
int
zmq::null_mechanism_t::process_handshake_command (msg_t *msg_)
{
...
int
zmq::null_mechanism_t::process_ready_command (const unsigned char *cmd_data_,
                                              size_t data_size_)
{
...
int
zmq::null_mechanism_t::process_error_command (const unsigned char *cmd_data_,
                                              size_t data_size_)
{
