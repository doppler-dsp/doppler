...
reconnect_default ()
{
  ...
  // ZMQ_EVENT_CONNECT_RETRIED should be last event, because of timeout set
  // above
  int event;
  char *event_address;
...
reconnect_stop_on_refused ()
{
  ...
  // ZMQ_EVENT_CLOSED should be last event, because of ZMQ_RECONNECT_STOP set
  // above
  int event = 0;
  char *event_address;
  int rc = get_monitor_event_with_timeout (sub_mon, &event, &event_address,
...
// test stopping reconnect on connection refused
void
reconnect_stop_on_handshake_failed ()
{
...
#if 1
  // ZMQ_EVENT_DISCONNECTED should be last event, because of ZMQ_RECONNECT_STOP
  // set above
  expect_monitor_event (sub_mon, ZMQ_EVENT_CONNECT_DELAYED);
  expect_monitor_event (sub_mon, ZMQ_EVENT_CONNECTED);
