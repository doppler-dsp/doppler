...
void
zmq::poller_base_t::adjust_load (int amount_)
{
  if (amount_ > 0)
    _load.add (amount_);
  else if (amount_ < 0)
...
void
zmq::poller_base_t::add_timer (int timeout_, i_poll_events *sink_, int id_)
{
...
void
zmq::poller_base_t::cancel_timer (i_poll_events *sink_, int id_)
{
  ...
  //  ()` on already canceled (deleted) timer. As soon as that is resolved an
  //  'assert (false)' should be put here.
}
...
void
zmq::worker_poller_base_t::start (const char *name_)
{
  zmq_assert (get_load () > 0);
...
void
zmq::worker_poller_base_t::worker_routine (void *arg_)
{
