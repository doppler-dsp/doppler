...
static PyGetSetDef RateSync_getset[] = {
  { "bn", (getter)RateSync_getprop_bn, (setter)RateSync_setprop_bn,
    "Timing-loop noise bandwidth, normalised to the symbol rate. Writing it "
    "re-derives the loop gains at the damping already in use, exactly as "
    "configure() does.\n",
    NULL },
...
static PyTypeObject RateSyncObjType = {
  ...
  = "Create a RateSync instance.\n"
    ...
    "    Nominal samples per symbol. Any double >= `m` -- 17.33389 is as "
    "valid\n"
    "    as 4, because the terminal stage's accumulator is a double and the "
    "loop\n"
    "    only has to steer the strobe. That is the real-world case whenever "
    "the\n"
    "    ADC clock is free-running against the symbol clock.\n"
    "pulse : Literal[\"iandd\", \"rrc\"], default \"rrc\"\n"
    "    Matched-filter pulse shape: \"rrc\" (root-raised cosine, roll-off "
    "`beta`)\n"
    "    or \"iandd\" (unit rectangle one symbol wide -- the matched filter "
    "for a\n"
    "    rectangular symbol, and exactly what an integrate-and-dump "
    "computes).\n"
    "    The rectangle needs far fewer taps, so an NRZ link's matched filter "
    "is\n"
    "    cheaper.\n"
    ...
    "    transition gate half a symbol from the on-time strobe, which is why "
    "m\n"
    "    must be even and at least 2. The oversampled stream is a by-product "
    "of\n"
    "    the same dot products, not an extra cost. Use m >= 4 with\n"
    "    pulse=\"iandd\": the rectangle is one symbol wide, so at m=2 its "
    "matched\n"
    ...
    "    between them. The rule rests on that separation, not on a "
    "particular\n"
    "    pair of lock_stat values -- those move with sps and with the "
    "stream.\n"
