...
zmq::pgm_receiver_t::pgm_receiver_t (class io_thread_t *parent_,
    ...
    : io_object_t (parent_), has_rx_timer (false), pgm_socket (true, options_),
      options (options_), session (NULL), active_tsi (NULL), insize (0)
{
}
...
int
zmq::pgm_receiver_t::init (bool udp_encapsulation_, const char *network_)
{
  return pgm_socket.init (udp_encapsulation_, network_);
}

void
zmq::pgm_receiver_t::plug (io_thread_t *io_thread_, session_base_t *session_)
{
...
void
zmq::pgm_receiver_t::restart_output ()
{
  drop_subscriptions ();
}

bool
zmq::pgm_receiver_t::restart_input ()
{
...
zmq::pgm_receiver_t::get_endpoint () const
{
...
zmq::pgm_receiver_t::in_event ()
{
  ...
  // Keep the internal state as is so that restart_input would process the
  // right data
  if (active_tsi)
    {
      return;
...
int
zmq::pgm_receiver_t::process_input (v1_decoder_t *decoder)
{
  zmq_assert (session != NULL);
...
void
zmq::pgm_receiver_t::timer_event (int token)
{
...
void
zmq::pgm_receiver_t::drop_subscriptions ()
{
  msg_t msg;
  msg.init ();
