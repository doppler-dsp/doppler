...
void
bounce (void *server_, void *client_)
{
...
static void
send_bounce_msg_may_fail (void *socket_)
{
  int timeout = 250;
  TEST_ASSERT_SUCCESS_ERRNO (
...
static void
recv_bounce_msg_fail (void *socket_)
{
  int timeout = 250;
  char buffer[32];
  TEST_ASSERT_SUCCESS_ERRNO (
...
void
expect_bounce_fail (void *server_, void *client_)
{
  //  Send message from client to server
...
void
close_zero_linger (void *socket_)
{
  int linger = 0;
...
void
setup_test_environment (int timeout_seconds_)
{
#if defined _WIN32
#if defined _MSC_VER
...
int
test_inet_pton (int af_, const char *src_, void *dst_)
{
#if defined(ZMQ_HAVE_WINDOWS) && (_WIN32_WINNT < 0x0600)
  if (af_ == AF_INET)
    {
...
fd_t
connect_socket (const char *endpoint_, const int af_, const int protocol_)
{
...
fd_t
bind_socket_resolve_port (const char *address_, const char *port_,
                          char *my_endpoint_, const int af_,
                          const int protocol_)
{
  struct sockaddr_storage addr;
  //  OSX is very opinionated and wants the size to match the AF family type
  ...
  if (af_ == AF_INET || af_ == AF_INET6)
    {
      ...
      snprintf (my_endpoint_, 6 + strlen (address_) + 7 * sizeof (char),
                "%s://%s:%u",
...
bool
streq (const char *lhs_, const char *rhs_)
{
  return strcmp (lhs_, rhs_) == 0;
}

bool
strneq (const char *lhs_, const char *rhs_)
{
  return strcmp (lhs_, rhs_) != 0;
}
...
#if defined _WIN32
int
fuzzer_corpus_encode (const char *dirname, uint8_t ***data, size_t **len,
                      size_t *num_cases)
{
  (void)dirname;
  (void)data;
