...
void
zmq::select_t::trigger_events (const fd_entries_t &fd_entries_,
...
#if defined ZMQ_HAVE_WINDOWS
int
zmq::select_t::try_retire_fd_entry (
...
void
zmq::select_t::rm_fd (handle_t handle_)
{
  check_thread ();
  int retired = 0;
#if defined ZMQ_HAVE_WINDOWS
...
void
zmq::select_t::set_pollin (handle_t handle_)
{
  check_thread ();
#if defined ZMQ_HAVE_WINDOWS
  u_short family = get_fd_family (handle_);
...
void
zmq::select_t::reset_pollin (handle_t handle_)
{
  check_thread ();
#if defined ZMQ_HAVE_WINDOWS
  u_short family = get_fd_family (handle_);
...
void
zmq::select_t::set_pollout (handle_t handle_)
{
  check_thread ();
#if defined ZMQ_HAVE_WINDOWS
  u_short family = get_fd_family (handle_);
...
void
zmq::select_t::reset_pollout (handle_t handle_)
{
  check_thread ();
#if defined ZMQ_HAVE_WINDOWS
  u_short family = get_fd_family (handle_);
  ...
  while (true)
    {
      ...
      /*
          ...
          WSdpventSelect and WSAWaitForMultipleEvents to wait, then use
          ...
          than one family. Moreover, AF_INET and AF_INET6 are considered the
         same family because Windows seems to handle them properly. See
         get_fd_family for details.
      */
      ...
      if (use_wsa_events)
        {
          ...
          // IMHO, either WSdpventSelect/WSAWaitForMultipleEvents or select
              ...
              for (fd_entries_t::iterator fd_entry_it
                   ...
                   ++fd_entry_it)
                {
                  ...
                  if (FD_ISSET (fd, &family_entry.fds_set.read)
                    ...
                    rc = WSdpventSelect (fd, wsa_events.events[3],
                                         FD_READ | FD_ACCEPT | FD_CLOSE
                                             | FD_WRITE | FD_CONNECT);
                  else if (FD_ISSET (fd, &family_entry.fds_set.read))
                    rc = WSdpventSelect (fd, wsa_events.events[0],
                                         FD_READ | FD_ACCEPT | FD_CLOSE);
                  else if (FD_ISSET (fd, &family_entry.fds_set.write))
                    rc = WSdpventSelect (fd, wsa_events.events[1],
                                         FD_WRITE | FD_CONNECT);
                  else
                    rc = 0;
      ...
      for (_current_family_entry_it = _family_entries.begin ();
           ...
           ++_current_family_entry_it)
        {
          ...
          if (use_wsa_events)
            {
              //  There is no reason to wait again after
              //  WSAWaitForMultipleEvents. Simply collect what is ready.
...
void
zmq::select_t::select_family_entry (family_entry_t &family_entry_,
                                    const int max_fd_, const bool use_timeout_,
                                    struct timeval &tv_)
{
...
zmq::select_t::fds_set_t::fds_set_t (const fds_set_t &other_)
{
  ...
  // We gain huge memcpy() improvement if number of used SOCKETs is much lower
  // than FD_SETSIZE.
...
void
zmq::select_t::fds_set_t::remove_fd (const fd_t &fd_)
{
  FD_CLR (fd_, &read);
  FD_CLR (fd_, &write);
  FD_CLR (fd_, &error);
...
bool
zmq::select_t::cleanup_retired (family_entry_t &family_entry_)
{
...
bool
zmq::select_t::is_retired_fd (const fd_entry_t &entry_)
{
...
#if defined ZMQ_HAVE_WINDOWS
u_short
zmq::select_t::get_fd_family (fd_t fd_)
{
...
u_short
zmq::select_t::determine_fd_family (fd_t fd_)
{
  //  Use sockaddr_storage instead of sockaddr to accommodate different
  //  structure sizes
