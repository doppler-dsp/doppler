...
void
recvall (int sock_fd, char *buffer, int len)
{
  int res;
  int total = 0;
  while (len - total > 0)
...
int
recvonce (int sock_fd, char *buffer, int len)
{
  int res;
...
void
sendall (int sock_fd, char *buffer, int len)
{
  int res;
  int total = 0;
  while (len - total > 0)
...
int
remote_connect (int socket, uint32_t addr, uint16_t port)
{
  int res;
...
void *
setup_socks_server (char *socks_server_address, int socks_server_address_len)
{
...
void
socks_server_task (void *socks_server, const char *username,
                   const char *password, int max_client_connect)
{
      ...
      do
        {
          ...
          fprintf (stderr, "socks_server: answered greetings (method: 0x%x)\n",
                   method);
          ...
          if (request_atype == 0x1) /* ATYPE IPv4 */
            {
              ...
              fprintf (stderr, "socks_server: received address (addr: 0x%x)\n",
                       ntohl (naddr));
            }
            ...
            {
              /* For the test we simulate IPV6 connection request ::1, but
               * connect to IPv4 localhost */
              unsigned char nipv6local[16] = { 0 };
              nipv6local[15] = 1;
              recvall (client, buffer, 16);
              fprintf (stderr, "socks_server: received ipv6 address (buffer:");
              for (int i = 0; i < 16; i++)
          ...
          if (err == 0)
            {
              ...
              if (res != 0)
              ...
              else
                {
                  ...
                  res = getsockname (remote, (struct sockaddr *)&ip4addr,
                                     &len);
                  TEST_ASSERT_SUCCESS_RAW_ERRNO (res);
                  fprintf (stderr,
          ...
          if (request_atype == 0x1) /* ATYPE IPv4 */
            ...
            {
              /* We simulate a bind to ::1, though the actual connection is
               * IPv4 */
...
void
socks_server_no_auth (void *socks_server)
{
  socks_server_task (socks_server, NULL, NULL, 1);
}

void
socks_server_no_auth_delay (void *socks_server)
{
...
void
socks_server_basic_auth (void *socks_server)
{
  socks_server_task (socks_server, "someuser", "somepass", 1);
}

void
socks_server_basic_auth_delay (void *socks_server)
{
...
void
socks_server_basic_auth_no_pass (void *socks_server)
{
  socks_server_task (socks_server, "someuser", NULL, 1);
}

void *
setup_push_server (char *connect_address, int connect_address_size)
{
  int res;
...
void *
setup_pull_client (const char *connect_address, const char *socks_proxy)
{
  int res;
...
void *
setup_pull_client_with_auth (const char *connect_address,
                             const char *socks_proxy, const char *username,
                             const char *password)
{
  int res;
  void *pull = test_context_socket (ZMQ_PULL);
...
void
communicate (void *push, void *pull)
{
...
void
test_socks_bind_before_connect (void)
{
  char socks_server_address[MAX_SOCKET_STRING];
  char connect_address[MAX_SOCKET_STRING];
...
void
test_socks_basic_auth_delay (void)
{
#ifdef ZMQ_BUILD_DRAFT_API
  char socks_server_address[MAX_SOCKET_STRING];
  char connect_address[MAX_SOCKET_STRING];
...
void
test_socks_basic_auth_empty_user (void)
{
#ifdef ZMQ_BUILD_DRAFT_API
  char socks_server_address[MAX_SOCKET_STRING];
  char connect_address[MAX_SOCKET_STRING];
...
void
test_socks_basic_auth_null_user (void)
{
#ifdef ZMQ_BUILD_DRAFT_API
  char socks_server_address[MAX_SOCKET_STRING];
  char connect_address[MAX_SOCKET_STRING];
...
void
test_socks_basic_auth_empty_pass (void)
{
#ifdef ZMQ_BUILD_DRAFT_API
  char socks_server_address[MAX_SOCKET_STRING];
  char connect_address[MAX_SOCKET_STRING];
...
void
test_socks_basic_auth_null_pass (void)
{
#ifdef ZMQ_BUILD_DRAFT_API
  char socks_server_address[MAX_SOCKET_STRING];
  char connect_address[MAX_SOCKET_STRING];
...
void
test_string_opt_ok (const char *msg, int opt, const char *value)
{
  int res;
...
void
test_opt_ok (const char *msg, int opt, const char *value, size_t len,
             const char *expected_value, size_t expected_len)
{
  int res;
  void *sub = test_context_socket (ZMQ_SUB);
...
void
test_opt_invalid (const char *msg, int opt, const char *value, int len)
{
  int res;
  void *sub = test_context_socket (ZMQ_SUB);
...
void
test_socks_userpass_options (void)
{
#ifdef ZMQ_BUILD_DRAFT_API
  char buffer[1024];
