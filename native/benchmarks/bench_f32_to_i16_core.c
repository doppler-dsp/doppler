...
main(void)
{
    ...
    int16_t *out = malloc(BENCH_N * sizeof(int16_t));
    ...
    volatile int16_t _sink;

    /* warmup */
    for (int i = 0; i < 16; i++) _sink = f32_to_i16_step(obj, in[i]);

    struct timespec t0, t1;
    jm_bench_t _bench = {0};

    printf("=== f32_to_i16 benchmark ===\n");
    ...
    for (int r = 0; r < ITERATIONS; r++) {
        ...
        for (int i = 0; i < BENCH_N; i++)
            _sink = f32_to_i16_step(obj, in[i]);
    ...
    for (int r = 0; r < ITERATIONS; r++) {
        clock_gettime(CLOCK_MONOTONIC, &t0);
        f32_to_i16_steps(obj, in, out, BENCH_N);
        clock_gettime(CLOCK_MONOTONIC, &t1);
    ...
    jm_bench_write_json(&_bench, "f32_to_i16");
