...
zmq::norm_engine_t::norm_engine_t (io_thread_t *parent_,
    ...
    : io_object_t (parent_), zmq_session (NULL), options (options_),
      ...
      norm_session (NORM_SESSION_INVALID), is_sender (false),
      is_receiver (false), zmq_encoder (0),
      norm_tx_stream (NORM_OBJECT_INVALID), tx_first_msg (true),
      tx_more_bit (false), zmq_output_ready (false), norm_tx_ready (false),
      tx_index (0), tx_len (0), zmq_input_ready (false)
{
  int rc = tx_msg.init ();
  errno_assert (0 == rc);
}
...
int
zmq::norm_engine_t::init (const char *network_, bool send, bool recv)
{
  ...
  //       (for now we use automatic, IP addr based assignment or passed in
  //       'id') a) Use ZMQ Identity somehow? b) Add function to use iface addr
  ...
  if (!NormIsUnicastAddress (addr))
    {
      ...
      if (NULL != ifacePtr)
        {
          // Note a bad interface may not be caught until sender or receiver
          // start (Since sender/receiver is not yet started, this always
          // succeeds here)
  ...
  if (recv)
    {
      ...
      // NORM_SYNC_STREAM tries to get everything the sender has
      // cached/buffered
      NormSetDefaultSyncPolicy (norm_session, NORM_SYNC_STREAM);
      if (!NormStartReceiver (norm_session,
  ...
  if (send)
    {
      // Handle invalid settings -- num_parity must be >= num_autoparity (which
      // has a default of 0)
      ...
      // Handle invalid settings -- block size must be > effective num_parity
      // (which is <255)
      ...
      if (!NormStartSender (norm_session, instanceId,
                            (unsigned long)options.norm_buffer_size * 1024,
                            options.norm_segment_size, blocksize, numparity))
        {
          // errno set by whatever failed
          int savedErrno = errno;
          NormDestroyInstance (norm_instance); // session gets closed, too
...
void
zmq::norm_engine_t::plug (io_thread_t *io_thread_, session_base_t *session_)
{
#ifdef ZMQ_USE_NORM_SOCKET_WRAPPER
...
#ifdef ZMQ_USE_NORM_SOCKET_WRAPPER
  wrapper_thread_handle = CreateThread (NULL, 0, normWrapperThread, threadArgs,
                                        0, &wrapper_thread_id);
#endif
...
void
zmq::norm_engine_t::unplug ()
{
  rm_fd (norm_descriptor_handle);
#ifdef ZMQ_USE_NORM_SOCKET_WRAPPER
  PostThreadMessage (wrapper_thread_id, WM_QUIT, (WPARAM)NULL, (LPARAM)NULL);
...
void
zmq::norm_engine_t::restart_output ()
{
          ...
          if (0 == tx_len)
            {
              if (tx_first_msg)
              ...
              else
                {
                  ...
                  // gets delivered quickly) NormStreamMarkEom(norm_tx_stream);
                  // // the flush below marks eom Note NORM_FLUSH_ACTIVE makes
                  // NORM fairly chatty for low duty cycle messaging but makes
                  // sure content is delivered quickly.  Positive
                  // acknowledgements with flush override would make NORM more
                  // succinct here
              ...
              // Should we write message size header for NORM to use? Or expect
              // NORM receiver to decode ZMQ message framing format(s)? OK - we
              // need to use a byte to denote when the ZMQ frame is the _first_
              //      frame of a message so it can be decoded properly when a
              //      receiver 'syncs' mid-stream.  We key off the the state of
              //      the 'more_flag' I.e.,If  more_flag _was_ false
              //      previously, this is the first frame of a ZMQ message.
              if (tx_more_bit)
                tx_buffer[0]
        ...
        if (NULL != rxState)
          {
            ...
            // This is now unnecessary since deletion takes care of list
            // removal but in the interest of being clear ...
...
bool
zmq::norm_engine_t::restart_input ()
{
...
void
zmq::norm_engine_t::recv_data (NormObjectHandle object)
{
  ...
  // inbound streams and pushed any accumulated messages we can up to the zmq
  // session.
      ...
      while (NULL != (rxState = iterator.GetNextItem ()))
        {
          ...
          while (!rxState->InSync ())
            {
              ...
              if (0 == numBytes)
                {
                  // This probably shouldn't happen either since we found msg
                  // start Need to wait for more data
                  break;
                }
              if (0 == syncFlag)
          ...
          // Now we're actually ready to read data from the NORM stream to the
          // zmq_decoder the underlying zmq_decoder->get_buffer() call sets how
          // much is needed.
      ...
      if (zmq_input_ready)
        {
          ...
          // list Now make a pass through the "msg_pending" list (if the zmq
          // session ready for more input).  This may possibly return streams
          // back to the "rx ready" stream list after their pending message is
          // handled
          ...
          while (NULL != (rxState = iterator.GetNextItem ()))
            {
              ...
              if (rxState->IsRxReady ()) // Move back to "rx_ready" list to
                                         // read more data
                rx_ready_list.Append (*rxState);
              else // Move back to "rx_pending" list until
                   // NORM_RX_OBJECT_UPDATED
                msg_ready_list.Append (*rxState);
            } // end while(NULL != (rxState = iterator.GetNextItem()))
        } // end if (zmq_input_ready)
    } // end while ((!rx_ready_list.empty() || (zmq_input_ready &&
      // !msg_ready_list.empty()))
...
zmq::norm_engine_t::NormRxStreamState::NormRxStreamState (
    ...
    : norm_stream (normStream), max_msg_size (maxMsgSize),
      zero_copy (zeroCopy), in_batch_size (inBatchSize), in_sync (false),
      rx_ready (false), zmq_decoder (NULL), skip_norm_sync (false),
      buffer_ptr (NULL), buffer_size (0), buffer_count (0), prev (NULL),
      next (NULL), list (NULL)
{
}
...
bool
zmq::norm_engine_t::NormRxStreamState::Init ()
{
  in_sync = false;
  skip_norm_sync = false;
  if (NULL != zmq_decoder)
    delete zmq_decoder;
  zmq_decoder = new (std::nothrow)
...
int
zmq::norm_engine_t::NormRxStreamState::Decode ()
{
...
void
zmq::norm_engine_t::NormRxStreamState::List::Destroy ()
{
...
void
zmq::norm_engine_t::NormRxStreamState::List::Append (NormRxStreamState &item)
{
  item.prev = tail;
  if (NULL != tail)
    tail->next = &item;
...
void
zmq::norm_engine_t::NormRxStreamState::List::Remove (NormRxStreamState &item)
{
  if (NULL != item.prev)
...
const zmq::endpoint_uri_pair_t &
zmq::norm_engine_t::get_endpoint () const
{
...
#include <iostream>
DWORD WINAPI
normWrapperThread (LPVOID lpParam)
{
