 ...
 * commas. Fifteen positional numbers cannot be read, and they were not being
 * read: the complex example below carried SIXTEEN arguments to a
 * fifteen-parameter function -- one left behind by a retired parameter -- and
 * said so to nobody, because nothing in the tree compiles a header's example
 * block.
 *
 * @code
...
extern "C"
{
   ...
   * `MpskReceiverR` came to be the one public class with no runnable
   * example: it had looked covered only because a view used to inherit its
   * parent's docstring, which just-makeit 0.70.1 correctly stopped doing.
   *
   * (Written without naming the doxygen command itself: the command is
