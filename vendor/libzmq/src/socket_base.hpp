...
public:
  ...
  static socket_base_t *create (int type_, zmq::ctx_t *parent_, uint32_t tid_,
                                int sid_);
...
private:
  ...
  void event (const endpoint_uri_pair_t &endpoint_uri_pair_,
              uint64_t values_[], uint64_t values_count_, uint64_t type_);

  // Socket event data dispatch
  void monitor_event (uint64_t event_, const uint64_t values_[],
...
private:
  ...
  // Next assigned name on a zmq_connect() call used by ROUTER and STREAM
  // socket types
