...
void
node_t::set_refcount (uint32_t value_)
{
...
void
node_t::set_prefix_length (uint32_t value_)
{
...
void
node_t::set_edgecount (uint32_t value_)
{
...
void
node_t::set_prefix (const unsigned char *bytes_)
{
...
void
node_t::set_first_bytes (const unsigned char *bytes_)
{
  memcpy (first_bytes (), bytes_, edgecount ());
}

unsigned char
node_t::first_byte_at (size_t index_)
{
...
void
node_t::set_first_byte_at (size_t index_, unsigned char byte_)
{
  zmq_assert (index_ < edgecount ());
...
void
node_t::set_node_pointers (const unsigned char *pointers_)
{
...
void
node_t::set_node_at (size_t index_, node_t node_)
{
  zmq_assert (index_ < edgecount ());
...
void
node_t::set_edge_at (size_t index_, unsigned char first_byte_, node_t node_)
{
...
bool
node_t::operator== (node_t other_) const
{
...
void
node_t::resize (size_t prefix_length_, size_t edgecount_)
{
...
node_t
make_node (size_t refcount_, size_t prefix_length_, size_t edgecount_)
{
  const size_t node_size = 3 * sizeof (uint32_t) + prefix_length_
                           + edgecount_ * (1 + sizeof (void *));
...
match_result_t::match_result_t (size_t key_bytes_matched_,
                                ...
                                size_t edge_index_, size_t parent_edge_index_,
                                node_t current_, node_t parent_,
                                node_t grandparent_)
    : _key_bytes_matched (key_bytes_matched_),
      _prefix_bytes_matched (prefix_bytes_matched_), _edge_index (edge_index_),
      _parent_edge_index (parent_edge_index_), _current_node (current_),
      _parent_node (parent_), _grandparent_node (grandparent_)
{
}

match_result_t
zmq::radix_tree_t::match (const unsigned char *key_, size_t key_size_,
...
bool
zmq::radix_tree_t::add (const unsigned char *key_, size_t key_size_)
{
...
bool
zmq::radix_tree_t::rm (const unsigned char *key_, size_t key_size_)
{
  const match_result_t match_result = match (key_, key_size_);
  const size_t key_bytes_matched = match_result._key_bytes_matched;
  const size_t prefix_bytes_matched = match_result._prefix_bytes_matched;
...
bool
zmq::radix_tree_t::check (const unsigned char *key_, size_t key_size_)
{
  if (_root.refcount () > 0)
    return true;
...
static void
visit_keys (node_t node_, std::vector<unsigned char> &buffer_,
...
void
zmq::radix_tree_t::apply (void (*func_) (unsigned char *data_, size_t size_,
                                         void *arg_),
                          void *arg_)
{
  if (_root.refcount () > 0)
