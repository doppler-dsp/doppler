...
static PyObject *
AccQ8_get_acc(
    AccQ8Object *self, PyObject *Py_UNUSED(ignored))
{
    if (!self->handle) {
        PyErr_SetString(PyExc_RuntimeError, "destroyed");
        return NULL;
...
static PyObject *
AccQ8_set_acc(
    AccQ8Object *self, PyObject *args)
{
    if (!self->handle) {
        PyErr_SetString(PyExc_RuntimeError, "destroyed");
        return NULL;
...
static PyMethodDef AccQ8_methods[] = {
    ...
    {"get_acc",
     (PyCFunction)AccQ8_get_acc, METH_NOARGS,
     "Get acc."},
    {"set_acc",
     (PyCFunction)AccQ8_set_acc, METH_VARARGS,
     "Set acc."},
    ...
    {"madd", (PyCFunction)AccQ8_madd, METH_VARARGS,
     ...
     "    >>> from doppler import AccQ8\n"
     "    >>> obj = AccQ8(0)\n"
     "    >>> obj.madd(np.zeros(4, dtype=np.int8), np.zeros(4, dtype=np.int8))\n"},
