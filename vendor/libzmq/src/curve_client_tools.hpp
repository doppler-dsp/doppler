...
{
  static int
  produce_hello (void *data_, const uint8_t *server_key_,
                 const uint64_t cn_nonce_, const uint8_t *cn_public_,
  ...
  static int
  process_welcome (const uint8_t *msg_data_, size_t msg_size_,
                   const uint8_t *server_key_, const uint8_t *cn_secret_,
                   uint8_t *cn_server_, uint8_t *cn_cookie_,
                   uint8_t *cn_precom_)
  {
    if (msg_size_ != 168)
      {
    ...
    int rc = crypto_box_open (&welcome_plaintext[0], welcome_box,
                              sizeof welcome_box, welcome_nonce, server_key_,
                              cn_secret_);
    if (rc != 0)
      {
        errno = EPROTO;
  ...
  static int
  produce_initiate (void *data_, size_t size_, const uint64_t cn_nonce_,
                    const uint8_t *server_key_, const uint8_t *public_key_,
                    const uint8_t *secret_key_, const uint8_t *cn_public_,
                    const uint8_t *cn_secret_, const uint8_t *cn_server_,
...
#pragma GCC diagnostic pop
  ...
  static bool
  is_handshake_command_welcome (const uint8_t *msg_data_,
  ...
  static bool
  is_handshake_command_ready (const uint8_t *msg_data_, const size_t msg_size_)
  {
    return is_handshake_command (msg_data_, msg_size_, "\5READY");
  }

  static bool
  is_handshake_command_error (const uint8_t *msg_data_, const size_t msg_size_)
  {
  ...
  int
  produce_hello (void *data_, const uint64_t cn_nonce_) const
  {
    return produce_hello (data_, server_key, cn_nonce_, cn_public, cn_secret);
  }
  ...
  {
    return produce_initiate (data_, size_, cn_nonce_, server_key, public_key,
                             secret_key, cn_public, cn_secret, cn_server,
                             cn_cookie, metadata_plaintext_, metadata_length_);
  }
...
private:
  template <size_t N>
  static bool
  is_handshake_command (const uint8_t *msg_data_, const size_t msg_size_,
