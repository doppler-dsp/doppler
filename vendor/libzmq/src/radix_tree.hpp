...
{
  match_result_t (size_t key_bytes_matched_, size_t prefix_bytes_matched_,
                  size_t edge_index_, size_t parent_edge_index_,
                  node_t current_, node_t parent_, node_t grandparent);
...
public:
  ...
  //  Retrieve size of the radix tree. Note this is a multithread safe
  //  function.
  size_t size () const;

private:
  match_result_t match (const unsigned char *key_, size_t key_size_,
                        bool is_lookup_) const;
