    ...
    if (nitems_ > S)
      {
        ...
        //  TODO since this function is called by a client, we could return
        //  errno == ENOMEM here
        alloc_assert (_buf);
      }
    else
...
timeout_t compute_timeout (bool first_pass_, long timeout_, uint64_t now_,
                           uint64_t end_);
#endif
...
#if defined ZMQ_HAVE_WINDOWS
inline size_t
valid_pollset_bytes (const fd_set &pollset_)
{
...
public:
  ...
  get ()
  {
    return reinterpret_cast<fd_set *> (&_fd_set[0]);
  }
...
public:
  void
  resize (size_t nevents_)
  {
    _fd_set.resize (1 + nevents_);
  }
