"""The EMA report's certified envelope, as tests that actually run.
...
This module is `util`'s copy of the fix, and it is deliberately thin: it
runs the object's own `build(write=False)` and asserts every limit that
run recorded. The evidence and the gate are therefore the *same code*,
so they cannot disagree the way a hand-copied assertion would.
...
validators import their own module's objects; a single tree-wide
collector would import every module's extension to run any object's
limits.

**The EMA's limits are the primitive's, not a consumer's.** doppler is a
C library, so every claim is certified in `native/tests/test_util_core.c`
...
from doppler.util.tests.validation.ema import validate as ema_validate

OBJECTS = {"ema": ema_validate}
