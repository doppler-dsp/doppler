 ...
 *
 * Sliding-window wrapper: one dp_snr_data_aided_db() call per output index,
...
void
dp_snr_data_aided_db_series (const float _Complex *soft, size_t soft_len,
  ...
  for (size_t i = 0; i < soft_len; i++)
    {
      ...
      out[i]
          = dp_snr_data_aided_db (soft + lo, hi - lo, sign_bits + lo, hi - lo);
    }
}
