...
- [ ] **Open, not a bug in the object itself**: real-capture A/B (`carrier_acq_c_vs_python.py`) against the validated Python `estimate_residual_freq_matched` surfaced that `det_threshold_noncoherent`/`det_n_noncoh` (derived for classic complex-correlator Rayleigh/Rician detection, the same model `Acquisition` uses) have NOT been verified to transfer cleanly to gating a *power-spectrum-vs-known-template* correlation — a single look can pass the CFAR test confidently while its own sub-bin estimate is still noisy from the data modulation's own realization, and tuning `design_snr` by hand didn't converge to a clean multi-look average matching Python's own ~2149-block result. Needs the null/signal distributions of this specific test statistic actually derived before treating the CFAR gate as calibrated for this domain.
