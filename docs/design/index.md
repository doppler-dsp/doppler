...
- [AsyncDsssReceiver](async-dsss-receiver.md) — continuous asynchronous DSSS in one page: the target waveform and the spec, the two acquisition front doors and the `DetectionEvent` hand-off, the asynchronous despreader and its look-back working design, the receiver as built, the continuous many-emitters-one-code use case, the searcher that reports every peak on the surface and cancels the strong ones, the lock-detector release, what the receiver must gain, and the work that measures it
- [AsyncDsssReceiver — the measurement record](async-dsss-receiver-measurements.md) — the dated record behind the design page's §12: every step's harness, numbers, wrong guesses and corrections, under the same section numbers
