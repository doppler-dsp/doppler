...
def env_sf(snr: float, x: np.ndarray) -> np.ndarray:
    ...
    return np.array([marcum_q(1, a, float(xi) * math.sqrt(math.pi / 2))
                     for xi in x])
...
pd_pow_th = np.array([det_pd_power(float(s)**2, DWELL, P_THRESH) for s in snr_sweep])
...
for row, (label, ts_h0, ts_h1, sf_fn, thresh,
          ...
          x_lim, x_lab, stat_name) in enumerate([
    (
        "Case 1: Envelope",
        env_h0, env_h1, env_sf, THETA,
        pd_env_mc, pd_env_th,
        axes[0, 0], axes[0, 1],
        8.0,
    ...
    for ts, snr, color, lbl in [
        ...
        ax_sf.semilogy(ts_s, sf_e, color=color, alpha=0.35, linewidth=1.2,
                       label=f"{lbl} — MC")
        ax_sf.semilogy(x_plot, sf_fn(snr, x_plot),
                       color=color, linewidth=2.0, linestyle="--",
                       label=f"{lbl} — theory")

    ax_sf.axvline(thresh, color="0.35", linestyle=":", linewidth=1.5)
    ax_sf.text(thresh + x_lim * 0.02, 3e-4,
               rf"$\tau={thresh:.2f}$", va="bottom", fontsize=9, color="0.3")
    ...
    ax_sf.annotate(
        rf"$P_{{fa}}={pfa_mc:.3f}$",
        xy=(thresh, PFA), xytext=(thresh - x_lim * 0.06, PFA * 6),
    ...
    pd_fine  = np.array([det_pd(float(s), DWELL, ETA) for s in snr_fine]) \
               if row == 0 else \
               np.array([det_pd_power(float(s)**2, DWELL, P_THRESH)
                         for s in snr_fine])

    ax_pd.plot(snr_fine, pd_fine, color=BLUE, linewidth=2.0, label="Theory")
    ax_pd.scatter(snr_sweep, pd_mc, color=RED, s=30, zorder=5,
                  label=f"MC ({N_TRIALS:,} trials)")

    ax_pd.axhline(0.9, color="0.5", linestyle="--", linewidth=0.9)
    ax_pd.text(snr_fine[-1] * 0.98, 0.915, r"$P_d = 0.9$",
    ...
    if snr_req is not None:
        ...
        ax_pd.text(snr_req + 0.03, 0.05,
                   f"snr={snr_req:.2f}", color="0.4", fontsize=8.5)
