...
zmq::wss_engine_t::wss_engine_t (fd_t fd_, const options_t &options_,
                                 const endpoint_uri_pair_t &endpoint_uri_pair_,
                                 ws_address_t &address_, bool client_,
                                 void *tls_server_cred_,
                                 const std::string &hostname_)
    : ws_engine_t (fd_, options_, endpoint_uri_pair_, address_, client_),
      _established (false), _tls_client_cred (NULL)
{
  int rc = 0;

  if (client_)
    {
      // TODO: move to session_base, to allow changing the socket options
      ...
      if (options_.wss_trust_pem.length () > 0)
        {
          ...
          rc = gnutls_certificate_set_x509_trust_mem (_tls_client_cred, &trust,
                                                      GNUTLS_X509_FMT_PEM);
          zmq_assert (rc >= 0);
        }
...
int
zmq::wss_engine_t::read (void *data_, size_t size_)
{
...
int
zmq::wss_engine_t::write (const void *data_, size_t size_)
{
