...
main(void)
{
    ...
    for (int i = 0; i < BENCH_N; i++) in[i] = (int32_t)(i);
    ...
    /* warmup */
    for (int i = 0; i < 16; i++) _sink = i32_to_f32_step(obj, in[i]);
    ...
    for (int r = 0; r < ITERATIONS; r++) {
        ...
        for (int i = 0; i < BENCH_N; i++)
            _sink = i32_to_f32_step(obj, in[i]);
    ...
    for (int r = 0; r < ITERATIONS; r++) {
        clock_gettime(CLOCK_MONOTONIC, &t0);
        i32_to_f32_steps(obj, in, out, BENCH_N);
        clock_gettime(CLOCK_MONOTONIC, &t1);
