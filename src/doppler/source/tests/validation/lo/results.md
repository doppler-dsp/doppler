...
`dp_lo_state_t` is the **NCO's 32-bit phase accumulator plus a 65536-entry float sine LUT**: the top 16 bits of the phase index the table, and a quarter-cycle offset (`LO_LUT_QTR = 16384`) maps `sin` to `cos` without a second table, so one lookup pair is a CF32 phasor. It is the carrier every mixer in the library turns — `ddc` and `ddcr` compose it, and `lo_step`/`lo_step_ctrl` exist so a tracking loop can embed one **by value** and wipe carrier off a sample at a time.

### Where the design lives — this report does not restate it

| source | holds |
|---|---|
| [`native/inc/doppler/lo/lo_core.h`](../../../../../../native/inc/doppler/lo/lo_core.h) | the contract: the LUT geometry, the emit-before-increment convention, the SFDR claim, the inline composition API and the control port |
| [`native/inc/doppler/nco/nco_core.h`](../../../../../../native/inc/doppler/nco/nco_core.h) | the conversion: `nco_norm_freq_to_inc` is the one double→integer boundary, and the LO calls it for both its configured rate and its control port |
...
- **F5 · FIXED** — the ctrl port was float32 while the configured rate was double, so one requested 0.1 landed on two different phase words (delta 7) — inherited from the NCO (its F2) along with the port itself, and the LO is the face a carrier loop actually steers. dp_lo_steps_ctrl now takes `double`, the width the conversion works in and the one lo_step_ctrl always used, so the block and inline faces of the same control port finally agree. Measured: delta is now 0.
