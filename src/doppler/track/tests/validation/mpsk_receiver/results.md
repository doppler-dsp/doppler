...
| C6 | the handover is opt-in and TWO-WAY, and the shared loop filter carries the estimate across it in both directions | C §4 — the flip is pinned; the DROP-BACK is **absent** (F7) |
...
| C15 | the drop threshold sits at 0.8x with 8-up / 32-down verify counts | **absent** from Python (F7) |
