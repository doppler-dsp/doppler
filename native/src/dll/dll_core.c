          ...
          double init_chip, double bn, double zeta, double spacing)
{
  ...
  /* rate_aid is carrier-aiding config (0 = off), set only by
     ...
     nco_norm_freq_to_inc(inv_tsamps*(1+garbage) + ctrl) every epoch. Benign
     when the stack happens to hold 0 (Linux); a garbage/NaN value on
     another host (macOS/arm64) makes the argument
