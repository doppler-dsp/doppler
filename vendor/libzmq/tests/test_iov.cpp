...
static void
do_check (void *sb_, void *sc_, size_t msg_size_)
{
  ...
  for (int i = 0; i < num_messages; i++)
    {
      ...
      // TODO: this assertion only checks if memcpy behaves as expected...
      // remove this or assert something else?
  ...
  TEST_ASSERT_EQUAL_INT ((int)msg_size_,
                         TEST_ASSERT_SUCCESS_ERRNO (zmq_sendiov (
                             sc_, send_iov, send_count, ZMQ_SNDMORE)));
  ...
  // Test success
  TEST_ASSERT_EQUAL_INT (num_messages, TEST_ASSERT_SUCCESS_ERRNO (zmq_recviov (
                                           sb_, recv_iov, &recv_count, 0)));
