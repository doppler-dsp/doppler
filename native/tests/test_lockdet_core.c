...
main (void)
{
  ...
  dp_lockdet_state_t *d = dp_lockdet_create (1.5, 1.2, 3, 2);
  DP_CHECK (d != NULL);
  if (!d)
    return 1;
  ...
  DP_CHECK (dp_lockdet_step (d, 2.0) == 0);
  DP_CHECK (dp_lockdet_step (d, 2.0) == 0);
  DP_CHECK (d->cnt == 2);
  DP_CHECK (dp_lockdet_step (d, 2.0) == 1);
  ...
  DP_CHECK (dp_lockdet_step (d, 1.3) == 1); /* band: no drop progress   */
  DP_CHECK (dp_lockdet_step (d, 1.0) == 1); /* miss 1 of 2              */
  DP_CHECK (d->cnt == 1);
  DP_CHECK (dp_lockdet_step (d, 1.3) == 1); /* hit resets the drop run  */
  DP_CHECK (d->cnt == 0);
  DP_CHECK (dp_lockdet_step (d, 1.0) == 1); /* miss 1 of 2 (fresh run)  */
  DP_CHECK (dp_lockdet_step (d, 1.0) == 0); /* miss 2 of 2 -> dropped   */
  DP_CHECK (d->cnt == 0);
  ...
  DP_CHECK (dp_lockdet_step (d, 2.0) == 0);
  DP_CHECK (dp_lockdet_step (d, 2.0) == 0);
  DP_CHECK (dp_lockdet_step (d, 1.3) == 0); /* band = miss while unlocked */
  DP_CHECK (d->cnt == 0);
  DP_CHECK (dp_lockdet_step (d, 2.0) == 0);
  DP_CHECK (dp_lockdet_step (d, 2.0) == 0);
  DP_CHECK (dp_lockdet_step (d, 2.0) == 1);
  ...
  DP_CHECK (dp_lockdet_step (d, 1.5) == 0);
  DP_CHECK (d->cnt == 0);
  d->locked = 1;
  DP_CHECK (dp_lockdet_step (d, 1.2) == 1);
  DP_CHECK (d->cnt == 0);

  /* configure preserves a live lock but restarts the verify run. */
  DP_CHECK (dp_lockdet_step (d, 1.0) == 1); /* miss 1 of 2 in flight */
  DP_CHECK (d->cnt == 1);
  dp_lockdet_configure (d, 2.0, 1.6, 4, 4);
  ...
  /* steps() block path matches per-step. */
  {
    ...
    int    got[8] = { 0 };
    dp_lockdet_steps (&a, seq, got, n);
    for (size_t i = 0; i < n; i++)
      DP_CHECK (got[i] == dp_lockdet_step (&b, seq[i]));
  }
  ...
  {
    dp_lockdet_state_t *a = dp_lockdet_create (1.5, 1.2, 3, 2);
    DP_CHECK (dp_lockdet_step (a, 2.0) == 0); /* cnt = 1 of 3, mid-declare */
    unsigned char blob[64];
    DP_CHECK (dp_lockdet_state_bytes (a) <= sizeof blob);
    dp_lockdet_get_state (a, blob);
    int ref1 = dp_lockdet_step (a, 2.0);
    int ref2 = dp_lockdet_step (a, 2.0);
    dp_lockdet_destroy (a);

    dp_lockdet_state_t *b = dp_lockdet_create (1.5, 1.2, 3, 2);
    DP_CHECK (dp_lockdet_set_state (b, blob) == DP_OK);
    DP_CHECK (b->cnt == 1 && b->locked == 0);
    blob[0] ^= (unsigned char)0xFF;
    DP_CHECK (dp_lockdet_set_state (b, blob) == DP_ERR_INVALID);
    DP_CHECK (dp_lockdet_step (b, 2.0) == ref1);
    DP_CHECK (dp_lockdet_step (b, 2.0) == ref2);
    DP_CHECK (b->locked == 1); /* the resumed run declared on schedule */
    dp_lockdet_destroy (b);
  }
   ...
   * every look, and held the lock lit forever on a dead metric. */
  {
    ...
    for (int i = 0; i < 10; i++)
      DP_CHECK (dp_lockdet_step (&nand, NAN) == 0);
    DP_CHECK (nand.cnt == 0);

    /* Locked: drops after exactly n_down, like any other miss. */
    DP_CHECK (dp_lockdet_step (&nand, 2.0) == 0);
    DP_CHECK (dp_lockdet_step (&nand, 2.0) == 1); /* declared */
    DP_CHECK (dp_lockdet_step (&nand, NAN) == 1); /* miss 1 of 3 */
    DP_CHECK (dp_lockdet_step (&nand, NAN) == 1); /* miss 2 of 3 */
    DP_CHECK (dp_lockdet_step (&nand, NAN) == 0); /* miss 3 of 3 -> dropped */

    /* Infinities are ordinary looks, not a special case: +inf is a hit,
       -inf a miss. Only NaN is unordered. */
    dp_lockdet_reset (&nand);
    DP_CHECK (dp_lockdet_step (&nand, INFINITY) == 0);
    DP_CHECK (dp_lockdet_step (&nand, INFINITY) == 1);
    DP_CHECK (dp_lockdet_step (&nand, -INFINITY) == 1);
    DP_CHECK (dp_lockdet_step (&nand, -INFINITY) == 1);
    DP_CHECK (dp_lockdet_step (&nand, -INFINITY) == 0);
    ...
    dp_lockdet_reset (&nand);
    (void)dp_lockdet_step (&nand, 2.0);
    DP_CHECK (dp_lockdet_step (&nand, 2.0) == 1);
    DP_CHECK (dp_lockdet_step (&nand, 1.2) == 1); /* exactly at down_thresh */
   ...
   * call only, which cannot see a seam. */
  {
    ...
    dp_lockdet_steps (&one, seq, a, n);
    /* Split at 3 — mid-run on purpose, so a dropped carry shows up. */
    dp_lockdet_steps (&split, seq, b, 3);
    dp_lockdet_steps (&split, seq + 3, b + 3, n - 3);
    for (size_t i = 0; i < n; i++)
      DP_CHECK (a[i] == b[i]);
    ...
    dp_lockdet_steps (&split, NULL, NULL, 0);
  ...
  {
    dp_lockdet_state_t *c0 = dp_lockdet_create (1.5, 1.2, 0, 0);
    DP_CHECK (c0 != NULL);
    DP_CHECK (c0->n_up == 1 && c0->n_down == 1);
    dp_lockdet_destroy (c0);
  }
  ...
  {
    dp_lockdet_state_t *src = dp_lockdet_create (1.5, 1.2, 3, 2);
    (void)dp_lockdet_step (src, 2.0);
    unsigned char blob[64];
    dp_lockdet_get_state (src, blob);

    dp_lockdet_state_t *dst = dp_lockdet_create (99.0, 98.0, 7, 9);
    DP_CHECK (dp_lockdet_set_state (dst, blob) == DP_OK);
