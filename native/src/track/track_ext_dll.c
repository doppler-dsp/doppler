...
static PyMethodDef DllObj_methods[] = {
  ...
  { "steps", (PyCFunction)(void *)DllObj_steps, METH_VARARGS | METH_KEYWORDS,
    ...
    ">>> chip = np.where(code & 1, -1.0, 1.0)             # BPSK spreading "
    "code\n"
    ">>> x = np.tile(np.repeat(chip, 2), 40).astype(np.complex64)  # 40 clean "
    "periods\n"
    ">>> d = Dll(code=code, sps=2)\n"
    ">>> sym = d.steps(x)                                 # one prompt per "
    "code period\n"
    ">>> sym.dtype\n"
    "dtype('complex64')\n"
    ">>> round(float(np.mean(sym.real[-10:])), 1)         # despread to a "
    "clean +1\n"
    "1.0\n"
    ">>> round(d.code_rate, 3)                            # locked at the "
    "nominal rate\n"
    "1.0\n" },
  ...
  { "configure", (PyCFunction)(void *)DllObj_configure,
    ...
    ">>> d.configure(bn=0.02, zeta=0.707)   # widen the loop bandwidth "
    "mid-run\n"
    ">>> round(d.bn, 3)\n"
    "0.02\n" },
  { "set_rate_aid", (PyCFunction)(void *)DllObj_set_rate_aid,
    ...
    ">>> idx = (np.arange(63 * 4 * 300) * (1 + delta) / 4).astype(np.int64) % "
    "63\n"
  ...
  { "configure_lock_raw", (PyCFunction)(void *)DllObj_configure_lock_raw,
    ...
    ">>> from doppler.track import Dll\n"
    ">>> rng = np.random.default_rng(1)\n"
    ">>> code = rng.integers(0, 2, 63).astype(np.uint8)   # >= 7 chips for a "
    "lock\n"
    ">>> chip = np.where(code & 1, -1.0, 1.0)\n"
    ">>> x = np.tile(np.repeat(chip, 4), 400).astype(np.complex64)  # clean "
    "signal\n"
    ">>> d = Dll(code, sps=4, bn=0.005)\n"
    ">>> # raw geometry: declare at R>3, drop at R<2.5, 8-look, 2-of-2 "
    "hysteresis\n"
    ">>> d.configure_lock_raw(3.0, 2.5, 8, 1.0 / 1024, 2, 2)\n"
    ">>> _ = d.steps(x)\n"
    ">>> d.locked                       # statistic cleared the declare "
    "threshold\n"
    "True\n"
  ...
  { "reset", (PyCFunction)DllObj_reset, METH_NOARGS,
    ...
    ">>> code = rng.integers(0, 2, 63).astype(np.uint8)\n"
    ">>> idx = (np.arange(63 * 4 * 300) * (1 + 3e-4) / 4).astype(np.int64) % "
    "63\n"
    ">>> x = np.where(code[idx] & 1, -1.0, 1.0).astype(np.complex64)\n"
    ">>> d = Dll(code, sps=4, bn=0.005)\n"
    ">>> _ = d.steps(x)\n"
    ">>> first = round(d.code_rate, 6)\n"
    ">>> d.reset()                       # back to the create-time code "
    "phase\n"
