...
zmq::socks_connecter_t::socks_connecter_t (class io_thread_t *io_thread_,
    ...
    : stream_connecter_base_t (io_thread_, session_, options_, addr_,
                               delayed_start_),
      _proxy_addr (proxy_addr_), _auth_method (socks_no_auth_required),
      _status (unplugged)
{
...
void
zmq::socks_connecter_t::set_auth_method_basic (const std::string &username_,
...
zmq::socks_connecter_t::check_proxy_connection () const
{
  ...
  if (err != 0)
    {
      wsa_assert (err == WSdpCONNREFUSED || err == WSdpTIMEDOUT
                  || err == WSdpCONNABORTED || err == WSdpHOSTUNREACH
                  || err == WSdpNETUNREACH || err == WSdpNETDOWN
                  || err == WSdpACCES || err == WSdpINVAL
                  || err == WSdpADDRINUSE);
      return -1;
    }
#else
...
int
zmq::socks_connecter_t::parse_address (const std::string &address_,
