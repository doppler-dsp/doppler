...
void
zmq::mechanism_t::set_peer_routing_id (const void *id_ptr_, size_t id_size_)
{
...
void
zmq::mechanism_t::set_user_id (const void *user_id_, size_t size_)
{
...
const char *
zmq::mechanism_t::socket_type_string (int socket_type_)
{
  // TODO the order must of the names must correspond to the values resp. order
  // of ZMQ_* socket type definitions in zmq.h!
  static const char *names[]
      = { socket_type_pair,   socket_type_pub,    socket_type_sub,
          ...
          socket_type_dgram,  socket_type_peer,   socket_type_channel
#endif
        };
...
static size_t
property_len (size_t name_len_, size_t value_len_)
{
  return name_len_size + name_len_ + value_len_size + value_len_;
}

static size_t
name_len (const char *name_)
{
...
size_t
zmq::mechanism_t::add_property (unsigned char *ptr_, size_t ptr_capacity_,
...
size_t
zmq::mechanism_t::property_len (const char *name_, size_t value_len_)
{
...
size_t
zmq::mechanism_t::add_basic_properties (unsigned char *ptr_,
...
void
zmq::mechanism_t::make_command_with_basic_properties (msg_t *msg_,
                                                      const char *prefix_,
                                                      size_t prefix_len_) const
{
  ...
  add_basic_properties (
      ptr,
      command_size - (ptr - static_cast<unsigned char *> (msg_->data ())));
}

int
zmq::mechanism_t::parse_metadata (const unsigned char *ptr_, size_t length_,
                                  bool zap_flag_)
{
  ...
  while (bytes_left > 1)
    {
      ...
      (zap_flag_ ? _zap_properties : _zmtp_properties)
          .ZMQ_MAP_INSERT_OR_EMPLACE (
              name, std::string (reinterpret_cast<const char *> (value),
                                 value_length));
    }
  if (bytes_left > 0)
    {
...
int
zmq::mechanism_t::property (const std::string & /* name_ */,
...
template <size_t N>
static bool
strequals (const char *actual_type_, const size_t actual_len_,
...
bool
zmq::mechanism_t::check_socket_type (const char *type_,
