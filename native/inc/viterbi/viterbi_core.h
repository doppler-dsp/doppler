 ...
 * @endcode
 *
 * @param poly  Input uint32_t array (length passed as poly_len).
 * @param k  k (default: 7).
 ...
 * traceback before it emits, exactly as after create.
 *
 ...
 * blocks and the bits come out continuously. A bit is only emitted once the
 * traceback can reach back `depth` steps, so the first call after a reset
 * returns fewer bits than its input implies — @ref viterbi_decode_max_out
 * states exactly how many for a given input length.
 *
 * @param state    The decoder.
 * @param in       Log-likelihood ratios, one per channel symbol. The sign
 *                 carries the bit (positive for 0) and the magnitude the
 *                 confidence; scale does not matter, only the ratio between
 *                 symbols does.
...
viterbi_state_t *viterbi_create_code (const conv_code_t *c, size_t depth);
   ...
   *               after `depth - 1` further bits have been seen, which is
   *               the decoder's latency and the dominant term in its memory.
   ...
   *
   * Streaming: the first `depth - 1` branches of a stream produce no output
   * — the traceback walks `depth - 1` steps back, so a decision needs that
   * many branches BEHIND it — and thereafter one bit is emitted per @c n
   * symbols consumed. @ref viterbi_decode_max_out is the same statement as
   * arithmetic, and is what a caller should size a buffer with rather than
   * repeating this sentence: they disagreed by one until a test asserted the
