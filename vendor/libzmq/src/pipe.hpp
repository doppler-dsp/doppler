...
int pipepair (zmq::object_t *parents_[2], zmq::pipe_t *pipes_[2],
              const int hwms_[2], const bool conflate_[2]);
...
public:
  ...
  //  Set the boost to high water marks, used by inproc sockets so total hwm
  //  are sum of connect and bind sockets watermarks
...
private:
  ...
  void
  process_pipe_peer_stats (uint64_t queue_count_, own_t *socket_base_,
  ...
  //  pipepair function.
  pipe_t (object_t *parent_, upipe_t *inpipe_, upipe_t *outpipe_, int inhwm_,
          int outhwm_, bool conflate_);
  ...
  // boosts for high and low watermarks, used with inproc sockets so hwm are
  // sum of send and recv hmws on each side of pipe
  int _in_hwm_boost;
  int _out_hwm_boost;
