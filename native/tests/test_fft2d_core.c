...
main (void)
{
  ...
  /* ── lifecycle ──────────────────────────────────────────────────── */
  {
    ...
    dp_fft2d_destroy (NULL);
  }

  /* ── max_out == ny*nx ───────────────────────────────────────────── */
  {
    dp_fft2d_state_t *obj = dp_fft2d_create (NY, NX, -1, 1);
    DP_CHECK (dp_fft2d_execute_cf64_max_out (obj) == N);
    DP_CHECK (dp_fft2d_execute_cf32_max_out (obj) == N);
    DP_CHECK (dp_fft2d_execute_inplace_cf64_max_out (obj) == N);
    DP_CHECK (dp_fft2d_execute_inplace_cf32_max_out (obj) == N);
    dp_fft2d_destroy (obj);
  }

  /* ── CF64 forward/inverse round-trip ────────────────────────────── */
  {
    dp_fft2d_state_t *fwd = dp_fft2d_create (NY, NX, -1, 1);
    dp_fft2d_state_t *inv = dp_fft2d_create (NY, NX, +1, 1);
    ...
    dp_fft2d_execute_cf64 (fwd, in, N, spec, N);
    dp_fft2d_execute_cf64 (inv, spec, N, rec, N);

    /* IDFT without normalisation: rec[k] == N * in[k] */
    for (size_t i = 0; i < N; i++)
      DP_CHECK (ceq64 (rec[i], (double)N * in[i]));

    dp_fft2d_destroy (fwd);
    dp_fft2d_destroy (inv);
  }
  ...
  /* ── CF32 forward/inverse round-trip ────────────────────────────── */
  {
    ...
    dp_fft2d_execute_cf32 (fwd, in, N, spec, N);
    dp_fft2d_execute_cf32 (inv, spec, N, rec, N);
  ...
  /* ── DC input: only bin (0,0) is non-zero ───────────────────────── */
  {
    ...
    for (size_t i = 0; i < N; i++)
      in[i] = 1.0 + 0.0 * I;
    dp_fft2d_execute_cf64 (obj, in, N, out, N);
  ...
  /* ── inplace CF64 matches out-of-place ──────────────────────────── */
  {
    ...
    dp_fft2d_execute_cf64 (obj, in, N, out_oop, N);
    dp_fft2d_execute_inplace_cf64 (obj, in, N, out_ip, N);
  ...
  /* ── short out: prefix of the full surface, nothing past max_out ──── */
  {
    ...
    dp_fft2d_execute_cf64 (obj, in, N, full, N);
    for (size_t k = 0; k < N; k++)
      part[k] = CANARY;
    DP_CHECK (dp_fft2d_execute_cf64 (obj, in, N, part, K) == K);
    for (size_t k = 0; k < K; k++)
      DP_CHECK (ceq64 (part[k], full[k]));
    for (size_t k = K; k < N; k++)
      DP_CHECK (ceq64 (part[k], CANARY));

    dp_fft2d_execute_inplace_cf64 (obj, in, N, full, N);
    for (size_t k = 0; k < N; k++)
      part[k] = CANARY;
    DP_CHECK (dp_fft2d_execute_inplace_cf64 (obj, in, N, part, K) == K);
    for (size_t k = 0; k < K; k++)
      DP_CHECK (ceq64 (part[k], full[k]));
    for (size_t k = K; k < N; k++)
      DP_CHECK (ceq64 (part[k], CANARY));

    dp_fft2d_execute_cf32 (obj, in32, N, full32, N);
    for (size_t k = 0; k < N; k++)
      part32[k] = CANARY32;
    DP_CHECK (dp_fft2d_execute_cf32 (obj, in32, N, part32, K) == K);
    for (size_t k = 0; k < K; k++)
      DP_CHECK (ceq32 (part32[k], full32[k]));
    for (size_t k = K; k < N; k++)
      DP_CHECK (ceq32 (part32[k], CANARY32));

    dp_fft2d_execute_inplace_cf32 (obj, in32, N, full32, N);
    for (size_t k = 0; k < N; k++)
      part32[k] = CANARY32;
    DP_CHECK (dp_fft2d_execute_inplace_cf32 (obj, in32, N, part32, K) == K);
    for (size_t k = 0; k < K; k++)
      DP_CHECK (ceq32 (part32[k], full32[k]));
    for (size_t k = K; k < N; k++)
      DP_CHECK (ceq32 (part32[k], CANARY32));
    ...
    for (size_t k = 0; k < N; k++)
      part[k] = CANARY;
    DP_CHECK (dp_fft2d_execute_cf64 (obj, in, N, part, 0) == 0);
    for (size_t k = 0; k < N; k++)
      DP_CHECK (ceq64 (part[k], CANARY));
