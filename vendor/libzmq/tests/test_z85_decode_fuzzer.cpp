...
LLVMFuzzerTestOneInput (const uint8_t *data, size_t size)
{
  ...
  // As per API definition, input must be divisible by 5, so truncate it if
  // it's not
  size -= size % 5;
  // As per API definition, the destination must be at least 0.8 times the
  // input data
...
test_z85_decode_fuzzer ()
{
  ...
  if (fuzzer_corpus_encode (
          "tests/libzmq-fuzz-corpora/test_z85_decode_fuzzer_seed_corpus",
          &data, &len, &num_cases)
      != 0)
    exit (77);
