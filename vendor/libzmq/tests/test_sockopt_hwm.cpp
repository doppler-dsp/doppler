...
int
send_until_wouldblock (void *socket_)
{
  int send_count = 0;
  while (send_count < MAX_SENDS
...
int
test_fill_up_to_hwm (void *socket_, int sndhwm_)
{
...
test_decrease_when_full ()
{
  ...
  rc = zmq_getsockopt (bind_socket, ZMQ_SNDHWM, &sndhwm_read,
                       &sndhwm_read_size);
  TEST_ASSERT_EQUAL_INT (0, rc);
  ...
  // Read out all data (should get up to previous hwm worth so none were
  // dropped)
  int read_count = 0;
  int read_data = 0;
  while (read_count < MAX_SENDS
         && zmq_recv (connect_socket, &read_data, sizeof (read_data),
                      ZMQ_DONTWAIT)
                == sizeof (read_data))
    {
