...
| 3–4, 7, any C         | **a waveform**: tone, PN, BPSK/QPSK, chirp, a bit pattern, a DSSS burst or a continuous asynchronous DSSS stream with data | `wfm_synth` (`dp_wfm_synth_create` + `set_dsss_cont` / `set_dsss` / `set_bits` / `set_symbols`), the generator wfmgen itself renders with; `wfm_synth_noise_steps` for the noise a gap carries                                                                                                                                                                                                                        |
