...
public:
  ...
  const metadata_t::dict_t &
  get_zmtp_properties () const
  {
    return _zmtp_properties;
  }

  const metadata_t::dict_t &
  get_zap_properties () const
  {
...
protected:
  ...
  void make_command_with_basic_properties (msg_t *msg_, const char *prefix_,
  ...
  int parse_metadata (const unsigned char *ptr_, size_t length_,
  ...
  virtual int property (const std::string &name_, const void *value_,
                        size_t length_);
