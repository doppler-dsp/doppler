...
{
  int _fails = 0;
  CHECK (i8_to_f32_create (0.0f) == NULL);
  CHECK (i8_to_f32_create (-1.0f) == NULL);
  ...
  CHECK (ALMOST_EQ (i8_to_f32_step (obj, INT8_MAX), 127.0f / 128.0f, 1e-6f));
  CHECK (ALMOST_EQ (i8_to_f32_step (obj, INT8_MIN), -1.0f, 1e-6f));
  CHECK (ALMOST_EQ (i8_to_f32_step (obj, 0), 0.0f, 1e-7f));
  ...
  if (_fails)
    {
      fprintf (stderr, "test_i8_to_f32_core FAILED (%d)\n", _fails);
      return 1;
    }
  printf ("test_i8_to_f32_core PASSED\n");
  return 0;
}
