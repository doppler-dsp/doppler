...
// The exact number of the messages that go through before (X)PUB blocks
// depends on ZeroMQ internals and how the OS will schedule the different
// threads. In the meanwhile asking statistics to the Proxy must NOT be
// blocking.
//
...
static void
publisher_thread_main (void *pvoid_)
{
  ...
  // this test has 3 threads doing I/O across 2 queues. Depending on the
  // scheduling, it might happen that 20, 30 or 40 messages go through before
  // the pub blocks. That's because the receiver thread gets kicked once every
  // (hwm_ + 1) / 2 sent messages (search for zeromq sources compute_lwm
  // function). So depending on the scheduling of the second thread, the
  // publisher might get one, two or three more batches in. The ceiling is 40
  // as there's 2 queues.
  //
...
static void
subscriber_thread_main (void *pvoid_)
{
  const proxy_hwm_cfg_t *const cfg
      = static_cast<const proxy_hwm_cfg_t *> (pvoid_);
      ...
      if (rc != -1)
        {
          ...
          // after receiving 1st message, set a finite timeout (default is
          // infinite)
          int timeout_ms = 100;
  ...
  // see publisher thread why we have 3 possible outcomes as number of RX
  // messages
...
static void
proxy_stats_asker_thread_main (void *pvoid_)
{
  const proxy_hwm_cfg_t *const cfg
      = static_cast<const proxy_hwm_cfg_t *> (pvoid_);
  ...
  // IMPORTANT: by setting the tx/rx timeouts, we avoid getting blocked when
  // interrogating a proxy which is
  //            itself blocked in a zmq_msg_send() on its XPUB socket having
  //            ZMQ_XPUB_NODROP=1!
  ...
  optval = 10;
  rc = zmq_setsockopt (control_req, ZMQ_REQ_CORRELATE, &optval,
                       sizeof (optval));
  assert (rc == 0);
...
proxy_thread_main (void *pvoid_)
{
  ...
  void *frontend_xsub
      = zmq_socket (cfg->context,
                    ZMQ_XSUB); // the frontend is the one exposed to internal
                               // threads (INPROC)
  ...
  int optval = 1;
  rc = zmq_setsockopt (backend_xpub, ZMQ_XPUB_NODROP, &optval,
                       sizeof (optval));
  assert (rc == 0);
