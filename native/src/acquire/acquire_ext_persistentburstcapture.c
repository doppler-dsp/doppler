...
{
  static char *kwlist[]
      = { "path", "preamble",   "burst_len",           "reps",
          "fs",   "cn0_dbhz",   "doppler_uncertainty", "pfa",
          "pd",   "noise_mode", "doppler_rate",        NULL };
  ...
  if (!PyArg_ParseTupleAndKeywords (
          args, kwds, "O&O|KKdddddsd", kwlist, PyUnicode_FSConverter, &path,
          &preamble_obj, &burst_len_raw, &reps_raw, &fs, &cn0_dbhz,
          &doppler_uncertainty, &pfa, &pd, &noise_mode_str, &doppler_rate))
    {
      Py_XDECREF (path);
      return -1;
    }
...
static PyObject *
PersistentBurstCaptureObj_release (PersistentBurstCaptureObject *self,
                                   PyObject *args, PyObject *kwds)
{
  if (!self->handle)
    {
...
static PyGetSetDef PersistentBurstCapture_getset[] = {
  ...
  { "pd_burst", (getter)PersistentBurstCapture_getprop_pd_burst, NULL,
    ...
    "against the `pd` that was asked for. It models the ENGINE, and the model "
    "does not price a detection the capture then resolves to the wrong "
    "repetition; measured, that loss fits inside the model's margin, so the "
    "capture delivers at least `pd_burst` at every depth (validation report "
    "§2.8, on a Zadoff-Chu 127 x 8 preamble). NaN with no design "
    "`cn0_dbhz`.\n",
    NULL },
...
static PyMethodDef PersistentBurstCaptureObj_methods[] = {
  ...
  { "release", (PyCFunction)(void *)PersistentBurstCaptureObj_release,
    METH_VARARGS | METH_KEYWORDS,
    "release(i) -> None\n"
    "\n"
