...
static bool
is_thread_safe (const zmq::socket_base_t &socket_)
{
  // do not use getsockopt here, since that would fail during context
  // termination
...
static It
find_if2 (It b_, It e_, const T &value, Pred pred)
{
  for (; b_ != e_; ++b_)
    {
      if (pred (*b_, value))
...
int
zmq::socket_poller_t::signaler_fd (fd_t *fd_) const
{
  if (_signaler)
    {
...
int
zmq::socket_poller_t::add (socket_base_t *socket_, void *user_data_,
                           short events_)
{
...
int
zmq::socket_poller_t::add_fd (fd_t fd_, void *user_data_, short events_)
{
...
int
zmq::socket_poller_t::modify (const socket_base_t *socket_, short events_)
{
...
int
zmq::socket_poller_t::modify_fd (fd_t fd_, short events_)
{
  const items_t::iterator it
...
int
zmq::socket_poller_t::remove (socket_base_t *socket_)
{
  const items_t::iterator it
      = find_if2 (_items.begin (), _items.end (), socket_, &is_socket);
...
int
zmq::socket_poller_t::remove_fd (fd_t fd_)
{
  const items_t::iterator it
      = find_if2 (_items.begin (), _items.end (), fd_, &is_fd);
...
zmq::socket_poller_t::rebuild ()
{
  ...
  for (items_t::iterator it = _items.begin (), end = _items.end (); it != end;
       ++it)
    {
      if (it->events)
        {
          //  If the poll item is a 0MQ socket we are interested in input on
          //  the notification file descriptor retrieved by the ZMQ_FD socket
          //  option.
          if (it->socket)
            {
              if (!is_thread_safe (*it->socket))
          ...
          //  Else, the poll item is a raw file descriptor. Convert the poll
          //  item events to the appropriate fd_sets.
          else
            {
...
#if defined ZMQ_POLL_BASED_ON_POLL
int
zmq::socket_poller_t::check_events (zmq::socket_poller_t::event_t *events_,
                                    int n_events_)
#elif defined ZMQ_POLL_BASED_ON_SELECT
int
zmq::socket_poller_t::check_events (zmq::socket_poller_t::event_t *events_,
                                    int n_events_, fd_set &inset_,
...
int
zmq::socket_poller_t::adjust_timeout (zmq::clock_t &clock_, long timeout_,
                                      uint64_t &now_, uint64_t &end_,
                                      bool &first_pass_)
{
...
int
zmq::socket_poller_t::wait (zmq::socket_poller_t::event_t *events_,
