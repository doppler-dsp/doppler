...
static zmtp_greeting_t greeting = { { 0xFF, 0, 0, 0, 0, 0, 0, 0, 1, 0x7F },
                                    { 3, 1 },
                                    { 'N', 'U', 'L', 'L' },
                                    0,
                                    { 0 } };
...
test_stream_to_dealer ()
{
  ...
  TEST_ASSERT_EQUAL_INT (sizeof (greeting),
                         TEST_ASSERT_SUCCESS_ERRNO (zmq_send (
                             stream, &greeting, sizeof (greeting), 0)));
  ...
  while (bytes_read < 9 + size)
    {
      ...
      //  Get next chunk
      TEST_ASSERT_GREATER_THAN_INT (
          0, TEST_ASSERT_SUCCESS_ERRNO (
                 rc = zmq_recv (stream, msgin + bytes_read,
                                9 + size - bytes_read, 0)));
      bytes_read += rc;
    }
...
test_stream_to_stream ()
{
  ...
  TEST_ASSERT_EQUAL_INT8_ARRAY (buffer, http_response, sizeof (http_response));
