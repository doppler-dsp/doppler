...
static inline int _almost_eq(double a, double b, double tol)
    { return fabs(a - b) <= tol; }
#define ALMOST_EQ(a, b, tol) _almost_eq((double)(a), (double)(b), (double)(tol))
...
static size_t
drive_first_frame(specan_state_t *obj, const float complex *tone, size_t total,
                  size_t chunk, float *out, size_t cap)
{
