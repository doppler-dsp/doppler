...
DelayCf64_init(DelayCf64Object *self, PyObject *args, PyObject *kwds)
{
    ...
    self->_ptr_buf = malloc(
        delay_ptr_max_out(self->handle) * sizeof(double complex));
    if (!self->_ptr_buf) { PyErr_NoMemory(); return -1; }
    self->_push_ptr_buf = malloc(
        delay_push_ptr_max_out(self->handle) * sizeof(double complex));
    if (!self->_push_ptr_buf) { PyErr_NoMemory(); return -1; }
    return 0;
}
