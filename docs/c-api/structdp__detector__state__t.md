...
# Struct dp\_detector\_state\_t



[**ClassList**](annotated.md) **>** [**dp\_detector\_state\_t**](structdp__detector__state__t.md)
...
|  [**dp\_corr\_state\_t**](structdp__corr__state__t.md) \* | [**corr**](#variable-corr)  <br> |
...
Allocate with [**dp\_detector\_create()**](detector__core_8h.md#function-dp_detector_create); never stack-allocate. 
...
```C++
int dp_detector_state_t::_last_corr_valid;
```
...
```C++
dp_corr_state_t* dp_detector_state_t::corr;
```
...
```C++
float* dp_detector_state_t::mag_buf;
```
...
```C++
float dp_detector_state_t::noise_est;
```
...
```C++
size_t dp_detector_state_t::noise_hi;
```
...
```C++
size_t dp_detector_state_t::noise_lo;
```
...
```C++
det_noise_mode_t dp_detector_state_t::noise_mode;
```
...
```C++
float* dp_detector_state_t::noise_scratch;
```
...
```C++
float _Complex* dp_detector_state_t::out_buf;
```
...
```C++
size_t dp_detector_state_t::peak_lag;
```
...
```C++
float dp_detector_state_t::peak_mag;
```
...
```C++
dp_f32_t* dp_detector_state_t::ring;
```
...
```C++
size_t dp_detector_state_t::ring_cap;
```
...
```C++
float dp_detector_state_t::test_stat;
```
...
```C++
float dp_detector_state_t::threshold;
```
