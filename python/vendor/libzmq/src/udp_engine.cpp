...
zmq::udp_engine_t::udp_engine_t (const options_t &options_)
    : _plugged (false), _fd (-1), _session (NULL),
      _handle (static_cast<handle_t> (NULL)), _address (NULL),
...
int
zmq::udp_engine_t::init (address_t *address_, bool send_, bool recv_)
{
  zmq_assert (address_);
...
void
zmq::udp_engine_t::plug (io_thread_t *io_thread_, session_base_t *session_)
{
  zmq_assert (!_plugged);
  _plugged = true;
...
int
zmq::udp_engine_t::set_udp_multicast_loop (fd_t s_, bool is_ipv6_, bool loop_)
{
  int level;
  int optname;
...
int
zmq::udp_engine_t::set_udp_multicast_ttl (fd_t s_, bool is_ipv6_, int hops_)
{
  int level;
...
int
zmq::udp_engine_t::set_udp_multicast_iface (fd_t s_, bool is_ipv6_,
...
int
zmq::udp_engine_t::set_udp_reuse_address (fd_t s_, bool on_)
{
  int on = on_ ? 1 : 0;
...
int
zmq::udp_engine_t::set_udp_reuse_port (fd_t s_, bool on_)
{
#ifndef SO_REUSEPORT
  return 0;
#else
...
int
zmq::udp_engine_t::add_membership (fd_t s_, const udp_address_t *addr_)
{
...
void
zmq::udp_engine_t::error (error_reason_t reason_)
{
  zmq_assert (_session);
...
void
zmq::udp_engine_t::sockaddr_to_msg (zmq::msg_t *msg_, const sockaddr_in *addr_)
{
...
int
zmq::udp_engine_t::resolve_raw_address (const char *name_, size_t length_)
{
  ...
  if (rc == 0)
    {
      ...
      if (rc < 0)
        {
#ifdef ZMQ_HAVE_WINDOWS
          if (WSAGetLastError () != WSdpWOULDBLOCK)
            {
              assert_success_or_recoverable (_fd, rc);
              error (connection_error);
            }
