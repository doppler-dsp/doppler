...
main(void)
{
    ...
    /* warmup */
    for (int i = 0; i < 16; i++) acc_f32_step(obj, in[i]);

    struct timespec t0, t1;
    jm_bench_t _bench = {0};

    printf("=== acc_f32 benchmark ===\n");
    ...
    /* bench: get() */
    {
        ...
        volatile float get_sink;
        for (int i = 0; i < 16; i++) get_sink = acc_f32_get(obj);
        for (int r = 0; r < ITERATIONS; r++) {
            clock_gettime(CLOCK_MONOTONIC, &t0);
            for (int i = 0; i < BENCH_N; i++)
                get_sink = acc_f32_get(obj);
            clock_gettime(CLOCK_MONOTONIC, &t1);
    ...
    /* bench: dump() */
    {
        ...
        volatile float dump_sink;
        for (int i = 0; i < 16; i++) dump_sink = acc_f32_dump(obj);
        for (int r = 0; r < ITERATIONS; r++) {
            clock_gettime(CLOCK_MONOTONIC, &t0);
            for (int i = 0; i < BENCH_N; i++)
                dump_sink = acc_f32_dump(obj);
            clock_gettime(CLOCK_MONOTONIC, &t1);
    ...
    /* bench: madd() */
    {
        ...
        for (int i = 0; i < 16; i++) acc_f32_madd(obj, NULL, 0, NULL, 0);
        for (int r = 0; r < ITERATIONS; r++) {
            clock_gettime(CLOCK_MONOTONIC, &t0);
            for (int i = 0; i < BENCH_N; i++)
                acc_f32_madd(obj, NULL, 0, NULL, 0);
            clock_gettime(CLOCK_MONOTONIC, &t1);
    ...
    /* bench: add2d() */
    {
        ...
        for (int i = 0; i < 16; i++) acc_f32_add2d(obj, NULL, 0);
        for (int r = 0; r < ITERATIONS; r++) {
            clock_gettime(CLOCK_MONOTONIC, &t0);
            for (int i = 0; i < BENCH_N; i++)
                acc_f32_add2d(obj, NULL, 0);
            clock_gettime(CLOCK_MONOTONIC, &t1);
    ...
    /* bench: madd2d() */
    {
        ...
        for (int i = 0; i < 16; i++) acc_f32_madd2d(obj, NULL, 0, NULL, 0);
        for (int r = 0; r < ITERATIONS; r++) {
            clock_gettime(CLOCK_MONOTONIC, &t0);
            for (int i = 0; i < BENCH_N; i++)
                acc_f32_madd2d(obj, NULL, 0, NULL, 0);
            clock_gettime(CLOCK_MONOTONIC, &t1);
