...
test_xsub_verbose_unsubscribe ()
{
  ...
  int xsub_verbose = 1;
  TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (sub, ZMQ_XSUB_VERBOSE_UNSUBSCRIBE,
                                             &xsub_verbose, sizeof (int)));

  // unsubscribe from topic A, does not exist yet
  send_array_expect_success (sub, unsubscribe_a_msg, 0);

  // does not exist, but with ZMQ_XSUB_VERBOSE_UNSUBSCRIBE set it will be
  // forwarded anyway
  recv_array_expect_success (pub, unsubscribe_a_msg, 0);
