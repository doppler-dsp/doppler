...
static const char *const _enum_Frame_wfm_seq_kind[] = {
  "literal", "pn", "gold", "dotted", NULL,
};

static const char *const _enum_Frame_wfm_lfsr[] = {
  "galois",
  "fibonacci",
  NULL,
};
...
FrameObj_add_field (FrameObject *self, PyObject *args, PyObject *kwds)
{
  ...
  if (!PyArg_ParseTupleAndKeywords (
          ...
          &derived_bits_raw))
    return NULL;
  int _arg_kind = _enum_index_Frame (_enum_Frame_wfm_seq_kind, kind);
  if (_arg_kind < 0)
    {
      PyErr_Format (PyExc_ValueError,
  ...
  int      _arg_lfsr = _enum_index_Frame (_enum_Frame_wfm_lfsr, lfsr);
  if (_arg_lfsr < 0)
    {
      PyErr_Format (PyExc_ValueError,
