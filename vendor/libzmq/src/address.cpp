...
zmq::address_t::address_t (const std::string &protocol_,
                           const std::string &address_, ctx_t *parent_)
...
int
zmq::address_t::to_string (std::string &addr_) const
{
...
zmq::zmq_socklen_t
zmq::get_socket_address (fd_t fd_, socket_end_t socket_end_,
                         sockaddr_storage *ss_)
{
  ...
  const int rc
      = socket_end_ == socket_end_local
            ...
            : getpeername (fd_, reinterpret_cast<struct sockaddr *> (ss_),
                           &sl);
