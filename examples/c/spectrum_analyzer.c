...
fftshift (double *mag, size_t N)
{
  ...
  for (size_t i = 0; i < half; i++)
    { double t = mag[i]; mag[i] = mag[i + half]; mag[i + half] = t; }
}
                 ...
                 double center_freq, uint64_t frame)
{
  ...
  for (int c = 1; c < DISP_WIDTH; c++)
    if (col_db[c] > peak_db) { peak_db = col_db[c]; peak_col = c; }
  ...
  double peak_mhz = f_centre + (((double)peak_col / DISP_WIDTH) - 0.5)
                    * (sample_rate / 1e6);
  ...
  for (int row = DISP_HEIGHT - 1; row >= 0; row--)
    {
      ...
      int    label  = (row == DISP_HEIGHT - 1) || (row == DISP_HEIGHT / 2)
                      || (row == 0);
      printf ("|");
      for (int c = 0; c < DISP_WIDTH; c++)
        putchar (col_db[c] >= row_db ? (c == peak_col ? '|' : '#') : ' ');
      printf ("|");
      if (label) printf (" %+5.0f dBFS", row_db);
      putchar ('\n');
    }

  /* Frequency axis */
  printf ("+");
  for (int c = 0; c < DISP_WIDTH; c++) putchar ('-');
  printf ("+\n");
...
main (int argc, char *argv[])
{
  ...
  if (!win_buf || !fft_out || !db_buf)
    {
      fputs ("malloc failed\n", stderr);
      free (win_buf); free (fft_out); free (db_buf);
      dp_sub_destroy (ctx); fft_destroy (fft);
      return 1;
    }
  ...
  while (keep_running)
    {
      ...
      for (size_t i = N; i < fft_size; i++) win_buf[i] = 0.0;
      dp_msg_free (msg);
      ...
      render_spectrum (db_buf, fft_size,
                       hdr.sample_rate, hdr.center_freq, ++frame);
