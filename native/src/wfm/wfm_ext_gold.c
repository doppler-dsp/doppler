...
static PyTypeObject GoldObjType = {
  ...
  = "Allocate and initialise a CCSDS-style Gold code generator. Two\n"
    ...
    "``length``, i.e. bit ``length-1``). Both registers shift left one bit "
    "per\n"
    "chip: the new bit (parity of the tapped stages, read *before* the "
    "shift)\n"
    "enters at stage 1 (bit 0), and the old stage-``length`` bit is "
    "discarded\n"
    "after being XORed into the output. The sequence period is ``2^length - "
    "1``\n"
    "for primitive ``taps_a``/``taps_b``. With the CCSDS default polynomials "
    "the\n"
    "two m-sequences form a genuine \"preferred pair\" — their XOR family has "
    "a\n"
    "strict three-valued periodic autocorrelation/cross-correlation set "
    "``{-1,\n"
    "-65, 63}`` — so varying ``seed_a`` (User dependent per the standard) "
    "walks\n"
    "the 2**length-1 XOR members of the Gold-code family while Register B "
    "stays\n"
    "fixed.\n"
    "\n"
    "Parameters\n"
    ...
    "    Register A feedback-tap mask; bit k set means stage k+1 is XORed "
    "into\n"
    ...
    "    Register A initial value; must be non-zero. Per CCSDS this is "
    "\"User\n"
    "    dependent\" — each of the 2^length-1 nonzero values selects a "
    "different\n"
