...
double
dp_det_pd_cfar (double snr, int dwell, double threshold, double k, double leak,
                double leak_cells)
{
  ...
  if (!(k > t + 1.0))
    return dp_det_pd (snr, dwell, threshold);
  ...
  (void)dp_gauss_hermite (z, DET_CFAR_GH_NODES, p, DET_CFAR_GH_NODES);
  double acc = 0.0;
  for (int j = 0; j < DET_CFAR_GH_NODES; j++)
    acc += p[j] * dp_det_pd (snr, dwell, g * (mu + sd * z[j]));
  return acc;
}
