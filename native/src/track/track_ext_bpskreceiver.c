...
static PyTypeObject BpskReceiverObjType = {
  ...
  = "A BPSK receiver stated in the units a caller actually holds: Hz.\n"
    ...
    "    ADC sample rate, Hz. One of the two facts a caller reading a "
    "capture\n"
    "    actually holds -- `sps` is `sample_rate_hz / symbol_rate_hz`, a "
    "ratio\n"
    ...
    "    above the derived `m_out`; a rate that cannot be strobed is refused "
    "at\n"
    "    construction rather than approximated.\n"
    "carrier_freq_hz : float, default 0.0\n"
    "    Carrier centre, Hz. 0 is complex baseband. Stated in Hz like "
    "everything\n"
    ...
    "    Matched-filter shape. Same default as `MpskReceiver` deliberately: "
    "one\n"
    ...
    "    Carrier loop noise bandwidth, normalised to the SYMBOL rate. A "
    "design\n"
    ...
    "    output bits MEAN and is not derivable from the signal, so it is a "
    "link\n"
