...
zmq::xpub_t::xpub_t (class ctx_t *parent_, uint32_t tid_, int sid_)
    : socket_base_t (parent_, tid_, sid_), _verbose_subs (false),
      _verbose_unsubs (false), _more_send (false), _more_recv (false),
      _process_subscribe (false), _only_first_subscribe (false), _lossy (true),
      _manual (false), _send_last_pipe (false), _pending_pipes (),
      _welcome_msg ()
{
  _last_pipe = NULL;
...
void
zmq::xpub_t::xattach_pipe (pipe_t *pipe_, bool subscribe_to_all_,
...
void
zmq::xpub_t::xread_activated (pipe_t *pipe_)
{
              ...
              if (!subscribe)
                {
                  ...
                  //  TODO reconsider what to do if rm_result ==
                  //  mtrie_t::not_found
                  notify
          ...
          if (_manual || (options.type == ZMQ_XPUB && notify))
            {
              ...
              //  reuse the same buffer and prefix a 0/1 byte to the topic,
              //  with inproc the subscribe/cancel command string is not
              //  present in the message, so this optimization is not possible.
...
void
zmq::xpub_t::xwrite_activated (pipe_t *pipe_)
{
  _dist.activated (pipe_);
}

int
zmq::xpub_t::xsetsockopt (int option_, const void *optval_, size_t optvallen_)
{
...
int
zmq::xpub_t::xgetsockopt (int option_, void *optval_, size_t *optvallen_)
{
...
static void
stub (zmq::mtrie_t::prefix_t data_, size_t size_, void *arg_)
{
  LIBZMQ_UNUSED (data_);
  LIBZMQ_UNUSED (size_);
  LIBZMQ_UNUSED (arg_);
...
void
zmq::xpub_t::xpipe_terminated (pipe_t *pipe_)
{
  if (_manual)
    {
...
void
zmq::xpub_t::mark_as_matching (pipe_t *pipe_, xpub_t *self_)
{
  self_->_dist.match (pipe_);
}

void
zmq::xpub_t::mark_last_pipe_as_matching (pipe_t *pipe_, xpub_t *self_)
{
...
void
zmq::xpub_t::send_unsubscription (zmq::mtrie_t::prefix_t data_, size_t size_,
                                  xpub_t *self_)
{
