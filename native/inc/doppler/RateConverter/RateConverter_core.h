 ...
 *
 * Do not initialise directly; use dp_RateConverter_create().
 */
typedef struct
{
 ...
 *
 * Plans the same cheap cascade as dp_RateConverter_create(), then puts a
 ...
 * timing delay — which is what makes dp_RateConverter_execute_ctrl() a timing
 ...
 *                   here — use dp_RateConverter_create() for a plain conversion.
 ...
 *
 * Sticky, cleared by dp_RateConverter_reset().  Always 0 for a cascade with no
 * CIC stage, which is the honest answer: those plans are scale-free.
 *
 * @param s  Pointer to a valid dp_RateConverter_state_t.
 ...
 * is a defect, and `dp_RateConverter_create()` returns 1.0 here at every rate.
 *
 ...
 * ("<prefix>.gain_db" and "<prefix>.level_db"; see dp_agc_set_telemetry()) and
...
/** @brief Bytes dp_RateConverter_get_state() writes for @p s (envelope + stages). */
...
/** @brief As dp_RateConverter_execute_max_out(), for the block control form. */
 ...
 * steer, so this **falls through to dp_RateConverter_execute()** (ctrl ignored).
 *
 * @param s        Pointer to a valid dp_RateConverter_state_t.
 ...
 *
 * The per-input streaming form of dp_RateConverter_execute_ctrl(), and the only
 ...
 * reproduces dp_RateConverter_execute_ctrl() on the same block bit-for-bit when
...
/**
 * @brief dp_RateConverter_execute_ctrl_push(), also emitting the PRE-TERMINAL
 ...
 * @endcode
 *
 * Use dp_RateConverter_create() directly when processing multiple blocks at
