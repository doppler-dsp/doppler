...
zmq::curve_client_t::curve_client_t (session_base_t *session_,
    ...
    : mechanism_base_t (session_, options_),
      curve_mechanism_base_t (session_, options_, "CurveZMQMESSAGEC",
                              "CurveZMQMESSAGES", downgrade_sub_),
      _state (send_hello),
      _tools (options_.curve_public_key, options_.curve_secret_key,
...
int
zmq::curve_client_t::next_handshake_command (msg_t *msg_)
{
  int rc = 0;
...
int
zmq::curve_client_t::process_handshake_command (msg_t *msg_)
{
...
int
zmq::curve_client_t::encode (msg_t *msg_)
{
...
int
zmq::curve_client_t::decode (msg_t *msg_)
{
  zmq_assert (_state == connected);
...
int
zmq::curve_client_t::produce_hello (msg_t *msg_)
{
...
int
zmq::curve_client_t::process_welcome (const uint8_t *msg_data_,
                                      size_t msg_size_)
{
...
int
zmq::curve_client_t::process_ready (const uint8_t *msg_data_, size_t msg_size_)
{
  if (msg_size_ < 30)
    {
      session->get_socket ()->event_handshake_failed_protocol (
...
int
zmq::curve_client_t::process_error (const uint8_t *msg_data_, size_t msg_size_)
{
