...
void
recv_string_expect_success_or_eagain (void *socket_, const char *str_,
                                      int flags_)
{
...
test_ppoll_signals ()
{
  ...
  if (child_pid > 0)
    { // parent
      ...
      // to make sure we don't hang when the child has already exited at the
      // end, we set a receive timeout of five seconds
      ...
      // bind is on the master process to avoid zombie children to hold on to
      // binds

      // first send a test message to check whether the signal mask is setup in
      // the child process
      ...
      // for good measure, and to make sure everything went as expected, close
      // off with another handshake, which will trigger the second poll call on
      // the other side
      send_string_expect_success (socket, "breaker breaker", 0);
      // in case the 1 second sleep was not enough on the child side, we are
      // also fine with an EAGAIN here
    ...
    { // child
      ...
      // now start ppolling, which should exit with EINTR because of the
      // SIGTERM
