...
_bind_ber_settle_syms (PyObject *self, PyObject *args, PyObject *kwds)
{
  ...
  if (!PyArg_ParseTupleAndKeywords (args, kwds, "dd", _kwlist, &bn_timing,
                                    &bn_carrier))
    return NULL;
...
_bind_ber_lock_symbol (PyObject *self, PyObject *args, PyObject *kwds)
{
  ...
  double             min_frac    = 0.9;
  if (!PyArg_ParseTupleAndKeywords (args, kwds, "O|Kd", _kwlist, &flags_obj,
...
_bind_ber_evm_db (PyObject *self, PyObject *args, PyObject *kwds)
{
  ...
  int                m         = 4;
  if (!PyArg_ParseTupleAndKeywords (args, kwds, "O|KKi", _kwlist, &rx_obj,
                                    &lo_raw, &hi_raw, &m))
    return NULL;
...
static PyMethodDef ber_module_methods[] = {
  { "ber_theory_ser", (PyCFunction)(void *)_bind_ber_theory_ser,
  ...
  { "ber_theory_ber", (PyCFunction)(void *)_bind_ber_theory_ber,
    ...
    "float\n"
    "    Output.\n" },
  { "ber_esn0_db_for_ser", (PyCFunction)(void *)_bind_ber_esn0_db_for_ser,
    METH_VARARGS | METH_KEYWORDS,
  ...
  { "ber_evm_scatter_floor_db",
    ...
    "dB.** \"Scattered reads ~0 dB\" is the BPSK limit only. At 8PSK a "
    "stream\n"
    ...
    "anchor\" and \"completely broken\" collapses as M grows: 5.4 dB at "
    "BPSK,\n"
  ...
  { "ber_settle_syms", (PyCFunction)(void *)_bind_ber_settle_syms,
    ...
    "2*(5/bn_timing + 5/bn_carrier). Three factors, and skipping any produces "
    "a confident wrong number -- 5/Bn per loop is the standard second-order "
    "settling time (in SYMBOLS, since both bn are symbol-rate normalised); "
    "the two budgets ADD because the loops are cascaded (the carrier "
    "discriminator reads the on-time strobe, so it cannot converge until "
    "timing has); and the sum DOUBLES for joint tracking. This is a FLOOR, "
    "not the answer: take the max of it and every lock indicator the receiver "
    "publishes. Pass a loop's bn as 0 if it is not running.\n"
    "\n"
    ...
    "int\n"
    "    Output.\n" },
  { "ber_lock_symbol", (PyCFunction)(void *)_bind_ber_lock_symbol,
    METH_VARARGS | METH_KEYWORDS,
    "First symbol from which a verify-counted lock flag is SUSTAINED, or -1 "
    "for 'never locked'. Sustained means `sustain` consecutive symbols high "
    "AND at least `min_frac` of everything after that point high too: the run "
    ...
    "window at all. The -1 is deliberate: it forces the caller to say 'never "
    "locked' rather than quietly measure a transient.\n"
    "\n"
    ...
    "    which forces the caller to say so rather than measure a "
    "transient.\n" },
  { "ber_evm_db", (PyCFunction)(void *)_bind_ber_evm_db,
    METH_VARARGS | METH_KEYWORDS,
    ...
    "estimated from the data. References neither the transmitted symbols nor "
    "a lag, so it cannot be fooled by an alignment search. A locked "
    "matched-filter output reads EVM_dB ~ -(Es/N0)_dB (an I/Q-plane quantity "
    "-- no factor of two; quoting one flatters the result by 3 dB). Read it "
    "against ber_evm_scatter_floor_db(m), NEVER against 0 dB. The window is "
    "explicit because BER and EVM must be measured on the SAME one: a "
  ...
  { "ber_settle_from", (PyCFunction)(void *)_bind_ber_settle_from,
    ...
    "last of all, contributing its instant PLUS the budget again (measured on "
    "8PSK: handover at symbol 2525 against a 2000-symbol budget, and 5.95x "
    "the coherent bound if the window started at 2000 rather than 4525). No "
    "receiver in this library hands over any more, so the term went with the "
    "handover rather than remaining as an argument that could only be passed "
    "-1. Pass -1 for an indicator the receiver does not publish, which is "
    "what ber_lock_symbol() returns for 'never locked'. A -1 timing or "
...
static PyModuleDef ber_moduledef = {
  ...
  .m_name = "ber",
  .m_doc  = "Bit-error-rate measurement: a BerMeter that aligns a recovered "
            "bit stream to a reference and scores errors.\n"
            "\n"
            "Examples\n"
            "--------\n"
            ">>> import numpy as np\n"
...
PyInit_ber (void)
{
  ...
  BerMeterObj_ber_type
      = BerMeterObj_ser_type; /* BerInterval: one public name, one type */
