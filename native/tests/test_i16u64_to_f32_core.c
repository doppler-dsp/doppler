...
int main(void)
{
...

    {
        ...
        CHECK(obj != NULL);
        CHECK(_feq(i16u64_to_f32_step(obj, 0x0000000000007FFFull),  32767.0f/32768.0f, 1e-6f));
        CHECK(_feq(i16u64_to_f32_step(obj, 0x0000000000008000ull), -1.0f, 1e-6f));
    ...
    /* ── steps() matches per-sample loop ─────────────────────────────── */
    {
        ...
        CHECK(oa && ob);
        uint64_t input[32]; float bulk[32], loop[32];
        for (int i = 0; i < 32; i++) input[i] = (uint64_t)(i * 1024);
        i16u64_to_f32_steps(oa, input, bulk, 32);
        for (int i = 0; i < 32; i++) loop[i] = i16u64_to_f32_step(ob, input[i]);
        CHECK(memcmp(bulk, loop, 32 * sizeof(float)) == 0);
        i16u64_to_f32_destroy(oa); i16u64_to_f32_destroy(ob);
    }
