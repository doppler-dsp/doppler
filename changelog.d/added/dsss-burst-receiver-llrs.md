- **`DsssBurstReceiver.llrs()` and `BurstDemod.llrs()` — the soft bits**
    ...
    deliver. One LLR per **frame** bit now, in `mpsk_soft_demap`'s convention
    (`L < 0` reproduces `push()`'s bits, asserted), scaled by the burst's own
    noise estimate so bursts are comparable — measured 17.9 → 67.2 → 259.2
    mean |L| at 6, 12 and 18 dB Es/N0.
