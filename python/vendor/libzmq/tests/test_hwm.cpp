...
int
count_msg (int send_hwm_, int recv_hwm_, TestType test_type_)
{
  void *bind_socket;
  void *connect_socket;
  if (test_type_ == BIND_FIRST)
    {
      ...
      //  have monitoring events for inproc, so we just hope SETTLE_TIME
      //  suffices
      msleep (SETTLE_TIME);
    }
  else
...
int
test_inproc_bind_first (int send_hwm_, int recv_hwm_)
{
  return count_msg (send_hwm_, recv_hwm_, BIND_FIRST);
}

int
test_inproc_connect_first (int send_hwm_, int recv_hwm_)
{
  return count_msg (send_hwm_, recv_hwm_, CONNECT_FIRST);
}

int
test_inproc_connect_and_close_first (int send_hwm_, int recv_hwm_)
{
  // Set up connect socket
  void *connect_socket = test_context_socket (ZMQ_PUSH);
  ...
  // Set up bind socket
  void *bind_socket = test_context_socket (ZMQ_PULL);
  TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (bind_socket, ZMQ_RCVHWM,
                                             &recv_hwm_, sizeof (recv_hwm_)));
  TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (bind_socket, "inproc://a"));
...
int
test_inproc_bind_and_close_first (int send_hwm_, int /* recv_hwm */)
{
  // Set up bind socket
  void *bind_socket = test_context_socket (ZMQ_PUSH);
  TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (bind_socket, ZMQ_SNDHWM,
                                             &send_hwm_, sizeof (send_hwm_)));
  TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (bind_socket, "inproc://a"));
  ...
  /* TODO Can't currently do connect without then wiring up a bind as things
  hang, this needs top be fixed.
  // Set up connect socket
  void *connect_socket = test_context_socket (ZMQ_PULL);
  TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (connect_socket, ZMQ_RCVHWM,
  &recv_hwm, sizeof (recv_hwm))); TEST_ASSERT_SUCCESS_ERRNO (zmq_connect
  (connect_socket, "inproc://a"));
...
{
  // Send hwm of 1, send from bind side before connect so total that can be
  // queued should be 1, however currently all messages get thrown away before
  // the connect.  BUG?
