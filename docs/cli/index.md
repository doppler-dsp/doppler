...
These are the built-in blocks shipped with `doppler-cli`. Custom blocks
can be added via a [Dopplerfile](dopplerfile.md) — they appear alongside
these in `doppler compose init`.
...
taps = firwin(101, cutoff=0.1).tolist()
```
