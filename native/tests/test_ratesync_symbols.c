/* test_ratesync_symbols.c -- generated by just-makeit (gh-1361).
 *
 * The address of every function the Python binding calls. This
 * file is linked into the C test, so a function that is declared
 * and never defined fails the build here, naming it -- rather than
 * at import, where a shared object's undefined symbol surfaces.
 * Regenerated by `jm apply`; do not edit.
 */
#include "doppler/ratesync/ratesync_core.h"

typedef void (*jm_any_fn)(void);

extern const jm_any_fn jm_bound_symbols_ratesync[];
/* `used` keeps the compiler from dropping the table; `retain` keeps
 * the linker's --gc-sections from dropping it (measured: without it
 * a -Wl,--gc-sections build links a missing definition cleanly). On
 * Mach-O, `used` alone already marks it no-dead-strip. */
#if defined(__has_attribute)
#if __has_attribute(retain)
__attribute__((used, retain))
#elif __has_attribute(used)
__attribute__((used))
#endif
#endif
const jm_any_fn jm_bound_symbols_ratesync[] = {
    (jm_any_fn)dp_ratesync_create,
    (jm_any_fn)dp_ratesync_destroy,
    (jm_any_fn)dp_ratesync_reset,
    (jm_any_fn)dp_ratesync_steps_max_out,
    (jm_any_fn)dp_ratesync_steps,
    (jm_any_fn)dp_ratesync_configure,
    (jm_any_fn)dp_ratesync_get_bn,
    (jm_any_fn)dp_ratesync_set_bn,
    (jm_any_fn)dp_ratesync_get_timing_error,
    (jm_any_fn)dp_ratesync_get_rate,
    (jm_any_fn)dp_ratesync_get_ctrl,
    (jm_any_fn)dp_ratesync_get_lock_stat,
    (jm_any_fn)dp_ratesync_get_locked,
    (jm_any_fn)dp_ratesync_get_clipped,
    (jm_any_fn)dp_ratesync_configure_lock_raw,
    (jm_any_fn)dp_ratesync_set_telemetry,
    (jm_any_fn)dp_ratesync_state_bytes,
    (jm_any_fn)dp_ratesync_get_state,
    (jm_any_fn)dp_ratesync_set_state,
};
