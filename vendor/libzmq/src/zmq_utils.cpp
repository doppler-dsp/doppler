...
unsigned long
zmq_stopwatch_intermediate (void *watch_)
{
...
unsigned long
zmq_stopwatch_stop (void *watch_)
{
...
void *
zmq_threadstart (zmq_thread_fn *func_, void *arg_)
{
  ...
  while (string_[char_nbr])
    {
      ...
      if (summand == 0xFF || summand > (UINT32_MAX - value))
        {
          //  Invalid z85 encoding, invalid character or represented value
          //  exceeds 0xffffffff
          goto error_inval;
        }
      value += summand;
...
int
zmq_curve_keypair (char *z85_public_key_, char *z85_secret_key_)
{
...
int
zmq_curve_public (char *z85_public_key_, const char *z85_secret_key_)
{
#if defined(ZMQ_HAVE_CURVE)
#if crypto_box_PUBLICKEYBYTES != 32 || crypto_box_SECRETKEYBYTES != 32
#error "CURVE encryption library not built correctly"
...
void
zmq_atomic_counter_set (void *counter_, int value_)
{
...
int
zmq_atomic_counter_inc (void *counter_)
{
...
int
zmq_atomic_counter_dec (void *counter_)
{
...
int
zmq_atomic_counter_value (void *counter_)
{
...
void
zmq_atomic_counter_destroy (void **counter_p_)
{
