...
int
zmq::set_tcp_send_buffer (fd_t sockfd_, int bufsize_)
{
  const int rc
...
int
zmq::set_tcp_receive_buffer (fd_t sockfd_, int bufsize_)
{
  const int rc
...
int
zmq::tune_tcp_keepalives (fd_t s_, int keepalive_, int keepalive_cnt_,
...
int
zmq::tcp_write (fd_t s_, const void *data_, size_t size_)
{
#ifdef ZMQ_HAVE_WINDOWS
  ...
  if (nbytes == SOCKET_ERROR && last_error == WSdpWOULDBLOCK)
    return 0;

  //  Signalise peer failure.
  if (nbytes == SOCKET_ERROR
      && (last_error == WSdpNETDOWN || last_error == WSdpNETRESET
          || last_error == WSdpHOSTUNREACH || last_error == WSdpCONNABORTED
          || last_error == WSdpTIMEDOUT || last_error == WSdpCONNRESET))
    return -1;

  //  Circumvent a Windows bug:
  //  See https://support.microsoft.com/en-us/kb/201213
  //  See https://zeromq.jira.com/browse/LIBZMQ-195
  if (nbytes == SOCKET_ERROR && last_error == WSdpNOBUFS)
    return 0;
  ...
  if (nbytes == -1)
    {
      ...
      errno_assert (errno != EACCES && errno != EBADF && errno != EDESTADDRREQ
                    && errno != EFAULT && errno != EISCONN && errno != EMSGSIZE
                    && errno != ENOMEM && errno != ENOTSOCK
                    && errno != EOPNOTSUPP);
#else
...
int
zmq::tcp_read (fd_t s_, void *data_, size_t size_)
{
#ifdef ZMQ_HAVE_WINDOWS
  ...
  if (rc == SOCKET_ERROR)
    {
      ...
      if (last_error == WSdpWOULDBLOCK)
      ...
      else
        {
          wsa_assert (
              last_error == WSdpNETDOWN || last_error == WSdpNETRESET
              || last_error == WSdpCONNABORTED || last_error == WSdpTIMEDOUT
              || last_error == WSdpCONNRESET || last_error == WSdpCONNREFUSED
              || last_error == WSdpNOTCONN || last_error == WSdpNOBUFS);
...
void
zmq::tcp_tune_loopback_fast_path (const fd_t socket_)
{
  ...
  if (SOCKET_ERROR == rc)
    {
      ...
      if (WSdpOPNOTSUPP == last_error)
        {
          // This system is not Windows 8 or Server 2012, and the call is not
          // supported.
        }
      else
        {
...
void
zmq::tune_tcp_busy_poll (fd_t socket_, int busy_poll_)
{
...
zmq::fd_t
zmq::tcp_open_socket (const char *address_, const zmq::options_t &options_,
                      bool local_, bool fallback_to_ipv4_,
                      zmq::tcp_address_t *out_tcp_addr_)
{
  ...
  //  This option removes several delays caused by scheduling, interrupts and
  //  context switching.
