...
private:
  ...
  //  Subscriptions will not bed added automatically, only after calling set
  //  option with ZMQ_SUBSCRIBE or ZMQ_UNSUBSCRIBE
  bool _manual;

  //  Send message to the last pipe, only used if xpub is on manual and after
  //  calling set option with ZMQ_SUBSCRIBE
