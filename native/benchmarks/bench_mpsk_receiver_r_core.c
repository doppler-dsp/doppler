/* bench_mpsk_receiver_r_core.c — no step() to benchmark */
#include "jm_bench.h"
...
main (void)
{
  mpsk_receiver_r_state_t *obj = mpsk_receiver_r_create (
      4, 16.0, 4, 0, 0.35, 8, 0.01, 0.707, 0.01, 0, 0.5, 0.0, 0, 1024,
      MPSK_RX_NDA_TAP_STROBE, 1, MPSK_RX_AGC_BW_RATIO);
  struct timespec t0, t1;
  jm_bench_t      _bench = { 0 };

  printf ("=== mpsk_receiver_r benchmark ===\n");
  printf ("  (no step(); methods below)\n");
  printf ("block = %d samples,  %d iterations\n\n", BENCH_N, ITERATIONS);

  jm_bench_write_json (&_bench, "mpsk_receiver_r");
