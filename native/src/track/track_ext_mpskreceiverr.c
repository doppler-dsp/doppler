...
static PyGetSetDef MpskReceiverR_getset[] = {
  ...
  { "zeta", (getter)MpskReceiverR_getprop_zeta, NULL,
    ...
    NULL },
  { "num_phases", (getter)MpskReceiverR_getprop_num_phases, NULL,
    ...
    "reported.\n",
    NULL },
  { "lock_thresh", (getter)MpskReceiverR_getprop_lock_thresh, NULL,
...
static PyTypeObject MpskReceiverRObjType = {
  ...
  = "Create the M-PSK receiver behind an R2C halfband: a real IF in. The "
    ...
    "    rather than the complex face's 8 purely to clear that bound: "
    "`m_out`\n"
    ...
    "    Gardner detector takes every m_out-th output as the on-time strobe "
    "and\n"
    ...
    "    coarsely. Measured on the complex face at its default sps=8 against "
    "the\n"
    "    coherent bound EVM_dB = -(Es/N0)_dB: at 18 dB Es/N0, m_out=8 lands "
    "0.41\n"
    "    dB off the bound where m_out=4 loses 3.11 dB; at 14 dB it is 0.25 "
    "dB\n"
    "    against 1.71 dB -- the gap widens as noise stops hiding it. Because\n"
    "    `sps` must clear `2 * m_out` here, this default is also what puts "
    "the\n"
    "    `sps` default at 32. **Never pair 2 with pulse=\"iandd\"**: the "
    "matched\n"
    "    filter degenerates to a two-tap sum, the eye barely opens (measured\n"
    "    lock statistic -0.34 at 2 against +0.95 at 4 on the same NRZ "
    "stream)\n"
    "    and acquisition itself fails about half the time. Replaces the old "
    "`n`\n"
    ...
    "    `Re((z/|z|)^M)` smoothed by an EMA, whose noise-only sd is 0.1132 "
    "for\n"
    "    EVERY M, so 0.4999 is 4.42 noise sigmas; to pin your own, divide "
    "your\n"
    ...
    "    The real IF **centre**, cycles/sample at the real input rate -- an "
    "IF\n"
    "    at 0.2*fs is 0.2, and the halved value the LO actually uses is this\n"
    "    object's business, not yours. Unlike the complex face, this one "
    "does\n"
    "    not acquire from a cold zero: a real IF must be tuned near, so this "
    "is\n"
    ...
    "    derives it** as 64, the measured saturation point. The bank is sized "
    "by\n"
    ...
    "    none. Read the applied gain back with `agc_gain_db`. On this face "
    "the\n"
    ...
    "    **0 (the default) derives it** as 0.05. The one AGC's noise "
    "bandwidth\n"
    ...
    "    loop directly. Must be in (0, 1) and construction refuses "
    "otherwise,\n"
    "    because an AGC at or above the bandwidth of a loop it feeds "
    "corrects\n"
    "    the excursions that loop is itself producing and the two integrate\n"
    "    against each other; the signal LEVEL is a slow property of the "
    "channel,\n"
    "    not a disturbance to reject at loop speed. Exposed rather than "
    "fixed\n"
    "    because the right separation depends on how fast the channel's "
    "level\n"
    "    moves against how fast its phase and timing do, which is a property "
    "of\n"
    "    the link. The cold start is not the loop's job either way -- the "
    "AGC\n"
