...
int
zmq::v2_decoder_t::one_byte_size_ready (unsigned char const *read_from_)
{
  return size_ready (_tmpbuf[0], read_from_);
}

int
zmq::v2_decoder_t::eight_byte_size_ready (unsigned char const *read_from_)
{
...
int
zmq::v2_decoder_t::size_ready (uint64_t msg_size_,
                               unsigned char const *read_pos_)
{
  ...
  if (unlikely (!_zero_copy
                       ...
                       allocator.data () + allocator.size () - read_pos_)))
    {
      ...
      // arena this happens every time when a message does not fit completely
      // into the buffer
