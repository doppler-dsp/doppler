/*
 * dsss_ext_burst_capture.c — BurstCapture type for the dsss module.
 *
  ...
  if (self->handle->underpowered)
    {
      if (PyErr_WarnEx (PyExc_UserWarning,
                        ...
                        "pd at this cn0_dbhz and geometry (pd_predicted < "
                        "pd). It still builds a best-effort grid, so the "
                        "symptom is bursts that are never captured rather "
                        "than an error. Lower pd, raise cn0_dbhz, or give "
                        "the preamble more repetitions.",
                        1)
          < 0)
        return -1;
    }
...
static PyObject *
BurstCapture_getprop_refine_margin (BurstCaptureObject *self,
                                    void               *Py_UNUSED (closure))
{
  if (!self->handle)
    {
...
static PyGetSetDef BurstCapture_getset[] = {
  ...
  { "cn0_dbhz_est", (getter)BurstCapture_getprop_cn0_dbhz_est, NULL,
    ...
    "not a fault.\n",
    NULL },
  { "refine_margin", (getter)BurstCapture_getprop_refine_margin, NULL,
...
static PyTypeObject BurstCaptureObjType = {
  PyVarObject_HEAD_INIT (NULL, 0).tp_name = "dsss.BurstCapture",
  ...
  = "Create a burst capture: acquisition, refine and retention behind one\n"
    ...
    "geometry\n"
    "    (pd_predicted < pd). It still builds a best-effort grid, so the "
    "symptom\n"
    "    is bursts that are never captured rather than an error. Lower pd, "
    "raise\n"
