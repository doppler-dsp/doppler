...
zmq::stream_t::stream_t (class ctx_t *parent_, uint32_t tid_, int sid_)
    : routing_socket_base_t (parent_, tid_, sid_), _prefetched (false),
      _routing_id_sent (false), _current_out (NULL), _more_out (false),
...
void
zmq::stream_t::xattach_pipe (pipe_t *pipe_, bool subscribe_to_all_,
...
void
zmq::stream_t::xpipe_terminated (pipe_t *pipe_)
{
...
void
zmq::stream_t::xread_activated (pipe_t *pipe_)
{
  _fq.activated (pipe_);
}

int
zmq::stream_t::xsend (msg_t *msg_)
{
      ...
      if (msg_->flags () & msg_t::more)
        {
          ...
          //  prefix. If there's no such pipe return an error
...
int
zmq::stream_t::xsetsockopt (int option_, const void *optval_,
                            size_t optvallen_)
{
  switch (option_)
    {
...
void
zmq::stream_t::identify_peer (pipe_t *pipe_, bool locally_initiated_)
{
