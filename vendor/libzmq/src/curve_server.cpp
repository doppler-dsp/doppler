...
zmq::curve_server_t::curve_server_t (session_base_t *session_,
    ...
    : mechanism_base_t (session_, options_),
      zap_client_common_handshake_t (session_, peer_address_, options_,
                                     sending_ready),
      curve_mechanism_base_t (session_, options_, "CurveZMQMESSAGES",
                              "CurveZMQMESSAGEC", downgrade_sub_)
{
  int rc;
...
int
zmq::curve_server_t::next_handshake_command (msg_t *msg_)
{
  int rc = 0;
...
int
zmq::curve_server_t::process_handshake_command (msg_t *msg_)
{
  int rc = 0;
...
int
zmq::curve_server_t::encode (msg_t *msg_)
{
...
int
zmq::curve_server_t::decode (msg_t *msg_)
{
  zmq_assert (state == ready);
...
int
zmq::curve_server_t::process_hello (msg_t *msg_)
{
...
int
zmq::curve_server_t::produce_welcome (msg_t *msg_)
{
...
int
zmq::curve_server_t::process_initiate (msg_t *msg_)
{
  int rc = check_basic_command_structure (msg_);
  if (rc == -1)
    return -1;
...
int
zmq::curve_server_t::produce_ready (msg_t *msg_)
{
...
int
zmq::curve_server_t::produce_error (msg_t *msg_) const
{
...
void
zmq::curve_server_t::send_zap_request (const uint8_t *key_)
{
