...
Mirrors python/dsp/doppler/tests/test_resamp_c.py
TestHalfbandDecimator{Create,Output,Spectral}.

The halfband bank is built at module scope from the same Kaiser spec
used by the reference (60 dB, 0.4/0.6 pb/sb, phases=2).
"""
...
# ------------------------------------------------------------------ #
# Spectral helpers (mirror test_spectral.py)                          #
# ------------------------------------------------------------------ #
...
class TestHalfbandDecimatorCreate:
    ...
    def test_2d_array_raises(self):
        """2-D bank is no longer accepted; pass a 1-D FIR row."""
        with pytest.raises(ValueError):
...
# ------------------------------------------------------------------ #
# Output size, dtype, statefulness                                     #
# ------------------------------------------------------------------ #
...
class TestHalfbandDecimatorOutput:
    ...
    def test_stateful_across_calls(self, hb_fir):
        """Two half-block calls must match one full-block call."""
...
N_IN = 8192
PASSBAND_FREQ_IN = 0.1  # output rate: 0.2 (< 0.25 passband edge)
STOPBAND_FREQ_IN = 0.6  # input rate: in the stopband (> 0.5·Fnyq_in)
...
class TestHalfbandDecimatorSpectral:
    ...
    def test_passband_amplitude(self, hb_fir):
        """Tone at 0.1 Fin (0.2 Fout) passes with < 0.1 dB error."""
        y = self._run(hb_fir, PASSBAND_FREQ_IN)
        bins, db = _spectrum(y)
        # input 0.1 → output 0.2 (rate = 0.5, so f_out = f_in / rate)
    ...
    def test_passband_tone_present(self, hb_fir):
        """Peak lands at the expected output frequency."""
        y = self._run(hb_fir, PASSBAND_FREQ_IN)
        bins, db = _spectrum(y)
