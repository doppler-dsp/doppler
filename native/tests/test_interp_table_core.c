...
main (void)
{
   ...
   * ---------------------------------------------------------------- */
  {
    DP_CHECK (dp_interp_table_create (NULL, 0, 2) == NULL);
    dp_interp_table_destroy (NULL); /* must not crash */

    double _Complex t[3]         = { 10.0, 20.0, 30.0 };
    dp_interp_table_state_t *obj = dp_interp_table_create (t, 3, 2);
    DP_CHECK (obj != NULL);
    if (!obj)
      return 1;
    DP_CHECK (obj->n == 3);
    dp_interp_table_reset (obj); /* no-op, must not crash */
    dp_interp_table_destroy (obj);
  }
  ...
  {
    double _Complex t[3]           = { 10.0, 20.0, 30.0 };
    dp_interp_table_state_t *obj   = dp_interp_table_create (t, 3, 0);
    double                   in[3] = { 0.9, 1.9, 2.9 };
    double _Complex out[3];
    DP_CHECK (dp_interp_table_execute (obj, in, 3, out, 3) == 3);
  ...
  {
    double _Complex t[3]           = { 10.0, 20.0, 30.0 };
    dp_interp_table_state_t *obj   = dp_interp_table_create (t, 3, 1);
   ...
   * ---------------------------------------------------------------- */
  {
    ...
    dp_interp_table_state_t *obj = dp_interp_table_create (t, 2, 2);
    t[0]         = 999.0; /* mutate the caller's own array after create() */
    double in[1] = { 0.0 };
    double _Complex out[1];
    dp_interp_table_execute (obj, in, 1, out, 1);
  ...
  /* ── pass_capacity: emission stops at max_out (jm gh-138) ────────── */
  {
    ...
    dp_interp_table_state_t *obj   = dp_interp_table_create (tab, 4, 2);
    ...
    DP_CHECK (dp_interp_table_execute (obj, in, 6, full, 6) == 6);
    DP_CHECK (dp_interp_table_execute (obj, in, 6, out, 2) == 2);
    for (int i = 0; i < 2; i++)
    ...
    DP_CHECK (dp_interp_table_execute (obj, in, 6, out, 0) == 0);
