...
void
socket_config_null_client (void *server_, void *server_secret_)
{
...
void
socket_config_null_server (void *server_, void *server_secret_)
{
...
void
socket_config_plain_client (void *server_, void *server_secret_)
{
  LIBZMQ_UNUSED (server_secret_);
...
void
socket_config_plain_server (void *server_, void *server_secret_)
{
  LIBZMQ_UNUSED (server_secret_);
...
void
socket_config_curve_server (void *server_, void *server_secret_)
{
  int as_server = 1;
  TEST_ASSERT_SUCCESS_ERRNO (
...
void
socket_config_curve_client (void *client_, void *data_)
{
...
void
zap_handler_generic (zap_protocol_t zap_protocol_,
...
static void
setup_handshake_socket_monitor (void *server_, void **server_mon_,
...
void
setup_context_and_server_side (void **zap_control_, void **zap_thread_,
                               void **server_, void **server_mon_,
                               char *my_endpoint_, zmq_thread_fn zap_handler_,
                               ...
                               const char *routing_id_)
{
  ...
  *server_ = test_context_socket (ZMQ_DEALER);
  ...
  TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (*server_, ZMQ_MAXMSGSIZE,
                                             &max_msg_size, sizeof (int64_t)));
...
void
shutdown_context_and_server_side (void *zap_thread_, void *server_,
                                  void *server_mon_, void *zap_control_,
...
void *
create_and_connect_client (char *my_endpoint_, socket_config_fn socket_config_,
                           void *socket_config_data_, void **client_mon_)
{
  void *client = test_context_socket (ZMQ_DEALER);
  //  As per API by default there's no limit to the size of a message,
  //  but the sanitizer allocator will barf over a gig or so
  int64_t max_msg_size = 64 * 1024 * 1024;
  TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (client, ZMQ_MAXMSGSIZE,
                                             &max_msg_size, sizeof (int64_t)));
...
void
expect_new_client_bounce_fail (char *my_endpoint_, void *server_,
                               socket_config_fn socket_config_,
                               void *socket_config_data_, void **client_mon_,
  ...
  if (expected_client_event_ != 0)
    {
      ...
      events_received = expect_monitor_event_multiple (
          my_client_mon, expected_client_event_, expected_client_value_,
          false);
