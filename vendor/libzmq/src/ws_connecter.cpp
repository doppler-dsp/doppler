...
zmq::ws_connecter_t::ws_connecter_t (class io_thread_t *io_thread_,
    ...
    : stream_connecter_base_t (io_thread_, session_, options_, addr_,
                               delayed_start_),
      _connect_timer_started (false), _wss (wss_), _hostname (tls_hostname_)
{
}
...
void
zmq::ws_connecter_t::process_term (int linger_)
{
...
zmq::ws_connecter_t::out_event ()
{
  ...
  if (_wss)
#ifdef ZMQ_HAVE_WSS
    create_engine (fd, get_socket_name<wss_address_t> (fd, socket_end_local));
#else
    assert (false);
#endif
  else
...
void
zmq::ws_connecter_t::timer_event (int id_)
{
...
void
zmq::ws_connecter_t::start_connecting ()
{
...
void
zmq::ws_connecter_t::add_connect_timer ()
{
...
zmq::ws_connecter_t::open ()
{
  ...
  if (last_error == WSdpINPROGRESS || last_error == WSdpWOULDBLOCK)
    errno = EINPROGRESS;
  else
...
zmq::ws_connecter_t::connect ()
{
  ...
  if (err != 0)
    {
      if (err == WSdpBADF || err == WSdpNOPROTOOPT || err == WSdpNOTSOCK
          || err == WSdpNOBUFS)
        {
          wsa_assert_no (err);
        }
  ...
  if (err != 0)
    {
      ...
      errno_assert (errno != EBADF && errno != ENOPROTOOPT && errno != ENOTSOCK
                    && errno != ENOBUFS);
#else
...
bool
zmq::ws_connecter_t::tune_socket (const fd_t fd_)
{
  const int rc
...
void
zmq::ws_connecter_t::create_engine (fd_t fd_,
                                    const std::string &local_address_)
{
  ...
  if (_wss)
    ...
    engine = new (std::nothrow) ws_engine_t (fd_, options, endpoint_pair,
                                             *_addr->resolved.ws_addr, true);
  alloc_assert (engine);
