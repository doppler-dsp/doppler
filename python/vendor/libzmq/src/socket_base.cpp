...
void
zmq::socket_base_t::inprocs_t::emplace (const char *endpoint_uri_,
                                        pipe_t *pipe_)
{
...
void
zmq::socket_base_t::inprocs_t::erase_pipe (const pipe_t *pipe_)
{
...
zmq::socket_base_t *
zmq::socket_base_t::create (int type_, class ctx_t *parent_, uint32_t tid_,
                            int sid_)
{
...
zmq::socket_base_t::socket_base_t (ctx_t *parent_, uint32_t tid_, int sid_,
                                   bool thread_safe_)
    : own_t (parent_, tid_), _sync (), _tag (0xbaddecaf),
      _ctx_terminated (false), _destroyed (false), _poller (NULL),
      _handle (static_cast<poller_t::handle_t> (NULL)), _last_tsc (0),
      _ticks (0), _rcvmore (false), _monitor_socket (NULL),
      _monitor_events (0), _thread_safe (thread_safe_),
      _reaper_signaler (NULL), _monitor_sync ()
{
...
int
zmq::socket_base_t::get_peer_state (const void *routing_id_,
...
int
zmq::socket_base_t::parse_uri (const char *uri_, std::string &protocol_,
                               std::string &path_)
{
...
int
zmq::socket_base_t::check_protocol (const std::string &protocol_) const
{
...
void
zmq::socket_base_t::attach_pipe (pipe_t *pipe_, bool subscribe_to_all_,
...
int
zmq::socket_base_t::setsockopt (int option_, const void *optval_,
                                size_t optvallen_)
{
...
int
zmq::socket_base_t::getsockopt (int option_, void *optval_, size_t *optvallen_)
{
  scoped_optional_lock_t sync_lock (_thread_safe ? &_sync : NULL);
...
int
zmq::socket_base_t::join (const char *group_)
{
  scoped_optional_lock_t sync_lock (_thread_safe ? &_sync : NULL);
...
int
zmq::socket_base_t::leave (const char *group_)
{
  scoped_optional_lock_t sync_lock (_thread_safe ? &_sync : NULL);
...
void
zmq::socket_base_t::add_signaler (signaler_t *s_)
{
...
void
zmq::socket_base_t::remove_signaler (signaler_t *s_)
{
  zmq_assert (_thread_safe);
...
int
zmq::socket_base_t::bind (const char *endpoint_uri_)
{
  scoped_optional_lock_t sync_lock (_thread_safe ? &_sync : NULL);
  ...
  if (protocol == protocol_name::udp)
    {
      ...
      //  TODO shouldn't this use _last_endpoint instead of endpoint_uri_? as
      //  in the other cases
...
int
zmq::socket_base_t::connect (const char *endpoint_uri_)
{
  scoped_optional_lock_t sync_lock (_thread_safe ? &_sync : NULL);
...
int
zmq::socket_base_t::connect_internal (const char *endpoint_uri_)
{
  if (unlikely (_ctx_terminated))
    {
      errno = ETERM;
  ...
  if (protocol == protocol_name::inproc)
    {
      ...
      object_t *parents[2]
          = { this, peer.socket == NULL ? this : peer.socket };
      pipe_t *new_pipes[2] = { NULL, NULL };
      ...
      if (!peer.socket)
      ...
      else
        {
          ...
          //  peer's seqnum was incremented in find_endpoint function. We don't
  ...
  if (protocol == protocol_name::tcp)
    {
      ...
      //  - IPv6 link local address may contain % followed by interface name /
      //  zone_id
  ...
  if (options.immediate != 1 || subscribe_to_all)
    {
      ...
      int hwms[2]
          = { conflate ? -1 : options.sndhwm, conflate ? -1 : options.rcvhwm };
      bool conflates[2] = { conflate, conflate };
      rc = pipepair (parents, new_pipes, hwms, conflates);
      errno_assert (rc == 0);
...
void
zmq::socket_base_t::add_endpoint (const endpoint_uri_pair_t &endpoint_pair_,
...
int
zmq::socket_base_t::term_endpoint (const char *endpoint_uri_)
{
  scoped_optional_lock_t sync_lock (_thread_safe ? &_sync : NULL);
  ...
  //  unprocessed process_own()'s (from launch_child() for example) we're asked
  //  to terminate now.
  const int rc = process_commands (0, false);
  if (unlikely (rc != 0))
    {
  ...
  //  Find the endpoints range (if any) corresponding to the endpoint_uri_pair_
  //  string.
...
int
zmq::socket_base_t::send (msg_t *msg_, int flags_)
{
  scoped_optional_lock_t sync_lock (_thread_safe ? &_sync : NULL);
...
int
zmq::socket_base_t::recv (msg_t *msg_, int flags_)
{
  scoped_optional_lock_t sync_lock (_thread_safe ? &_sync : NULL);
...
void
zmq::socket_base_t::start_reaping (poller_t *poller_)
{
...
int
zmq::socket_base_t::process_commands (int timeout_, bool throttle_)
{
  if (timeout_ == 0)
    {
...
void
zmq::socket_base_t::process_bind (pipe_t *pipe_)
{
  attach_pipe (pipe_);
}

void
zmq::socket_base_t::process_term (int linger_)
{
...
void
zmq::socket_base_t::process_term_endpoint (std::string *endpoint_)
{
...
zmq::socket_base_t::process_pipe_stats_publish (
    uint64_t outbound_queue_count_, uint64_t inbound_queue_count_,
...
void
zmq::socket_base_t::update_pipe_options (int option_)
{
...
int
zmq::socket_base_t::xsetsockopt (int, const void *, size_t)
{
  errno = EINVAL;
  return -1;
}
...
int
zmq::socket_base_t::xgetsockopt (int, void *, size_t *)
{
  errno = EINVAL;
  return -1;
}
...
int
zmq::socket_base_t::xjoin (const char *group_)
{
...
int
zmq::socket_base_t::xleave (const char *group_)
{
  LIBZMQ_UNUSED (group_);
  errno = ENOTSUP;
  return -1;
...
void
zmq::socket_base_t::xread_activated (pipe_t *)
{
  zmq_assert (false);
}
void
zmq::socket_base_t::xwrite_activated (pipe_t *)
{
  zmq_assert (false);
}

void
zmq::socket_base_t::xhiccuped (pipe_t *)
{
  zmq_assert (false);
}
...
void
zmq::socket_base_t::read_activated (pipe_t *pipe_)
{
  xread_activated (pipe_);
}

void
zmq::socket_base_t::write_activated (pipe_t *pipe_)
{
  xwrite_activated (pipe_);
}

void
zmq::socket_base_t::hiccuped (pipe_t *pipe_)
{
...
void
zmq::socket_base_t::pipe_terminated (pipe_t *pipe_)
{
...
int
zmq::socket_base_t::monitor (const char *endpoint_, uint64_t events_,
...
void
zmq::socket_base_t::event (const endpoint_uri_pair_t &endpoint_uri_pair_,
                           uint64_t values_[], uint64_t values_count_,
                           uint64_t type_)
{
  scoped_lock_t lock (_monitor_sync);
...
zmq::socket_base_t::monitor_event (
    uint64_t event_, const uint64_t values_[], uint64_t values_count_,
...
void
zmq::socket_base_t::stop_monitor (bool send_monitor_stopped_event_)
{
  // this is a private method which is only called from
  // contexts where the _monitor_sync mutex has been locked before
...
int
zmq::routing_socket_base_t::xsetsockopt (int option_, const void *optval_,
                                         size_t optvallen_)
{
  switch (option_)
    {
...
void
zmq::routing_socket_base_t::xwrite_activated (pipe_t *pipe_)
{
...
void
zmq::routing_socket_base_t::add_out_pipe (blob_t routing_id_, pipe_t *pipe_)
{
...
bool
zmq::routing_socket_base_t::has_out_pipe (const blob_t &routing_id_) const
{
...
{
  // TODO we could probably avoid constructor a temporary blob_t to call this
  // function
...
void
zmq::routing_socket_base_t::erase_out_pipe (const pipe_t *pipe_)
{
