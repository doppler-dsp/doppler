...
zmq::ws_decoder_t::ws_decoder_t (size_t bufsize_, int64_t maxmsgsize_,
                                 bool zero_copy_, bool must_mask_)
    : decoder_base_t<ws_decoder_t, shared_message_memory_allocator> (bufsize_),
      _msg_flags (0), _zero_copy (zero_copy_), _max_msg_size (maxmsgsize_),
      _must_mask (must_mask_), _size (0)
{
...
int
zmq::ws_decoder_t::opcode_ready (unsigned char const *)
{
...
int
zmq::ws_decoder_t::size_first_byte_ready (unsigned char const *read_from_)
{
...
int
zmq::ws_decoder_t::short_size_ready (unsigned char const *read_from_)
{
...
int
zmq::ws_decoder_t::long_size_ready (unsigned char const *read_from_)
{
...
int
zmq::ws_decoder_t::mask_ready (unsigned char const *read_from_)
{
...
int
zmq::ws_decoder_t::flags_ready (unsigned char const *read_from_)
{
  unsigned char flags;
...
int
zmq::ws_decoder_t::size_ready (unsigned char const *read_pos_)
{
  ...
  // buffer data into a new message and complete it in the next receive.
  ...
  if (unlikely (!_zero_copy || allocator.data () > read_pos_
    ...
    {
      // a new message has started, but the size would exceed the pre-allocated
      // arena (or read_pos_ is in the initial handshake buffer) this happens
      // every time when a message does not fit completely into the buffer
  ...
  else
    {
      ...
      // For small messages, data has been copied and refcount does not have to
      // be increased
...
int
zmq::ws_decoder_t::message_ready (unsigned char const *)
{
  if (_must_mask)
    {
