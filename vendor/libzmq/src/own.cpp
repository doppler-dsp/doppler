...
zmq::own_t::own_t (class ctx_t *parent_, uint32_t tid_)
    : object_t (parent_, tid_), _terminating (false), _sent_seqnum (0),
      _processed_seqnum (0), _owner (NULL), _term_acks (0)
{
}

zmq::own_t::own_t (io_thread_t *io_thread_, const options_t &options_)
    : object_t (io_thread_), options (options_), _terminating (false),
      _sent_seqnum (0), _processed_seqnum (0), _owner (NULL), _term_acks (0)
{
}
...
void
zmq::own_t::set_owner (own_t *owner_)
{
  zmq_assert (!_owner);
  _owner = owner_;
}
...
void
zmq::own_t::launch_child (own_t *object_)
{
...
void
zmq::own_t::term_child (own_t *object_)
{
  process_term_req (object_);
}

void
zmq::own_t::process_term_req (own_t *object_)
{
...
void
zmq::own_t::process_own (own_t *object_)
{
...
void
zmq::own_t::process_term (int linger_)
{
...
void
zmq::own_t::register_term_acks (int count_)
{
  _term_acks += count_;
}
