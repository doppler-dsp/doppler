...
//
static int
close_wait_ms (int fd_, unsigned int max_ms_ = 2000)
{
...
zmq::signaler_t::send ()
{
  ...
  if (unlikely (pid != getpid ()))
    {
      // printf("Child process %d signaler_t::send returning without sending
      // #1\n", getpid());
  ...
  do
    {
      ...
      // wsa_assert does not abort on WSdpWOULDBLOCK. If we get this, we retry.
    }
  while (nbytes == SOCKET_ERROR);
  // Given the small size of dummy (should be 1) expect that send was able to
  // send everything.
  ...
  while (true)
    {
      ...
      if (unlikely (pid != getpid ()))
        {
          // printf("Child process %d signaler_t::send returning without
          // sending #2\n", getpid());
          errno = EINTR;
          break;
        }
...
int
zmq::signaler_t::wait (int timeout_) const
{
#ifdef HAVE_FORK
  if (unlikely (pid != getpid ()))
    {
  ...
  wsa_assert (rc != SOCKET_ERROR);
#else
  int rc = select (_r + 1, fds.get (), NULL, NULL,
  ...
  if (nbytes == SOCKET_ERROR)
    {
      ...
      wsa_assert (last_error == WSdpWOULDBLOCK);
    }
#elif defined ZMQ_HAVE_VXWORKS
  ssize_t nbytes = ::recv (_r, (char *)&dummy, sizeof (dummy), 0);
  if (nbytes == -1)
