...
`--seed`), so output is reproducible and receiver-correlatable. A `bits`
waveform instead plays back **your** sequence — a preamble, sync word, or test
vector — given as a 0/1 string (`--bits 10110101`), a hex string (`--bits-hex AA55`, MSB first), or a file (`--bits-file pattern.txt`); `--modulation`
(`none`/`bpsk`/`qpsk`) maps the bits to symbols. Each bit is held `--sps`
samples and the pattern cycles to fill `--count`.
...
| -------------- | ------------------------------ | -------- | ---------------------------------------------------------------- |
| `--type`       | `tone noise pn bpsk qpsk bits` | `tone`   | waveform                                                         |
