...
//  in case of error.
static int
get_monitor_event_internal (void *monitor_, int *value_, char **address_,
                            int recv_flag_)
{
...
int
get_monitor_event_with_timeout (void *monitor_, int *value_, char **address_,
                                int timeout_)
{
  int res;
  if (timeout_ == -1)
    {
      ...
      zmq_setsockopt (monitor_, ZMQ_RCVTIMEO, &timeout_step,
                      sizeof (timeout_step));
      while ((res = get_monitor_event_internal (monitor_, value_, address_, 0))
             == -1)
        {
...
int
get_monitor_event (void *monitor_, int *value_, char **address_)
{
  return get_monitor_event_with_timeout (monitor_, value_, address_, -1);
}

void
expect_monitor_event (void *monitor_, int expected_event_)
{
...
static void
print_unexpected_event (char *buf_, size_t buf_size_, int event_, int err_,
                        int expected_event_, int expected_err_)
{
...
void
print_unexpected_event_stderr (int event_, int err_, int expected_event_,
                               int expected_err_)
{
  char buf[256];
...
int
expect_monitor_event_multiple (void *server_mon_, int expected_event_,
                               int expected_err_, bool optional_)
{
  ...
  while ((event
         ...
         || !count_of_expected_events)
    {
      ...
      if (event != expected_event_
        ...
        {
          char buf[256];
          print_unexpected_event (buf, sizeof buf, event, err, expected_event_,
                                  expected_err_);
...
static int64_t
get_monitor_event_internal_v2 (void *monitor_, uint64_t **value_,
                               char **local_address_, char **remote_address_,
                               int recv_flag_)
{
  ...
  if (value_)
    {
      *value_ = (uint64_t *)malloc ((size_t)value_count * sizeof (uint64_t));
...
static int64_t
get_monitor_event_with_timeout_v2 (void *monitor_, uint64_t **value_,
                                   char **local_address_,
                                   char **remote_address_, int timeout_)
{
  int64_t res;
  if (timeout_ == -1)
    {
...
int64_t
get_monitor_event_v2 (void *monitor_, uint64_t **value_, char **local_address_,
...
void
expect_monitor_event_v2 (void *monitor_, int64_t expected_event_,
...
const char *
get_zmqEventName (uint64_t event)
{
  switch (event)
    {
...
void
print_events (void *socket, int timeout, int limit)
{
