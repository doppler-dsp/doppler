...
zmq::ws_listener_t::ws_listener_t (io_thread_t *io_thread_,
                                   ...
                                   const options_t &options_, bool wss_)
...
std::string
zmq::ws_listener_t::get_socket_name (zmq::fd_t fd_,
...
int
zmq::ws_listener_t::create_socket (const char *addr_)
{
...
int
zmq::ws_listener_t::set_local_address (const char *addr_)
{
  if (options.use_fd != -1)
    {
  ...
  if (sock == retired_fd)
    {
      ...
      wsa_assert (last_error == WSdpWOULDBLOCK || last_error == WSdpCONNRESET
                  || last_error == WSdpMFILE || last_error == WSdpNOBUFS);
...
void
zmq::ws_listener_t::create_engine (fd_t fd_)
{
