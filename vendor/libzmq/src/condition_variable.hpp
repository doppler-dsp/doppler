...
//  Condition variable class encapsulates OS mutex in a platform-independent
//  way.
...
public:
  ...
  inline int
  wait (mutex_t *mutex_, int timeout_)
  {
    zmq_assert (false);
    return -1;
  }
...
public:
  ...
  wait (mutex_t *mutex_, int timeout_)
  {
    ...
    if (timeout_ == -1)
      {
        _cv.wait (*mutex_); // unlock mtx and wait cv.notify_all(), lock mtx
                            // after cv.notify_all()
      }
  ...
  broadcast ()
  {
    // this assumes that the mutex associated with _cv has been locked by the
    // caller
    _cv.notify_all ();
  }
...
#ifdef ZMQ_HAVE_OSX
        rc = pthread_cond_timedwait_relative_np (&_cond, mutex_->get_mutex (),
                                                 &timeout);
#elif defined(ANDROID_LEGACY)
        rc = pthread_cond_timedwait_monotonic_np (&_cond, mutex_->get_mutex (),
                                                  &timeout);
#else
