...
zmq::req_t::req_t (class ctx_t *parent_, uint32_t tid_, int sid_)
    : dealer_t (parent_, tid_, sid_), _receiving_reply (false),
      _message_begins (true), _reply_pipe (NULL),
      _request_id_frames_enabled (false), _request_id (generate_random ()),
      _strict (true)
{
  ...
  while (_message_begins)
    {
      ...
      // TODO: Failing this check should also close the connection with the
      // peer!
      int rc = recv_reply_pipe (msg_);
      if (rc != 0)
        return rc;
...
int
zmq::req_t::xsetsockopt (int option_, const void *optval_, size_t optvallen_)
{
...
void
zmq::req_t::xpipe_terminated (pipe_t *pipe_)
{
...
int
zmq::req_t::recv_reply_pipe (msg_t *msg_)
{
  while (true)
    {
      pipe_t *pipe = NULL;
...
int
zmq::req_session_t::push_msg (msg_t *msg_)
{
