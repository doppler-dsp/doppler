...
{
public:
  test_ip_resolver_t (zmq::ip_resolver_options_t opts_) : ip_resolver_t (opts_)
  {
  }
...
protected:
  ...
  int
  do_getaddrinfo (const char *node_, const char *service_,
  ...
  unsigned int
  do_if_nametoindex (const char *ifname_) ZMQ_FINAL
  {
...
//
//  On windows we can receive an IPv4 address even when an IPv6 is requested,
//  if we're in this situation then we compare to 'expected_addr_v4_failover_'
//  instead.
static void
test_resolve (zmq::ip_resolver_options_t opts_, const char *name_,
              const char *expected_addr_, uint16_t expected_port_ = 0,
...
static void
test_bind_any_port0 (bool ipv6_)
{
  zmq::ip_resolver_options_t resolver_opts;
...
static void
test_nobind_any_port (bool ipv6_)
{
  zmq::ip_resolver_options_t resolver_opts;
...
static void
test_nobind_addr_anyport (bool ipv6_)
{
  zmq::ip_resolver_options_t resolver_opts;
...
static void
test_nobind_addr_port0 (bool ipv6_)
{
  zmq::ip_resolver_options_t resolver_opts;
...
test_dns_ipv6_scope ()
{
  ...
  test_resolve (resolver_opts, "ip.zeromq.org%lo0", "fdf5:d058:d656::1", 0, 1);
}
...
static void
test_addr (int family_, const char *addr_, bool multicast_)
{
