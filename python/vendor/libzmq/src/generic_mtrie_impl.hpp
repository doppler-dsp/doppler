...
template <typename T>
bool
generic_mtrie_t<T>::add (prefix_t prefix_, size_t size_, value_t *pipe_)
{
...
void
generic_mtrie_t<T>::rm (value_t *pipe_,
                        void (*func_) (prefix_t data_, size_t size_, Arg arg_),
                        Arg arg_, bool call_on_uniq_)
{
  //  This used to be implemented as a non-tail recursive traversal of the
  //  trie, which means remote clients controlled the depth of the recursion
  //  and the stack size. To simulate the non-tail recursion, with
  //  post-recursion changes depending on the result of the recursive call, a
  //  stack is used to re-visit the same node and operate on it again after
  //  children have been visited. A boolean is used to record whether the node
  //  had already been visited and to determine if the pre- or post- children
  //  visit actions have to be taken. In the case of a node with (N > 1)
  //  children, the node has to be re-visited N times, in the correct order
  //  after each child visit.
      ...
      if (!it.processed_for_removal)
        {
          ...
          switch (it.node->_count)
            {
            case 0:
              //  If there are no subnodes in the trie, we are done with this
              //  node pre-processing.
              break;
            case 1:
              {
                ...
                //  Mark this node as pre-processed and push it, so that the
                //  next visit after the operation on the child can do the
                //  removals.
            ...
            default:
              {
                ...
                //  When first visiting this node, initialize the new_min/max
                //  parameters which will then be used after each child has
                //  been processed, on the post-children iterations.
                if (it.current_child == 0)
                  {
                    //  New min non-null character in the node table after the
                    //  removal
                    it.new_min = it.node->_min + it.node->_count - 1;
                    //  New max non-null character in the node table after the
                    //  removal
        ...
        {
          //  Reset back for the next time, in case this node doesn't get
          //  deleted. This is done unconditionally, unlike when setting this
          //  variable to true.
              ...
              //  If there are multiple subnodes.
              {
                if (it.node->_next.table[it.current_child])
                  {
                    ...
                    if (it.node->_next.table[it.current_child]
                            ->is_redundant ())
                      {
                        LIBZMQ_DELETE (it.node->_next.table[it.current_child]);
                      ...
                      {
                        //  The node is not redundant, so it's a candidate for
                        //  being the new min/max node.
                        //
                        //  We loop through the node array from left to right,
                        //  so the first non-null, non-redundant node
                        //  encountered is the new minimum index. Conversely,
                        //  the last non-redundant, non-null node encountered
                        //  is the new maximum index.
                ...
                switch (it.node->_live_nodes)
                  ...
                  case 1:
                    ...
                    zmq_assert (it.new_min < it.node->_min + it.node->_count);
                    {
                  ...
                  default:
                    if (it.new_min > it.node->_min
                      ...
                      {
                        zmq_assert (it.new_max - it.new_min + 1 > 1);
...
generic_mtrie_t<T>::rm (prefix_t prefix_, size_t size_, value_t *pipe_)
{
  ...
  //  children have been visited. A boolean is used to record whether the node
  //  had already been visited and to determine if the pre- or post- children
  //  visit actions have to be taken.
      ...
      if (!it.processed_for_removal)
        {
          ...
          stack.push_back (it);
          struct iter next
              = { it.next_node, NULL, it.prefix + 1, it.size - 1, 0, 0, 0,
                  false };
          stack.push_back (next);
        }
      else
...
template <typename Arg>
void
generic_mtrie_t<T>::match (prefix_t data_, size_t size_,
                           void (*func_) (value_t *pipe_, Arg arg_), Arg arg_)
{
...
template <typename T>
bool
generic_mtrie_t<T>::is_redundant () const
{
