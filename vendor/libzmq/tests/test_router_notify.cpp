...
test_sockopt_router_notify ()
{
  ...
  // value boundary
  opt_notify = -1;
  TEST_ASSERT_FAILURE_ERRNO (EINVAL, zmq_setsockopt (router, ZMQ_ROUTER_NOTIFY,
                                                     &opt_notify,
                                                     sizeof (opt_notify)));
  ...
  opt_notify = ZMQ_NOTIFY_CONNECT;
  TEST_ASSERT_FAILURE_ERRNO (EINVAL, zmq_setsockopt (dealer, ZMQ_ROUTER_NOTIFY,
                                                     &opt_notify,
                                                     sizeof (opt_notify)));
...
void
test_router_notify_helper (int opt_notify_)
{
  void *router = test_context_socket (ZMQ_ROUTER);
  int opt_more;
