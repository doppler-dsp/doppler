...
main (void)
{
    ...
    for (int r = 0; r < n_rates; r++)
      {
        ...
        size_t n_meas = (n_out > (size_t)n_drop) ? n_out - (size_t)n_drop : 0;

        double settled = (n_meas > 0) ? rms (out + n_drop, n_meas) : 0.0;
  ...
  printf ("--- 4. Split-block state persistence ---\n");
  {
    ...
    int ok
        = (nw == ns)
          && memcmp (out_whole, out_split, nw * sizeof (float _Complex)) == 0;
