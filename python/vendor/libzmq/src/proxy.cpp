...
int
zmq::proxy (class socket_base_t *frontend_, class socket_base_t *backend_,
...
static int
forward (class zmq::socket_base_t *from_, class zmq::socket_base_t *to_,
         class zmq::socket_base_t *capture_, zmq::msg_t *msg_,
         stats_socket &recving, stats_socket &sending)
{
...
static int
handle_control (class zmq::socket_base_t *control_, proxy_state_t &state,
...
#ifdef ZMQ_HAVE_POLLER
int
zmq::proxy_steerable (class socket_base_t *frontend_,
                      ...
                      class socket_base_t *control_)
{
  ...
  //  Don't allocate these pollers from stack because they will take more than
  //  900 kB of stack! On Windows this blows up default stack of 1 MB and
  //  aborts the program. I wanted to use std::shared_ptr here as the best
  //  solution but that requires C++11...
  ...
  zmq::socket_poller_t *poller_in = new (std::nothrow)
      zmq::socket_poller_t; //  Poll only 'ZMQ_POLLIN' on all sockets. Initial
                            //  blocking poll in loop.
  zmq::socket_poller_t *poller_receive_blocked = new (std::nothrow)
      zmq::socket_poller_t; //  All except 'ZMQ_POLLIN' on 'frontend_'.

  //  If frontend_==backend_ 'poller_send_blocked' and 'poller_receive_blocked'
  //  are the same, 'ZMQ_POLLIN' is ignored. In that case 'poller_send_blocked'
  //  is not used. We need only 'poller_receive_blocked'. We also don't need
  //  'poller_both_blocked', 'poller_backend_only' nor 'poller_frontend_only'
  //  no need to initialize it. We save some RAM and time for initialization.
  ...
  if (frontend_ != backend_)
    {
      ...
      poller_both_blocked = new (std::nothrow)
          zmq::socket_poller_t; //  All except 'ZMQ_POLLIN' on both 'frontend_'
                                //  and 'backend_'.
      poller_frontend_only = new (std::nothrow)
          zmq::socket_poller_t; //  Only 'ZMQ_POLLIN' and 'ZMQ_POLLOUT' on
                                //  'frontend_'.
  ...
  if (frontend_equal_to_backend)
    {
      //  If frontend_==backend_ 'poller_send_blocked' and
      //  'poller_receive_blocked' are the same, so we don't need
      //  'poller_send_blocked'. We need only 'poller_receive_blocked'. We also
      //  don't need 'poller_both_blocked', no need to initialize it.
  ...
  else
    {
      ...
      rc = poller_receive_blocked->add (
          ...
          ZMQ_POLLIN
              | ZMQ_POLLOUT); //  All except 'ZMQ_POLLIN' on 'frontend_'.
      CHECK_RC_EXIT_ON_FAILURE ();
      rc = poller_frontend_only->add (frontend_, NULL,
  ...
  while (state != terminated)
    {
      //  Blocking wait initially only for 'ZMQ_POLLIN' - 'poller_wait' points
      //  to 'poller_in'. If one of receiving end's queue is full
      //  ('ZMQ_POLLOUT' not available), 'poller_wait' is pointed to
      //  'poller_receive_blocked', 'poller_send_blocked' or
      //  'poller_both_blocked'.
      ...
      //  Some of events waited for by 'poller_wait' have arrived, now poll for
      ...
      for (int i = 0; i < rc; i++)
        {
          ...
          if (events[i].socket == frontend_)
            ...
            //  This 'if' needs to be after check for 'frontend_' in order
            //  never to be reached in case frontend_==backend_, so we ensure
            //  backend_in=false in that case.
      ...
      if (state == active)
        {
          //  Process a request, 'ZMQ_POLLIN' on 'frontend_' and 'ZMQ_POLLOUT'
          //  on 'backend_'. In case of frontend_==backend_ there's no
          //  'ZMQ_POLLOUT' event.
          ...
          //  Process a reply, 'ZMQ_POLLIN' on 'backend_' and 'ZMQ_POLLOUT' on
          //  'frontend_'. If 'frontend_' and 'backend_' are the same this is
          //  not needed because previous processing covers all of the cases.
          //  'backend_in' is always false if frontend_==backend_ due to design
          //  in 'for' event processing loop.
          ...
          if (request_processed || reply_processed)
            {
              //  If request/reply is processed that means we had at least one
              //  'ZMQ_POLLOUT' event. Enable corresponding 'ZMQ_POLLIN' for
              //  blocking wait if any was disabled.
            ...
            {
              //  No requests have been processed, there were no 'ZMQ_POLLIN'
              //  with corresponding 'ZMQ_POLLOUT' events. That means that out
              //  queue(s) is/are full or one out queue is full and second one
              //  has no messages to process. Disable receiving 'ZMQ_POLLIN'
              //  for sockets for which there's no 'ZMQ_POLLOUT', or wait only
              //  on both 'backend_''s or 'frontend_''s 'ZMQ_POLLIN' and
              //  'ZMQ_POLLOUT'.
              if (frontend_in)
                {
                  if (frontend_out)
                    // If frontend_in and frontend_out are true, obviously
                    // backend_in and backend_out are both false. In that case
                    // we need to wait for both 'ZMQ_POLLIN' and 'ZMQ_POLLOUT'
                    // only on 'backend_'. We'll never get here in case of
                    // frontend_==backend_ because then frontend_out will
                    // always be false.
                    poller_wait = poller_backend_only;
                  else
                    {
              ...
              if (backend_in)
                {
                  //  Will never be reached if frontend_==backend_,
                  //  'backend_in' will always be false due to design in 'for'
                  //  event processing loop.
                  if (backend_out)
                    // If backend_in and backend_out are true, obviously
                    // frontend_in and frontend_out are both false. In that
                    // case we need to wait for both 'ZMQ_POLLIN' and
                    // 'ZMQ_POLLOUT' only on 'frontend_'.
                    poller_wait = poller_frontend_only;
                  else
                    {
                      ...
                      class socket_base_t *control_)
{
  ...
  zmq_pollitem_t itemsout[]
      = { { frontend_, 0, ZMQ_POLLOUT, 0 }, { backend_, 0, ZMQ_POLLOUT, 0 } };
  ...
  while (state != terminated)
    {
      ...
      //  Get the pollout separately because when combining this with pollin it
      //  maxes the CPU because pollout shall most of the time return directly.
      //  POLLOUT is only checked when frontend and backend sockets are not the
      //  same.
      if (frontend_ != backend_)
        {
