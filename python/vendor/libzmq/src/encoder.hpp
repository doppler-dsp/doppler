...
public:
  explicit encoder_base_t (size_t bufsize_)
      : _write_pos (0), _to_write (0), _next (NULL), _new_msg_flag (false),
        _buf_size (bufsize_),
  ...
  size_t
  encode (unsigned char **data_, size_t size_) ZMQ_FINAL
  {
...
protected:
  ...
  void
  next_step (void *write_pos_, size_t to_write_, step_t next_,
             bool new_msg_flag_)
  {
