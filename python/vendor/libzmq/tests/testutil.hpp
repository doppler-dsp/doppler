...
const uint8_t zmtp_ready_dealer[43]
    = { 4,   41,  5,   'R', 'E', 'A', 'D', 'Y', 11,  'S', 'o',
        'c', 'k', 'e', 't', '-', 'T', 'y', 'p', 'e', 0,   0,
        0,   6,   'D', 'E', 'A', 'L', 'E', 'R', 8,   'I', 'd',
        'e', 'n', 't', 'i', 't', 'y', 0,   0,   0,   0 };
...
inline const char *
as_setsockopt_opt_t (const void *opt)
{
...
inline const void *
as_setsockopt_opt_t (const void *opt_)
{
  return opt_;
}
#endif
...
//  Binds an ipv4 BSD socket to an ephemeral port, returns the compiled
//  sockaddr
...
fd_t connect_socket (const char *endpoint_, const int af_ = AF_INET,
                     const int protocol_ = IPPROTO_TCP);

//  Binds a BSD socket to an ephemeral port, returns the file descriptor.
//  The resulting ZMQ endpoint will be stored in my_endpoint, including the
//  protocol prefix, so ensure it is writable and of appropriate size. Works
//  with ipv4/ipv6/unix. With unix sockets address_/port_ can be empty and
//  my_endpoint_ will contain a random path.
fd_t bind_socket_resolve_port (const char *address_, const char *port_,
                               char *my_endpoint_, const int af_ = AF_INET,
                               const int protocol_ = IPPROTO_TCP);
