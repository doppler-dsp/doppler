...
`manifest-drift` gate. As of jm 0.55.1 doppler sets `c_style = "clang-format"`
...
> **Known defect — just-makeit#917.** `c_style` does not reach members
> reconciled in place, so `jm apply` leaves six fragments carrying two C
> styles at once (`ddc_ext_ddcr.c` gets `DdcrObj_enter` in GNU and
> `DdcrObj_destroy`/`DdcrObj_exit` in K&R beside it). Run
> `make lint-clang-format` after any `jm apply` until it lands.
