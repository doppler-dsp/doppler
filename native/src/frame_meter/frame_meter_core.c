...
dp_frame_meter_create (size_t target_errors, double conf)
{
  if (conf < 0.0 || conf >= 1.0)
    return NULL;
  dp_frame_meter_state_t *s = calloc (1, sizeof *s);
  if (!s)
    return NULL;
...
void
dp_frame_meter_destroy (dp_frame_meter_state_t *state)
{
  free (state);
}

void
dp_frame_meter_reset (dp_frame_meter_state_t *state)
{
  if (!state)
    return;
  state->frames        = 0;
...
void
dp_frame_meter_add (dp_frame_meter_state_t *state, int sync_ok, int crc)
{
  if (!state)
    return;
  state->frames++;
...
size_t
dp_frame_meter_get_frames (const dp_frame_meter_state_t *state)
{
  return state->frames;
}

size_t
dp_frame_meter_get_sync_detected (const dp_frame_meter_state_t *state)
{
  return state->sync_detected;
}

size_t
dp_frame_meter_get_crc_passed (const dp_frame_meter_state_t *state)
{
  return state->crc_passed;
}

size_t
dp_frame_meter_get_errors (const dp_frame_meter_state_t *state)
{
  return state->errors;
}

int
dp_frame_meter_get_enough (const dp_frame_meter_state_t *state)
{
...
size_t
dp_frame_meter_state_bytes (const dp_frame_meter_state_t *state)
{
  (void)state;
...
void
dp_frame_meter_get_state (const dp_frame_meter_state_t *state, void *blob)
{
  dp_writer_t w = dp_writer_init (blob, dp_frame_meter_state_bytes (state));
  dp_w_hdr (&w, FRAME_METER_STATE_MAGIC, FRAME_METER_STATE_VERSION,
            dp_frame_meter_state_bytes (state));
...
int
dp_frame_meter_set_state (dp_frame_meter_state_t *state, const void *blob)
{
  int rc
      = dp_state_validate (blob, dp_frame_meter_state_bytes (state),
                           FRAME_METER_STATE_MAGIC, FRAME_METER_STATE_VERSION);
  if (rc != DP_OK)
    return rc;
  dp_reader_t r = dp_reader_init (blob, dp_frame_meter_state_bytes (state));
