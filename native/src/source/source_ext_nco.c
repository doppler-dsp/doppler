...
NCOObj_steps_u32_scaled_ctrl (NCOObject *self, PyObject *args, PyObject *kwds)
{
  ...
  if (out_obj && out_obj != Py_None)
    {
      /* Require the exact output dtype — see steps_u32_ctrl above (gh-581). */
      if (!PyArray_Check (out_obj)
          || PyArray_TYPE ((PyArrayObject *)out_obj) != NPY_UINT32
          || !PyArray_ISWRITEABLE ((PyArrayObject *)out_obj))
        {
  ...
  nco_steps_u32_scaled_ctrl (self->handle,
                             (const float *)PyArray_DATA (ctrl_arr), ctrl_len,
                             (uint32_t *)PyArray_DATA ((PyArrayObject *)arr));
  Py_DECREF (ctrl_arr);
  return arr;
}
...
static PyMethodDef NCOObj_methods[] = {
  ...
  { "steps_u32_scaled_ctrl", (PyCFunction)NCOObj_steps_u32_scaled_ctrl,
    METH_VARARGS | METH_KEYWORDS,
  ...
  { "steps_u32_scaled_ctrl_max_out",
    ...
    "the ``out=`` buffer." },
  { "steps_u32_ovf_ctrl", (PyCFunction)NCOObj_steps_u32_ovf_ctrl,
    METH_VARARGS | METH_KEYWORDS,
