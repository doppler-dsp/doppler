...
{
  // NOTE1: if setrlimit() fails with EPERM, this means that current user has
  // not enough permissions. NOTE2: even for privileged users (e.g., root)
  // getrlimit() would usually return 0 as nice limit; the only way to
  //        discover if the user is able to increase the nice value is to
  //        actually try to change the rlimit:
  struct rlimit rlim;
  rlim.rlim_cur = 40;
  rlim.rlim_max = 40;
  if (setrlimit (RLIMIT_NICE, &rlim) == 0)
    {
      // rlim_cur == 40 means that this process is allowed to set a nice value
      // of -20
...
test_ctx_thread_opts ()
{
  ...
  // set context options that alter the background thread CPU
  // scheduling/affinity settings; as of ZMQ 4.2.3 this has an effect only on
  // POSIX systems (nothing happens on Windows, but still it should return
  // success):
  ...
  // in theory SCHED_OTHER supports only the static priority 0 but quoting the
  // docs
  ...
  // sched_setattr(2)." ZMQ will internally use nice(2) to set the nice value
  // when using SCHED_OTHER. However changing the nice value of a process
  // requires appropriate permissions... check that the current effective user
  // is able to do that:
  ...
  // (e.g., 5 to use CPU index 5) and use "top -H" or "taskset -pc" to see the
  // result
...
#if WAIT_FOR_BACKGROUND_THREAD_INSPECTION
  // this is useful when you want to use an external tool (like top or taskset)
  // to view properties of the background threads
