...
zmq::stream_listener_base_t::stream_listener_base_t (
    zmq::io_thread_t *io_thread_, zmq::socket_base_t *socket_,
    const zmq::options_t &options_)
    : own_t (io_thread_, options_), io_object_t (io_thread_), _s (retired_fd),
      _handle (static_cast<handle_t> (NULL)), _socket (socket_)
{
}
...
int
zmq::stream_listener_base_t::get_local_address (std::string &addr_) const
{
...
void
zmq::stream_listener_base_t::process_term (int linger_)
{
  rm_fd (_handle);
...
void
zmq::stream_listener_base_t::create_engine (fd_t fd_)
{
