...
Usage
-----
    from doppler.polyphase.matlab_optimization import optimize_dpmfs
...
__all__ = ["optimize_dpmfs", "optimize_pbf", "pbf_freq_resp",
           "pbf_imp_resp"]
...
def _der_trans_mat(M: int, i: int, a: float, b: float) -> np.ndarray:
    ...
    if i <= M:
        vals[i:] = a ** i * np.array(
            [_fact(m) / _fact(m - i) for m in range(i, M + 1)]
        )
...
def pbf_freq_resp(
    C: np.ndarray, omega_p: np.ndarray, a: float, b: float
) -> np.ndarray:
...
def pbf_imp_resp(
    ...
    n_grid  = (
        np.tile(mu_p, (N, 1))
        + np.tile(np.arange(N)[:, np.newaxis], (1, P))
    )                                            # (N, P)
    n_mu    = n_grid.flatten()
...
def optimize_pbf(
    ...
    W     = (K_pass * (omega <= omega_pass)
             + K_stop * (omega >= omega_stop))
    ...
    result = _linprog(
        g, A_ub=A_ub, b_ub=b_ub, A_eq=Aeq, b_eq=beq,
        bounds=bounds,
        method='highs',
        options={'disp': False},
    )
...
def optimize_dpmfs(
    ...
    C, delta = optimize_pbf(
        ...
        a=a, b=b,
        K_pass=K_pass, K_stop=K_stop,
        n_pass=n_pass, n_stop=n_stop,
        num_der=num_der,
    )
