...
Corr2DObj_init (Corr2DObject *self, PyObject *args, PyObject *kwds)
{
  ...
  if (!PyArg_ParseTupleAndKeywords (args, kwds, "O|Ki", kwlist, &ref_obj,
                                    &dwell_raw, &nthreads))
    return -1;
  ...
  self->handle = corr2d_create ((const float complex *)PyArray_DATA (ref_arr),
                                ref_dim0, ref_dim1, dwell, nthreads);
  Py_DECREF (ref_arr);
  if (!self->handle)
    {
