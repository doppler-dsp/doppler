...
int
zmq::do_getsockopt (void *const optval_, size_t *const optvallen_,
...
int
zmq::do_getsockopt (void *const optval_, size_t *const optvallen_,
                    const void *value_, const size_t value_len_)
{
...
static int
do_getsockopt_curve_key (void *const optval_, const size_t *const optvallen_,
...
template <typename T>
static int
do_setsockopt (const void *const optval_, const size_t optvallen_,
               T *const out_value_)
{
...
int
zmq::do_setsockopt_int_as_bool_strict (const void *const optval_,
...
int
zmq::do_setsockopt_int_as_bool_relaxed (const void *const optval_,
                                        const size_t optvallen_,
                                        bool *const out_value_)
{
  int value = -1;
                                         ...
                                         const size_t max_len_)
{
  ...
  // TODO why are empty strings allowed for some socket options, but not for
  // others?
...
template <typename T>
static int
do_setsockopt_set (const void *const optval_, const size_t optvallen_,
...
zmq::options_t::options_t ()
    : sndhwm (default_hwm), rcvhwm (default_hwm), affinity (0),
      routing_id_size (0), rate (100), recovery_ivl (10000),
      multicast_hops (1), multicast_maxtpdu (1500), sndbuf (-1), rcvbuf (-1),
      tos (0), priority (0), type (-1), linger (-1), connect_timeout (0),
      tcp_maxrt (0), reconnect_stop (0), reconnect_ivl (100),
      reconnect_ivl_max (0), backlog (100), maxmsgsize (-1), rcvtimeo (-1),
      sndtimeo (-1), ipv6 (false), immediate (0), filter (false),
      invert_matching (false), recv_routing_id (false), raw_socket (false),
      raw_notify (true), tcp_keepalive (-1), tcp_keepalive_cnt (-1),
      tcp_keepalive_idle (-1), tcp_keepalive_intvl (-1), mechanism (ZMQ_NULL),
      as_server (0), gss_principal_nt (ZMQ_GSSAPI_NT_HOSTBASED),
      gss_service_principal_nt (ZMQ_GSSAPI_NT_HOSTBASED),
      gss_plaintext (false), socket_id (0), conflate (false),
      handshake_ivl (30000), connected (false), heartbeat_ttl (0),
      heartbeat_interval (0), heartbeat_timeout (-1), use_fd (-1),
      zap_enforce_domain (false), loopback_fastpath (false),
      multicast_loop (true), in_batch_size (8192), out_batch_size (8192),
      zero_copy (true), router_notify (0), monitor_event_version (1),
      wss_trust_system (false), hello_msg (), can_send_hello_msg (false),
      disconnect_msg (), can_recv_disconnect_msg (false), hiccup_msg (),
      can_recv_hiccup_msg (false), norm_mode (ZMQ_NORM_CC),
      norm_unicast_nacks (false), norm_buffer_size (2048),
      norm_segment_size (1400), norm_block_size (16), norm_num_parity (4),
      norm_num_autoparity (0), norm_push_enable (false), busy_poll (0)
{
...
int
zmq::options_t::set_curve_key (uint8_t *destination_, const void *optval_,
                               size_t optvallen_)
{
  switch (optvallen_)
    {
    ...
    case CURVE_KEYSIZE_Z85 + 1:
      {
        const std::string s (static_cast<const char *> (optval_), optvallen_);
...
int
zmq::options_t::setsockopt (int option_, const void *optval_,
                            size_t optvallen_)
{
  ...
  switch (option_)
    ...
    case ZMQ_IMMEDIATE:
      // TODO why is immediate not bool (and called non_immediate, as its
      // meaning appears to be reversed)
    ...
    case ZMQ_ZAP_DOMAIN:
      return do_setsockopt_string_allow_empty_relaxed (optval_, optvallen_,
                                                       &zap_domain, UCHAR_MAX);
      ...
      if (optvallen_ > 0)
        {
          ...
          hello_msg = std::vector<unsigned char> (bytes, bytes + optvallen_);
        }
      else
        {
    ...
    case ZMQ_HICCUP_MSG:
      if (optvallen_ > 0)
        {
          unsigned char *bytes = (unsigned char *)optval_;
          hiccup_msg = std::vector<unsigned char> (bytes, bytes + optvallen_);
        }
      else
        {
...
int
zmq::options_t::getsockopt (int option_, void *optval_,
