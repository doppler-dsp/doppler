...
static int
clipped_maxsocket (int max_requested_)
{
...
zmq::ctx_t::ctx_t ()
    : _tag (ZMQ_CTX_TAG_VALUE_GOOD), _starting (true), _terminating (false),
      _reaper (NULL), _max_sockets (clipped_maxsocket (ZMQ_MAX_SOCKETS_DFLT)),
      _max_msgsz (INT_MAX), _io_thread_count (ZMQ_IO_THREADS_DFLT),
  ...
  if (!_starting)
    {
      ...
      //  Check whether termination was already underway, but interrupted and
      //  now restarted.
...
int
zmq::ctx_t::set (int option_, const void *optval_, size_t optvallen_)
{
...
int
zmq::ctx_t::get (int option_, void *optval_, const size_t *optvallen_)
{
  const bool is_int = (*optvallen_ == sizeof (int));
...
zmq::socket_base_t *
zmq::ctx_t::create_socket (int type_)
{
  scoped_lock_t locker (_slot_sync);
...
void
zmq::ctx_t::destroy_socket (class socket_base_t *socket_)
{
  scoped_lock_t locker (_slot_sync);
...
void
zmq::thread_ctx_t::start_thread (thread_t &thread_, thread_fn *tfn_,
                                 void *arg_, const char *name_) const
{
...
int
zmq::thread_ctx_t::set (int option_, const void *optval_, size_t optvallen_)
{
  const bool is_int = (optvallen_ == sizeof (int));
  int value = 0;
  if (is_int)
...
int
zmq::thread_ctx_t::get (int option_, void *optval_, const size_t *optvallen_)
{
  const bool is_int = (*optvallen_ == sizeof (int));
  int *value = static_cast<int *> (optval_);
...
void
zmq::ctx_t::send_command (uint32_t tid_, const command_t &command_)
{
  _slots[tid_]->send (command_);
}

zmq::io_thread_t *
zmq::ctx_t::choose_io_thread (uint64_t affinity_)
{
...
int
zmq::ctx_t::register_endpoint (const char *addr_, const endpoint_t &endpoint_)
{
...
int
zmq::ctx_t::unregister_endpoint (const std::string &addr_,
...
void
zmq::ctx_t::unregister_endpoints (const socket_base_t *const socket_)
{
  scoped_lock_t locker (_endpoints_sync);
...
zmq::endpoint_t
zmq::ctx_t::find_endpoint (const char *addr_)
{
  scoped_lock_t locker (_endpoints_sync);
...
void
zmq::ctx_t::pend_connection (const std::string &addr_,
...
void
zmq::ctx_t::connect_pending (const char *addr_,
                             zmq::socket_base_t *bind_socket_)
{
  ...
  for (pending_connections_t::iterator p = pending.first; p != pending.second;
       ++p)
    connect_inproc_sockets (bind_socket_, _endpoints[addr_].options, p->second,
                            bind_side);
...
void
zmq::ctx_t::connect_inproc_sockets (
    zmq::socket_base_t *bind_socket_, const options_t &bind_options_,
...
int
zmq::ctx_t::get_vmci_socket_family ()
{
