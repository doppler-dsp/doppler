...
std::pair<const K, V>
make_pair_fix_const (const K &k, const V &v)
{
...
struct blob_t
{
  ...
  explicit blob_t (const size_t size_)
      : _data (static_cast<unsigned char *> (malloc (size_))), _size (size_),
        _owned (true)
  {
  ...
  bool
  operator< (blob_t const &other_) const
  {
    const int cmpres
  ...
  void
  set_deep_copy (blob_t const &other_)
  {
    clear ();
  ...
  void
  set (const unsigned char *const data_, const size_t size_)
  {
    clear ();
  ...
  blob_t &
  operator= (blob_t &&other_) ZMQ_NOEXCEPT
  {
    if (this != &other_)
      {
        clear ();
