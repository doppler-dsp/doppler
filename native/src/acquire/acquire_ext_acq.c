/*
 * acquire_ext_acq.c — Acquisition type for the acquire module.
 *
...
static PyTypeObject AcquisitionObjType = {
  PyVarObject_HEAD_INIT (NULL, 0).tp_name = "acquire.Acquisition",
  ...
  = "Create a continuous-mode acquisition engine: always wideband\n"
    ...
    "    engine needs one -- its non-coherent looks cannot be chosen without "
    "a\n"
    "    target.\n"
    ...
    "    Whole code-only epochs a waveform's code-only window holds at any "
    "chip\n"
