...
# Struct dp\_ber\_meter\_state\_t



[**ClassList**](annotated.md) **>** [**dp\_ber\_meter\_state\_t**](structdp__ber__meter__state__t.md)
...
```C++
size_t dp_ber_meter_state_t::bit_errors;
```
...
```C++
size_t dp_ber_meter_state_t::bits;
```
...
```C++
size_t dp_ber_meter_state_t::bursts;
```
...
```C++
double dp_ber_meter_state_t::conf;
```
...
```C++
size_t dp_ber_meter_state_t::errors;
```
...
```C++
ber_align_t dp_ber_meter_state_t::last;
```



Result of the last [**dp\_ber\_meter\_align()**](ber__meter__core_8h.md#function-dp_ber_meter_align). 
...
```C++
size_t dp_ber_meter_state_t::mk_n;
```
...
```C++
size_t dp_ber_meter_state_t::mk_period;
```
...
```C++
size_t dp_ber_meter_state_t::mk_t0;
```
...
```C++
size_t dp_ber_meter_state_t::skipped;
```
...
```C++
size_t dp_ber_meter_state_t::symbols;
```
...
```C++
size_t dp_ber_meter_state_t::target_errors;
```
...
```C++
uint8_t* dp_ber_meter_state_t::truth;
```
...
```C++
size_t dp_ber_meter_state_t::truth_len;
```
