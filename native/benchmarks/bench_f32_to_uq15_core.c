...
main(void)
{
    ...
    uint16_t *out = malloc(BENCH_N * sizeof(uint16_t));
    ...
    volatile uint16_t _sink;

    /* warmup */
    for (int i = 0; i < 16; i++) _sink = f32_to_uq15_step(obj, in[i]);

    struct timespec t0, t1;
    jm_bench_t _bench = {0};

    printf("=== f32_to_uq15 benchmark ===\n");
    ...
    for (int r = 0; r < ITERATIONS; r++) {
        ...
        for (int i = 0; i < BENCH_N; i++)
            _sink = f32_to_uq15_step(obj, in[i]);
    ...
    for (int r = 0; r < ITERATIONS; r++) {
        clock_gettime(CLOCK_MONOTONIC, &t0);
        f32_to_uq15_steps(obj, in, out, BENCH_N);
        clock_gettime(CLOCK_MONOTONIC, &t1);
    ...
    jm_bench_write_json(&_bench, "f32_to_uq15");
