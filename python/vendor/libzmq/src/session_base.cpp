...
zmq::session_base_t *
zmq::session_base_t::create (class io_thread_t *io_thread_, bool active_,
  ...
  switch (options_.type)
    ...
    case ZMQ_CHANNEL:
      ...
      if (options_.can_send_hello_msg && options_.hello_msg.size () > 0)
        s = new (std::nothrow) hello_msg_session_t (io_thread_, active_,
...
zmq::session_base_t::session_base_t (class io_thread_t *io_thread_,
                                     ...
                                     address_t *addr_)
    : own_t (io_thread_, options_), io_object_t (io_thread_),
      _active (active_), _pipe (NULL), _zap_pipe (NULL),
      _incomplete_in (false), _pending (false), _engine (NULL),
      _socket (socket_), _io_thread (io_thread_), _has_linger_timer (false),
      _addr (addr_)
#ifdef ZMQ_HAVE_WSS
      ,
...
void
zmq::session_base_t::attach_pipe (pipe_t *pipe_)
{
...
int
zmq::session_base_t::pull_msg (msg_t *msg_)
{
...
int
zmq::session_base_t::push_msg (msg_t *msg_)
{
...
int
zmq::session_base_t::read_zap_msg (msg_t *msg_)
{
  if (_zap_pipe == NULL)
    {
      errno = ENOTCONN;
...
int
zmq::session_base_t::write_zap_msg (msg_t *msg_)
{
...
void
zmq::session_base_t::pipe_terminated (pipe_t *pipe_)
{
...
void
zmq::session_base_t::read_activated (pipe_t *pipe_)
{
...
void
zmq::session_base_t::write_activated (pipe_t *pipe_)
{
  // Skip activating if we're detaching this pipe
  if (_pipe != pipe_)
    {
...
void
zmq::session_base_t::hiccuped (pipe_t *)
{
...
//  is not setup (IE: inproc://zeromq.zap.01 does not exist in the same
//  context) or it aborts on any other error. In other words, either ZAP is not
...
void
zmq::session_base_t::process_attach (i_engine *engine_)
{
  ...
  if (!_pipe && !is_terminating ())
    {
      ...
      int hwms[2]
          = { conflate ? -1 : options.rcvhwm, conflate ? -1 : options.sndhwm };
...
void
zmq::session_base_t::engine_error (bool handshaked_,
  ...
  if (_pipe)
    {
      ...
      //  Only send hiccup message if socket was connected and handshake was
      //  completed
...
void
zmq::session_base_t::process_term (int linger_)
{
  zmq_assert (!_pending);
  ...
  if (_pipe != NULL)
    {
      ...
      //  messages are processed in case the linger time is non-zero.
...
void
zmq::session_base_t::timer_event (int id_)
{
...
void
zmq::session_base_t::start_connecting (bool wait_)
{
  zmq_assert (_active);
...
int
zmq::hello_msg_session_t::pull_msg (msg_t *msg_)
{
  if (_new_pipe)
    {
      _new_pipe = false;

      const int rc = msg_->init_buffer (&options.hello_msg[0],
