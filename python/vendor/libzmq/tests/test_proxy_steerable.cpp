...
client_task (void *db_)
{
  ...
  zmq_pollitem_t items[]
      = { { client, 0, ZMQ_POLLIN, 0 }, { control, 0, ZMQ_POLLIN, 0 } };
  ...
  while (run)
    {
      ...
      // if we don't poll, we have to use ZMQ_DONTWAIT, if we poll, we can
      // block-receive with 0
...
void
steer (void *proxy_control, const char *command, const char *runctx)
{
  if (is_verbose)
    {
