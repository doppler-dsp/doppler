...
zmq::gssapi_client_t::gssapi_client_t (session_base_t *session_,
    ...
    : mechanism_base_t (session_, options_),
      gssapi_mechanism_base_t (session_, options_), state (call_next_init),
      token_ptr (GSS_C_NO_BUFFER), mechs (),
...
int
zmq::gssapi_client_t::next_handshake_command (msg_t *msg_)
{
  if (state == send_ready)
    {
...
int
zmq::gssapi_client_t::process_handshake_command (msg_t *msg_)
{
  if (state == recv_ready)
    {
...
int
zmq::gssapi_client_t::encode (msg_t *msg_)
{
...
int
zmq::gssapi_client_t::decode (msg_t *msg_)
{
  zmq_assert (state == connected);
...
zmq::gssapi_client_t::status () const
{
  return state == connected ? mechanism_t::ready : mechanism_t::handshaking;
}

int
zmq::gssapi_client_t::initialize_context ()
{
  ...
  if (target_name == GSS_C_NO_NAME)
    {
      ...
      OM_uint32 maj = gss_import_name (&min_stat, &send_tok, service_name_type,
                                       &target_name);
...
int
zmq::gssapi_client_t::produce_next_token (msg_t *msg_)
{
...
int
zmq::gssapi_client_t::process_next_token (msg_t *msg_)
{
