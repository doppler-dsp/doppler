...
zmq::stream_engine_base_t::stream_engine_base_t (
    fd_t fd_, const options_t &options_,
    const endpoint_uri_pair_t &endpoint_uri_pair_, bool has_handshake_stage_)
    : _options (options_), _inpos (NULL), _insize (0), _decoder (NULL),
      _outpos (NULL), _outsize (0), _encoder (NULL), _mechanism (NULL),
      _next_msg (NULL), _process_msg (NULL), _metadata (NULL),
      _input_stopped (false), _output_stopped (false),
      _endpoint_uri_pair (endpoint_uri_pair_), _has_handshake_timer (false),
      _has_ttl_timer (false), _has_timeout_timer (false),
      _has_heartbeat_timer (false), _peer_address (get_peer_address (fd_)),
      _s (fd_), _handle (static_cast<handle_t> (NULL)), _plugged (false),
      _handshaking (true), _io_error (false), _session (NULL), _socket (NULL),
...
void
zmq::stream_engine_base_t::plug (io_thread_t *io_thread_,
...
int
zmq::stream_engine_base_t::next_handshake_command (msg_t *msg_)
{
...
int
zmq::stream_engine_base_t::process_handshake_command (msg_t *msg_)
{
  zmq_assert (_mechanism != NULL);
...
int
zmq::stream_engine_base_t::write_credential (msg_t *msg_)
{
  zmq_assert (_mechanism != NULL);
  zmq_assert (_session != NULL);
...
int
zmq::stream_engine_base_t::pull_and_encode (msg_t *msg_)
{
  zmq_assert (_mechanism != NULL);
...
int
zmq::stream_engine_base_t::decode_and_push (msg_t *msg_)
{
  zmq_assert (_mechanism != NULL);
...
int
zmq::stream_engine_base_t::push_one_then_decode_and_push (msg_t *msg_)
{
...
int
zmq::stream_engine_base_t::pull_msg_from_session (msg_t *msg_)
{
  return _session->pull_msg (msg_);
}

int
zmq::stream_engine_base_t::push_msg_to_session (msg_t *msg_)
{
  return _session->push_msg (msg_);
}

void
zmq::stream_engine_base_t::error (error_reason_t reason_)
{
  zmq_assert (_session);
  ...
  // protocol errors have been signaled already at the point where they
  // occurred
  if (reason_ != protocol_error
          ...
          || _mechanism->status () == mechanism_t::handshaking))
    {
      ...
      // special case: connecting to non-ZMTP process which immediately drops
      // connection, or which never responds with greeting, should be treated
      // as a protocol error (i.e. stop reconnect)
...
bool
zmq::stream_engine_base_t::init_properties (properties_t &properties_)
{
...
void
zmq::stream_engine_base_t::timer_event (int id_)
{
...
int
zmq::stream_engine_base_t::read (void *data_, size_t size_)
{
...
int
zmq::stream_engine_base_t::write (const void *data_, size_t size_)
{
