...
## BurstDemod — full pipeline, and its ceiling

`BurstDemod` is a **one-shot feedforward** design: estimate `(freq, rate)`
once from the preamble, dechirp once, despread, frame-sync, check the
CRC-16. No tracking loop:
...
At this example's scale it fails the CRC on more than half the bursts — not
a bug, a real ceiling. See below.
...
- **`BurstDespreader.snr_est` is numerically unstable once the Costas loop
    is well locked on BPSK.** It's an EMA of `Re(prompt)^2 / Im(prompt)^2`;
    a locked BPSK prompt has `Im -> 0`, so the ratio can spike to absurd
    values — this demo observed everything from single digits up to `6.9e6`
    "dB" across otherwise-healthy bursts. Treat it as a rough lock-quality
    signal, not a calibrated SNR in dB.
- **`BurstDemod`'s one-shot feedforward estimate has a real payload-length
    ceiling.** At this demo's scale (1000-symbol payload, 5x512-chip
    preamble, Es/N0=10dB) a residual few-Hz Doppler-estimate error
    accumulates enough uncorrected phase drift across the frame to fail the
    CRC on more than half of runs. Raising `est_segments` from 10 to 200
    barely moves the pass rate (verified: 1-2/10 either way) — the
    bottleneck is the preamble's total coherent duration, not the
    estimator's segmentation, so more `est_segments` can't buy back
    precision the preamble doesn't have. **Practical takeaway:** match the
    receiver to the payload length — feedforward `BurstDemod` for short
    frames (tens to a few hundred symbols, as in its own test suite), the
    continuously-tracking `BurstDespreader` for long ones.
