...
class Func:
    ...
    def returns_complex(self) -> bool:
        return "double complex" in self.ret and "*" not in self.ret
...
CatParam = ArrayIn | ArrayOut | ScalarP | StringP | ComplexP | UnknownP
...
def _parse_one_param(s: str) -> Param:
    ...
    if is_ptr:
        ...
        base = " ".join(words[:-1]) if len(words) > 1 else words[0]
...
def parse_header(
    ...
    text = _strip_pp(text)
    text = re.sub(r"__attribute__\s*\(\s*\([^)]*\)\s*\)", "", text)
    ...
    for m in re.finditer(r"\b(dp_\w+)\s*\(", flat):
        ...
        if not ret or ret in ("typedef", "struct", "enum"):
            continue
        funcs.append(Func(ret, fname, _parse_params(params_str)))
...
def analyse_params(
    ...
    for i, p in enumerate(rest):
        ...
        if not p.is_ptr and p.base_type in SCALAR_FMT:
            result.append(ScalarP(p.name, SCALAR_FMT[p.base_type], p.base_type))
            continue
...
class Gen:
    ...
    def generate(
        ...
        for f in list(free) + [
            m for ht in hts.values() for m in ht.constructors + ht.methods
        ]:
            cats, _ = analyse_params(f.params)
            if any(isinstance(c, (ArrayIn, ArrayOut)) for c in cats):
                self._numpy = True
            if any(isinstance(c, ComplexP) for c in cats) or f.returns_complex:
                self._complex = True
    ...
    def _banner(self) -> None:
        ...
        self.w(
            "/*",
            f" * dp_{m}.c — Python C extension for dp/{m}.h",
            " *",
            " * AUTO-GENERATED by tools/gen_pyext.py — DO NOT EDIT.",
            " * Regenerate:",
            f" *   python tools/gen_pyext.py c/include/dp/{m}.h",
            " */",
            "",
        )

    def _includes(self) -> None:
        self.w("#define PY_SSIZE_T_CLEAN", "#include <Python.h>")
        if self._numpy:
            self.w(
    ...
    def _handle_class(self, ht: HandleType) -> None:
        ...
        if ht.destructor:
            ...
            self.w(
                "static PyObject *",
                f"{C}_{mn} ({C}Object *self, PyObject *Py_UNUSED (ignored))",
                "{",
                "  if (self->handle)",
                "    {",
                f"      {D} (self->handle);",
        ...
        self.w(
            "static PyObject *",
            f"{C}_enter ({C}Object *self, PyObject *Py_UNUSED (ignored))",
            "{",
            "  Py_INCREF (self);",
            "  return (PyObject *)self;",
            "}",
        ...
        # type object
        init_ln = f"    .tp_init = (initproc){C}_init," if ht.constructors else ""
        self.w(
    ...
    def _gen_init(self, ht: HandleType, ctor: Func) -> None:
        ...
        kw, fmt, decls, refs, pre, post, call = self._ctor_args(cats)
        ...
        self.w(
            ...
            *decls,
            ...
            f'          args, kwds, "{fmts}", kwlist, {refstr}))',
            "    return -1;",
            *pre,
            "",
    ...
    def _ctor_args(
        ...
        for c in cats:
            if isinstance(c, ArrayIn):
                kw.append(c.name)
                fmt.append("O")
                decls.append(f"  PyObject *{c.name}_obj = NULL;")
                refs.append(f"&{c.name}_obj")
                pre += self._array_conv(c, c.name, ret_err=ret_err)
                call.append(f"({c.cast}) PyArray_DATA ({c.name}_arr)")
                call.append(f"(size_t) {c.name}_n")
                post.append(f"  Py_DECREF ({c.name}_arr);")
                ...
                kw.append(c.name)
                fmt.append("z")
                decls.append(f"  const char *{c.name} = NULL;")
                refs.append(f"&{c.name}")
                call.append(c.name)
            else:
    ...
    def _array_conv(
        ...
        lines = [
            "",
            f"  PyArrayObject *{nm}_arr = (PyArrayObject *)PyArray_FROM_OTF (",
            f"      {nm}_obj, {c.npy}, NPY_ARRAY_C_CONTIGUOUS);",
            f"  if (!{nm}_arr)",
            f"    return {ret_err};",
        ]
        if c.factor == 2:
            lines += [
                ...
                f"      return {ret_err};",
                "    }",
                f"  Py_ssize_t {nm}_n = PyArray_SIZE ({nm}_arr) / 2;",
            ]
        else:
            lines.append(f"  Py_ssize_t {nm}_n = PyArray_SIZE ({nm}_arr);")
        return lines
    ...
    def _gen_classmethod_ctor(
        ...
        cats, _ = analyse_params(ctor.params)
        kw, fmt, decls, refs, pre, post, call = self._ctor_args(cats, ret_err="NULL")
        fmts = "".join(fmt)
        refstr = ", ".join(refs)
        callstr = ",\n      ".join(call)
        dfn = ht.destructor.name if ht.destructor else "free"
        ...
        self.w(
            ...
            *decls,
            "",
            f'  if (!PyArg_ParseTuple (args, "{fmts}", {refstr}))',
            "    return NULL;",
            *pre,
            "",
    ...
    def _gen_void_array_exec(
        ...
        if ins:
            ...
            lines += [
                ...
                f"      in_obj, {inp.npy}, NPY_ARRAY_C_CONTIGUOUS);",
                "  if (!in_arr)",
                "    return NULL;",
        ...
        for k, out in enumerate(outs):
            ...
            for j in range(k):
                cleanup.append(f"      Py_DECREF ({_prev_arr(j)});")
            lines += [
                "",
                f"  npy_intp dims{k}[] = {{{od}}};",
                f"  PyObject *{av}",
                f"      = PyArray_SimpleNew (1, dims{k}, {out.npy});",
                f"  if (!{av})",
                "    {",
                *cleanup,
                "      return NULL;",
        ...
        cargs = ["self->handle"]
        if ins:
            cargs.append(f"({ins[0].cast}) PyArray_DATA (in_arr)")
        for k, out in enumerate(outs):
            av = _prev_arr(k)
            cargs.append(f"({out.cast}) PyArray_DATA ((PyArrayObject *){av})")
        ...
        if multi:
            ...
            lines += [
                "",
                f"  PyObject *result = PyTuple_Pack ({len(outs)}, {pack});",
            ]
    ...
    def _gen_array_exec(
        ...
        self.w(
            ...
            *sz,
            ...
            f"      ({out.cast}) PyArray_DATA ((PyArrayObject *)out_arr),",
            "      (size_t) n);",
            "  Py_DECREF (in_arr);",
            "",
            "  if (rc != DP_OK)",
    ...
    def _gen_scalar_method(
        ...
        has_args = bool(fmts)
        arg_d = "PyObject *args" if has_args else "PyObject *Py_UNUSED (ignored)"
        fmt_s = "".join(fmts)
        ref_s = ", ".join(refs)
        ca = ", ".join(cargs)
        ...
        if has_args:
            self.w(
                f'  if (!PyArg_ParseTuple (args, "{fmt_s}", {ref_s}))',
                "    return NULL;",
            )
        self.w("")
    ...
    def _emit_call_and_return(
        ...
        if func.returns_void:
            self.w(f"  {func.name} ({cargs});", "  Py_RETURN_NONE;")
            ...
            self.w(f"  /* TODO: return */ {func.name} ({cargs});", "  Py_RETURN_NONE;")
    ...
    def _gen_method_table(self, ht: HandleType) -> None:
        ...
        for ctor in ht.constructors[1:]:
            mn = _mname(ctor.name, self.mod)
            entries.append(f'  {{"{mn}", (PyCFunction){C}_{mn},')
            entries.append("   METH_VARARGS | METH_CLASS,")
            entries.append(f'   "{ctor.name}."}},')

        for meth in ht.methods:
            mn = _mname(meth.name, self.mod)
            cats, _ = analyse_params(meth.params, skip_first=True)
        ...
        entries += [
            f'  {{"__enter__", (PyCFunction){C}_enter,',
            "   METH_NOARGS, NULL},",
            f'  {{"__exit__", (PyCFunction){C}_exit,',
            "   METH_VARARGS, NULL},",
            "  {NULL}",
        ]

        self.w(f"static PyMethodDef {C}_methods[] = {{", *entries, "};", "")
    ...
    def _gen_free_func(self, func: Func) -> None:
        ...
        if func.returns_complex and all(isinstance(c, ComplexP) for c in cats):
    ...
    def _gen_complex_func(
        ...
        for c in cats:
            ...
            conv.append(
                f"  double complex {c.name} = py_{c.name}.real + py_{c.name}.imag * I;"
            )
            ca.append(c.name)
        fmt_s = "".join(fmts)
        ref_s = ", ".join(refs)
        ...
        self.w(
            ...
            *conv,
            ...
            "  return PyComplex_FromDoubles (creal (rv), cimag (rv));",
            "}",
            "",
        )
    ...
    def _gen_free_array(
        ...
        self.w(
            ...
            *sz,
            "",
            f"  npy_intp dims[] = {{{od}}};",
            f"  PyObject *out_arr = PyArray_SimpleNew (1, dims, {out.npy});",
            "  if (!out_arr)",
            "    {",
            "      Py_DECREF (in_arr);",
            "      return NULL;",
    ...
    def _module_def(
        ...
        if free:
            ...
            entries.append("  {NULL}")
            self.w("static PyMethodDef module_methods[] = {", *entries, "};", "")
            mm = "module_methods"
        else:
            mm = "NULL"
        ...
        self.w(
            ...
            f'    .m_name = "dp_{m}",',
            f'    .m_doc = "Python binding for dp/{m}.h (auto-generated).",',
            "    .m_size = -1,",
            f"    .m_methods = {mm},",
            "};",
            "",
    ...
    def _init_func(
        ...
        m = self.mod
        self.w("PyMODINIT_FUNC", f"PyInit_dp_{m} (void)", "{")
        if self._numpy:
            self.w("  import_array ();")
        for ht in hts.values():
            C = ht.py_class
            self.w(f"  if (PyType_Ready (&{C}Type) < 0)", "    return NULL;")
        self.w(
            "",
            f"  PyObject *m = PyModule_Create (&dp_{m}_module);",
            "  if (!m)",
            "    return NULL;",
        )
        for ht in hts.values():
...
def main() -> None:
    ...
    ap.add_argument("input", type=Path, help="C source (.c) or header (.h)")
    ap.add_argument(
        "--dry-run", action="store_true", help="Print to stdout instead of writing file"
    )
    ap.add_argument(
        "-o", "--output", type=Path, default=None, help="Override output path"
    )
    args = ap.parse_args()
    ...
    if args.dry_run:
        ...
        if hts:
            print(f"  classes: {', '.join(h.py_class for h in hts.values())}")
        if free:
