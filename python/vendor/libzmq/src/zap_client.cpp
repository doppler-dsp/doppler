...
void
zap_client_t::send_zap_request (const char *mechanism_,
...
int
zap_client_t::receive_and_process_zap_reply ()
{
  int rc = 0;
...
zap_client_common_handshake_t::zap_client_common_handshake_t (
    session_base_t *const session_, const std::string &peer_address_,
    const options_t &options_, state_t zap_reply_ok_state_)
    : mechanism_base_t (session_, options_),
      zap_client_t (session_, peer_address_, options_),
      state (waiting_for_hello), _zap_reply_ok_state (zap_reply_ok_state_)
{
}

zmq::mechanism_t::status_t
zap_client_common_handshake_t::status () const
{
  if (state == ready)
...
int
zap_client_common_handshake_t::zap_msg_available ()
{
...
void
zap_client_common_handshake_t::handle_zap_status_code ()
{
  zap_client_t::handle_zap_status_code ();
...
int
zap_client_common_handshake_t::receive_and_process_zap_reply ()
{
  zmq_assert (state == waiting_for_zap_reply);
