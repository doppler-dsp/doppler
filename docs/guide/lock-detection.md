...
| [`Despreader`](../gallery/despreader.md)      | forwards to its embedded `Dll` (code) and `Costas` (carrier)                                     | `configure_code_lock(pfa, n_looks, ref_snr_db=0.0)` (derived) / `configure_carrier_lock(up_thresh, down_thresh, n_up, n_down)` (raw) | `.code_locked` / `.carrier_locked` |
