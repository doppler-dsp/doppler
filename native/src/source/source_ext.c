...
{
    import_array();
    if (PyType_Ready(&NCOType) < 0) return NULL;
    if (PyType_Ready(&LOType) < 0) return NULL;
    PyObject *m = PyModule_Create(&source_moduledef);
    if (!m) return NULL;
    Py_INCREF(&NCOType);
    if (PyModule_AddObject(m, "NCO", (PyObject *)&NCOType) < 0) {
        Py_DECREF(&NCOType); Py_DECREF(m); return NULL;
    }
    Py_INCREF(&LOType);
    if (PyModule_AddObject(m, "LO", (PyObject *)&LOType) < 0) {
        Py_DECREF(&LOType); Py_DECREF(m); return NULL;
    }
    return m;
}
