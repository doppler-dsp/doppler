...
void
test_push_round_robin_out (const char *bind_address_)
{
...
// strategy.
void
test_pull_fair_queue_in (const char *bind_address_)
{
...
// available peers.
void
test_push_block_on_send_no_peers (const char *bind_address_)
{
...
void
test_destroy_queue_on_disconnect (const char *bind_address_)
{
...
void
test_push_multipart_atomic_drop (const char *bind_address_, const bool block_)
{
  int linger = 0;
  int hwm = 1;
  ...
  if (block_)
    {
      TEST_ASSERT_EQUAL_INT (len, zmq_msg_send (&msg_data, push, ZMQ_SNDMORE));
    }
  else
    {
  ...
  // frame of "3".
  char buffer[2];
  rc = TEST_ASSERT_SUCCESS_ERRNO (zmq_recv (pull, buffer, sizeof (buffer), 0));
