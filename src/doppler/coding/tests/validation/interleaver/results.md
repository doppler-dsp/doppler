...
- **F7 · GAP** — **The view's runtime docstring is hand-written and can drift.** jm derives a class docstring from `create()`'s doxygen, but a VIEW's `tp_doc` gets a placeholder rather than its own `create_fn`'s, so `Deinterleaver`'s runtime doc is maintained by hand in the sacred fragment while the `.pyi` stub's IS derived — two faces of one class, from two sources (just-makeit#1160). The two agree today (THEY DO NOT, derived from the stub and the live class rather than asserted), and nothing keeps them agreeing. Retire the hand-written block when the fix ships.
