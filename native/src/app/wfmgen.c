 ...
 * (raw/csv/BLUE/SigMF, `--file-type`) in any wire type / byte order, streams to
 * a file, stdout, or a ZMQ PUB endpoint (`--output zmq://…`), and writes a JSON
 * record of exactly what it produced (`--record`). All of it is thin glue over
 * the C cores in the wfmcompose c_dep — wfm_compose / wfm_writer / wfm_sink —
 * which is why this lives by hand rather than via `jm app` (a composer is not a
...
#define CHOICE(dst, tbl)                                                    \
    ...
    if (out_path && !strncmp(out_path, "zmq://", 6)) {
        ...
        if (sigmf && rc == 0) {
            char *meta =
                wfm_sigmf_meta_json(sample_type, endian, fs, fc, segs, n_segs);
            if (meta) {
                char meta_path[1024];
