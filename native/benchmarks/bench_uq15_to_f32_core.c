...
main(void)
{
    uint16_t *in  = malloc(BENCH_N * sizeof(uint16_t));
    if (!in) { fprintf(stderr, "OOM\n"); return 1; }
    float *out = malloc(BENCH_N * sizeof(float));
    if (!out) { fprintf(stderr, "OOM\n"); return 1; }
    for (int i = 0; i < BENCH_N; i++) in[i] = (uint16_t)(i);
    ...
    /* warmup */
    for (int i = 0; i < 16; i++) _sink = uq15_to_f32_step(obj, in[i]);

    struct timespec t0, t1;
    jm_bench_t _bench = {0};

    printf("=== uq15_to_f32 benchmark ===\n");
    ...
    for (int r = 0; r < ITERATIONS; r++) {
        ...
        for (int i = 0; i < BENCH_N; i++)
            _sink = uq15_to_f32_step(obj, in[i]);
    ...
    for (int r = 0; r < ITERATIONS; r++) {
        clock_gettime(CLOCK_MONOTONIC, &t0);
        uq15_to_f32_steps(obj, in, out, BENCH_N);
        clock_gettime(CLOCK_MONOTONIC, &t1);
    ...
    jm_bench_write_json(&_bench, "uq15_to_f32");
