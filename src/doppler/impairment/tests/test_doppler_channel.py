...
# array instead. Two properties carry the whole design: a flat profile must
# reproduce the scalar route (the profile is ABSOLUTE, not a correction), and
# the array must express a geometry no (d0, d_dot) can.


def test_flat_profile_reproduces_the_scalar_route() -> None:
    """A constant profile is the scalar Doppler, by another route.

    Not asserted bit-exact: the two paths reach the resampler's rate as
    ``base + 0`` and as ``1 + (base - 1)``, the same number by different
    floating-point sums.
    """
    x = _dc()
    scalar = DopplerChannel(fs=FS, carrier_hz=FC, doppler_ppm=PPM)
    profile = DopplerChannel(fs=FS, carrier_hz=FC)

    ya = scalar.execute(x)
    yb = profile.execute_profile(x, np.full(N, PPM))

    assert len(ya) == len(yb)
    worst = np.abs(ya.astype(complex) - yb.astype(complex)).max()
    assert worst < 1e-4, f"flat profile is not the scalar route ({worst:.2e})"
...
def test_profile_is_absolute_not_a_correction() -> None:
    ...
    assert len(ya) == len(yb)
    worst = np.abs(ya.astype(complex) - yb.astype(complex)).max()
    assert worst < 1e-4, f"the create-time scalar leaked in ({worst:.2e})"
...
def test_profile_resumes_bit_exact_across_a_split() -> None:
    """The accumulated excess IS the carrier phase in profile mode, so a blob
    that dropped it would resume at zero excess and step the phase."""
    n = 8192
    x = _dc(n)
    prof = 25.0 * np.cos(np.pi * np.arange(n) / n)
...
def test_profile_length_must_match_the_waveform() -> None:
    ...
    no channel to turn that into a raise (``error_negative`` needs a signed
    int return, which an array-returning method does not have). Empty is
    blunt but it is not silent — nothing downstream mistakes it for a
    capture. Tracked as doppler#938.
    """
    x = _dc()
    ch = DopplerChannel(fs=FS, carrier_hz=FC)
