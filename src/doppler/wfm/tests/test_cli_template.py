...
construction, so these tests confirm it both *parses as JSON* and *round-trips*:
feeding the dumped template back through `--from-file` must compose without
...
def test_json_template_roundtrips_through_from_file(tmp_path):
    # Dump the template, then feed it straight back: it must compose and emit IQ
