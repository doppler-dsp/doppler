...
main (void)
{
  ...
  DP_CHECK (f32_buffer_get_capacity (ab) == cap);
  DP_CHECK (f32_buffer_get_available (ab) == cap);
  DP_CHECK (f32_buffer_get_space (ab) == 0);
  DP_CHECK (f32_buffer_get_dropped (ab) == cap + 1);
  DP_CHECK (!f32_buffer_get_closed (ab));
  dp_f32_close (ab);
  DP_CHECK (f32_buffer_get_closed (ab));
