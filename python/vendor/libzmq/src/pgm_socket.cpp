...
zmq::pgm_socket_t::pgm_socket_t (bool receiver_, const options_t &options_)
    : sock (NULL), options (options_), receiver (receiver_), pgm_msgv (NULL),
      pgm_msgv_len (0), nbytes_rec (0), nbytes_processed (0),
...
//       ;[fe80::1%en0]:7500
int
zmq::pgm_socket_t::init_address (const char *network_,
...
int
zmq::pgm_socket_t::init (bool udp_encapsulation_, const char *network_)
{
...
void
zmq::pgm_socket_t::get_receiver_fds (fd_t *receive_fd_, fd_t *waiting_pipe_fd_)
{
  socklen_t socklen;
  bool rc;
...
void
zmq::pgm_socket_t::get_sender_fds (fd_t *send_fd_, fd_t *receive_fd_,
                                   ...
                                   fd_t *pending_notify_fd_)
{
  ...
  rc = pgm_getsockopt (sock, IPPROTO_PGM, PGM_PENDING_SOCK, pending_notify_fd_,
                       &socklen);
  zmq_assert (rc);
...
size_t
zmq::pgm_socket_t::send (unsigned char *data_, size_t data_len_)
{
  size_t nbytes = 0;
...
ssize_t
zmq::pgm_socket_t::receive (void **raw_data_, const pgm_tsi_t **tsi_)
{
...
int
zmq::pgm_socket_t::compute_sqns (int tpdu_)
{
