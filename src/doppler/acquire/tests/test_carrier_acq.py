...
def test_create():
    ca = CarrierAcquisition(SAMPLE_RATE_HZ, SYMBOL_RATE_HZ)
...
def test_detects_tone(sequential):
    ...
    ca = CarrierAcquisition(
        SAMPLE_RATE_HZ, SYMBOL_RATE_HZ, sequential=sequential
    )
    ca.steps(x)
    assert ca.ready
...
def test_template_override_runs_to_completion():
    probe = CarrierAcquisition(SAMPLE_RATE_HZ, SYMBOL_RATE_HZ)
    nfft = probe.nfft
    tmpl = np.zeros(nfft, dtype=np.float32)
    tmpl[nfft // 2] = 1.0  # a DC spike, deliberately not the default sinc^2

    x = _make_signal()
    ca = CarrierAcquisition(SAMPLE_RATE_HZ, SYMBOL_RATE_HZ, psd_template=tmpl)
    assert ca.nfft == nfft
    ca.steps(x)
    assert ca.n_blocks > 0
...
def test_template_wrong_length_rejected():
    with pytest.raises(MemoryError):
        CarrierAcquisition(
            SAMPLE_RATE_HZ,
            SYMBOL_RATE_HZ,
            psd_template=np.zeros(3, dtype=np.float32),
        )


def test_invalid_sample_rate_rejected():
    with pytest.raises(MemoryError):
        CarrierAcquisition(0.0, SYMBOL_RATE_HZ)


def test_context_manager():
    with CarrierAcquisition(SAMPLE_RATE_HZ, SYMBOL_RATE_HZ) as ca:
