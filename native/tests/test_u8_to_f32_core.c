...
{
  /* 1. mode validation */
  DP_CHECK (dp_u8_to_f32_create (-1) == NULL);
  DP_CHECK (dp_u8_to_f32_create (2) == NULL);

  dp_u8_to_f32_state_t *sh = dp_u8_to_f32_create (U8_TO_F32_SHIFT);
  dp_u8_to_f32_state_t *mp = dp_u8_to_f32_create (U8_TO_F32_MIDPOINT);
  ...
  for (int x = 0; x < 256; x++)
    {
      ...
      if (dp_u8_to_f32_step (sh, (uint8_t)x) != want)
        shift_bad++;
    }
  DP_CHECK_MSG (shift_bad == 0, "shift != (x - 128)/128 exactly");
  DP_CHECK (dp_u8_to_f32_step (sh, 0) == -1.0f);
  DP_CHECK (dp_u8_to_f32_step (sh, 128) == 0.0f);
  DP_CHECK (dp_u8_to_f32_step (sh, 255) == 127.0f / 128.0f);

  /* 3. the shift DC bias, measured */
  double zero_mean
      = 0.5 * (dp_u8_to_f32_step (sh, 127) + dp_u8_to_f32_step (sh, 128));
  DP_CHECK_NEAR (zero_mean, -0.5 / 128.0, 0.0);
  double all_mean = 0.0;
  for (int x = 0; x < 256; x++)
    all_mean += dp_u8_to_f32_step (sh, (uint8_t)x);
  all_mean /= 256.0;
  ...
  for (int x = 0; x < 256; x++)
    {
      ...
      double err  = fabs ((double)dp_u8_to_f32_step (mp, (uint8_t)x) - want);
      if (err > worst)
        worst = err;
    }
  ...
  for (int x = 0; x < 256; x++)
    {
      float y = dp_u8_to_f32_step (mp, (uint8_t)x);
      if (y != -dp_u8_to_f32_step (mp, (uint8_t)(255 - x)))
        anti_bad++;
      mp_mean += y;
    }
  DP_CHECK_MSG (anti_bad == 0, "midpoint not exactly antisymmetric");
  DP_CHECK_NEAR (mp_mean, 0.0, 0.0);
  DP_CHECK_NEAR (dp_u8_to_f32_step (mp, 127) + dp_u8_to_f32_step (mp, 128),
                 0.0, 0.0);

  /* 6. both rails */
  DP_CHECK_NEAR (dp_u8_to_f32_step (mp, 0), -1.0, FLT_EPSILON);
  DP_CHECK_NEAR (dp_u8_to_f32_step (mp, 255), 1.0, FLT_EPSILON);
  ...
  dp_u8_to_f32_state_t *objs[2] = { sh, mp };
  for (int k = 0; k < 2; k++)
    {
      dp_u8_to_f32_steps (objs[k], in, blk, NSTREAM);
      int blk_bad = 0;
      for (int i = 0; i < NSTREAM; i++)
        if (blk[i] != dp_u8_to_f32_step (objs[k], in[i]))
          blk_bad++;
      ...
      for (size_t c = 0; c < sizeof cut / sizeof cut[0]; c++)
        {
          dp_u8_to_f32_steps (objs[k], in + off, pcs + off, cut[c]);
          off += cut[c];
        }
  ...
  /* 8. reset is a no-op */
  float before = dp_u8_to_f32_step (mp, 200);
  dp_u8_to_f32_reset (mp);
  dp_u8_to_f32_reset (sh);
  DP_CHECK (dp_u8_to_f32_step (mp, 200) == before);
  DP_CHECK (dp_u8_to_f32_step (sh, 200) == 72.0f / 128.0f);

  dp_u8_to_f32_destroy (sh);
  dp_u8_to_f32_destroy (mp);
  dp_u8_to_f32_destroy (NULL);
