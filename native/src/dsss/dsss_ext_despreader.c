...
static PyMethodDef DespreaderObj_methods[] = {
  ...
  { "steps", (PyCFunction)(void *)DespreaderObj_steps,
    ...
    "carrier (Costas) and correlate early/prompt/late against the code\n"
    "(DLL), update both loops each code period, and emit one complex prompt\n"
    "symbol per period.\n"
    "\n"
    ...
    "integrate-and-dump, updates the code loop on the early/late envelopes\n"
    "and the carrier loop on the same prompt, and emits that prompt. A\n"
    "partial period is carried in state across calls, so a long stream can\n"
    "be fed in blocks. Each emitted symbol's sign is the BPSK decision; its\n"
    "phase and magnitude are the soft information.\n"
    "\n"
    "Parameters\n"
    "----------\n"
  ...
  { "bits", (PyCFunction)(void *)DespreaderObj_bits,
    ...
    "    Output buffer for hard bits, 0/1 (>= max_out).\n"
    "\n"
    "Returns\n"
    "-------\n"
    "NDArray[np.uint8]\n"
