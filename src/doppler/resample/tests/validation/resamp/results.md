...
- **Status.** **CERTIFIED** — all 14 limits hold.
- **Findings.** 9 recorded, none open.

**What a caller most needs to know**

- **`mu` is the diagnostic for a closed timing loop**, and it is now bound: steady `mu` means settled, a slewing or wrapping `mu` is a residual RATE error, and one wrap is one INPUT interval of slip (§2.5, F4). Poll it before believing a lock statistic.
- **The -6 dB at the output Nyquist is the folding rule at its own edge, not a stopband failure** (§2.4). Judge the filter inside the band you asked for, not at the seam.
- **`execute` and `execute_ctrl` differ by GROUP DELAY below unity, not by quality** (§2.6) — so a comparison between them measures alignment unless you account for it first.
- **Rate 1.0 was a trap and is fixed.** A private rounding cast reached past 2^32 and stalled the interpolator for a window just above unity — and zero Doppler IS rate 1.0, so any ramp through closest approach crossed it (F7).
