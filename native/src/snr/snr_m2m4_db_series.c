 ...
 *
 * Sliding-window wrapper: one dp_snr_m2m4_db() call per output index, over a
 * [i - window/2, i + window/2] span clamped at the edges.
 */
#include "doppler/snr/snr_core.h"

void
dp_snr_m2m4_db_series (const float _Complex *x, size_t x_len, size_t window,
                       double *out)
{
  ...
  for (size_t i = 0; i < x_len; i++)
    {
      ...
      out[i]    = dp_snr_m2m4_db (x + lo, hi - lo);
    }
}
