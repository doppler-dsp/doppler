...
| `build-c-linux` (x86_64 + aarch64) / `build-c-macos` | Standalone C library tarballs (`lib/` layout, not `lib64/`) via `make package-c-tarball`                         | Run `make package-c-tarball VERSION=x.y.z` locally; check `cmake --install` output       |
