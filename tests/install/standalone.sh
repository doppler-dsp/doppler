#!/usr/bin/env bash
# standalone.sh — build and run example-projects/standalone from the repo root.
# Doc snippets are tagged with --8<-- markers; the script is also
# directly runnable after `cmake --build build` completes.
set -euo pipefail

STANDALONE_DIR="example-projects/standalone"
BUILD_DIR="${1:-build}"                      # doppler build tree
OUT_DIR="example-projects/standalone/build"
...
gcc -o awgn_example example-projects/standalone/main.c \
    ...
    build/libdoppler.a -lm -lpthread
./awgn_example
# --8<-- [end:gcc-static]
