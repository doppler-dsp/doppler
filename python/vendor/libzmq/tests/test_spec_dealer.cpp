...
// strategy.
void
test_round_robin_out (const char *bind_address_)
{
  void *dealer = test_context_socket (ZMQ_DEALER);

  char connect_address[MAX_SOCKET_STRING];
  test_bind (dealer, bind_address_, connect_address, sizeof (connect_address));
...
// strategy.
void
test_fair_queue_in (const char *bind_address_)
{
  ...
  for (size_t peer = 0; peer < services; ++peer)
    {
      ...
      TEST_ASSERT_SUCCESS_ERRNO (zmq_connect (senders[peer], connect_address));
    }
...
// SHALL block on sending, or return a suitable error, when it has no connected
// peers.
void
test_block_on_send_no_peers (const char *bind_address_)
{
