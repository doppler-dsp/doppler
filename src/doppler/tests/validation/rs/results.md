...
- **Do not read `1/E!` off a textbook for a small field.** It is the large-field limit of the exact form and is 37 % high at RS(15,11), where the exact value is 0.364 (§2.2, F4).
...
Second, that model is exact and `1/E!` is not. The probability is the fraction of the space the decoding spheres fill, `V(E) / q^(n-k)`; `1/E!` is its large-field limit, reached by substituting `C(n,E) ~ n^E/E!` and `n ~ q`. The two agree to a couple of percent at `q = 256` and part company at `q = 16`, where RS(15,11) measures 0.361 against a model of 0.364 and a textbook 0.50 — the rule is **37 % high** on the small field (F4).
...
Silent failures on this channel: **0** in 1400 codewords. At `E = 16` the sphere model puts a miscorrection at 5e-14, so a run that produced one would be evidence of a defect rather than of bad luck (§2.2).
...
- **F4 · FIXED** — `test_rs_core.c`'s refusal gate carried its rationale as *with probability ~1/E! for random errors ... at E = 2 it is a coin toss*. `1/E!` is the large-field limit of the exact `V(E)/q^(n-k)`, and at RS(15,11) it is 37 % high: the miscorrection rate there is 0.361, against 0.50 from the rule and 0.364 from the exact form the measurement lands on (§2.2). The gate's own floor was never in danger — it sits at 32 of 64, which the exact model puts more than two standard deviations away — but the number a reader takes from that comment was wrong for the one configuration where the two models differ, and it is the configuration the comment used as its example. The comment now carries the exact form, both measurements, and why they diverge.
