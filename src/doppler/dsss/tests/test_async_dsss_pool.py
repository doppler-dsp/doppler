...
def _emitter(doppler_hz, delay, seed):
    """One emitter: async BPSK on the code at a fixed carrier offset,
    starting `delay` samples in, AWGN from CN0 (dp_dsss_capture's shape)."""
...
def test_create_defaults_and_read_back():
    ...
    assert p.coherent_bins == 1
    assert p.doppler_res_hz > 0.0
    r = p.status(0)
