...
void
zmq::ip_addr_t::set_port (uint16_t port_)
{
  if (family () == AF_INET6)
    {
...
zmq::ip_resolver_options_t::ip_resolver_options_t ()
    : _bindable_wanted (false), _nic_name_allowed (false),
      _ipv6_wanted (false), _port_expected (false), _dns_allowed (false),
      _path_allowed (false)
{
}
...
zmq::ip_resolver_options_t &
zmq::ip_resolver_options_t::ipv6 (bool ipv6_)
{
  _ipv6_wanted = ipv6_;
...
zmq::ip_resolver_options_t &
zmq::ip_resolver_options_t::allow_dns (bool allow_)
{
  _dns_allowed = allow_;
...
zmq::ip_resolver_options_t &
zmq::ip_resolver_options_t::allow_path (bool allow_)
{
...
int
zmq::ip_resolver_t::resolve (ip_addr_t *ip_addr_, const char *name_)
{
  std::string addr;
  uint16_t port;
...
int
zmq::ip_resolver_t::resolve_getaddrinfo (ip_addr_t *ip_addr_,
                                         const char *addr_)
{
...
#if defined AI_V4MAPPED
  // Some OS do have AI_V4MAPPED defined but it is not supported in
  // getaddrinfo() returning EAI_BADFLAGS. Detect this and retry
...
int
zmq::ip_resolver_t::resolve_nic_name (ip_addr_t *ip_addr_, const char *nic_)
{
  //  Create a socket.
...
int
zmq::ip_resolver_t::get_interface_name (unsigned long index_,
                                        char **dest_) const
{
...
int
zmq::ip_resolver_t::wchar_to_utf8 (const WCHAR *src_, char **dest_) const
{
  int rc;
      ...
      for (const IP_ADAPTER_ADDRESSES *current_addresses = addresses;
           current_addresses; current_addresses = current_addresses->Next)
        {
          ...
          if (((str_rc1 == 0) && (!strcmp (nic_, if_name)))
            ...
            {
              //  Iterate over all unicast addresses bound to the current
              //  network interface
...
int
zmq::ip_resolver_t::do_getaddrinfo (const char *node_, const char *service_,
...
void
zmq::ip_resolver_t::do_freeaddrinfo (struct addrinfo *res_)
{
  freeaddrinfo (res_);
}

unsigned int
zmq::ip_resolver_t::do_if_nametoindex (const char *ifname_)
{
