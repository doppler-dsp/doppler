...
static int
get_monitor_event (void *monitor_)
{
  for (int i = 0; i < 10; i++)
    {
...
static void
recv_with_retry (raw_socket fd_, char *buffer_, int bytes_)
{
  int received = 0;
  while (true)
    {
...
static void
mock_handshake (raw_socket fd_, int mock_ping_)
{
  char buffer[128];
  ...
  if (mock_ping_)
    {
      ...
      const uint8_t zmtp_ping[12]
          = { 4, 10, 4, 'P', 'I', 'N', 'G', 0, 0, 'L', 'O', 'L' };
      ...
      uint8_t zmtp_pooong[65]
          = { 4, 21, 4, 'P', 'O', 'N', 'G', 'L', 'O', 'L' };
...
static void
setup_curve (void *socket_, int is_server_)
{
...
static void
prep_server_socket (int set_heartbeats_, int is_curve_, void **server_out_,
                    void **mon_out_, char *endpoint_, size_t ep_length_,
                    int socket_type_)
{
...
static void
test_heartbeat_timeout (int server_type_, int mock_ping_)
{
  int rc;
...
// its thing correctly.
static void
test_heartbeat_ttl (int client_type_, int server_type_)
{
  int rc, value;
  char my_endpoint[MAX_SOCKET_STRING];
...
test_heartbeat_notimeout (int is_curve_, int client_type_, int server_type_)
{
  ...
  rc = get_monitor_event (server_mon);
  // TODO: this fails ~1% of the runs on OBS but it does not seem to be
...
void
test_setsockopt_heartbeat_success (const int value_)
{
...
test_setsockopt_heartbeat_ttl_more_than_max_fails ()
{
  ...
  TEST_ASSERT_FAILURE_ERRNO (EINVAL, zmq_setsockopt (socket, ZMQ_HEARTBEAT_TTL,
                                                     &value, sizeof (value)));
