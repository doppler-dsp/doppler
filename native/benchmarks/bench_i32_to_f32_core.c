...
main (void)
{
  int32_t *in = malloc (BENCH_N * sizeof (int32_t));
  if (!in)
    {
      fprintf (stderr, "OOM\n");
      return 1;
  ...
  /* warmup */
  for (int i = 0; i < 16; i++)
    _sink = dp_i32_to_f32_step (obj, in[i]);

  uint64_t   t0, t1;
  jm_bench_t _bench = { 0 };

  printf ("=== i32_to_f32 benchmark ===\n");
  ...
  for (int r = 0; r < ITERATIONS; r++)
    {
      t0 = jm_bench_now_ns ();
      dp_i32_to_f32_steps (obj, in, out, BENCH_N);
      t1              = jm_bench_now_ns ();
  ...
  jm_bench_write_json (&_bench, "i32_to_f32");
