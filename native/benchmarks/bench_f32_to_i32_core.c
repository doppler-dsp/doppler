...
main (void)
{
  ...
  int32_t *out = malloc (BENCH_N * sizeof (int32_t));
  if (!out)
    {
      fprintf (stderr, "OOM\n");
      free (in);
  ...
  f32_to_i32_state_t *obj = f32_to_i32_create (2147483648.0);
  ...
  /* warmup */
  for (int i = 0; i < 16; i++)
    _sink = f32_to_i32_step (obj, in[i]);

  struct timespec t0, t1;
  jm_bench_t      _bench = { 0 };

  printf ("=== f32_to_i32 benchmark ===\n");
  ...
  for (int r = 0; r < ITERATIONS; r++)
    {
      clock_gettime (CLOCK_MONOTONIC, &t0);
      f32_to_i32_steps (obj, in, out, BENCH_N);
      clock_gettime (CLOCK_MONOTONIC, &t1);
  ...
  jm_bench_write_json (&_bench, "f32_to_i32");
