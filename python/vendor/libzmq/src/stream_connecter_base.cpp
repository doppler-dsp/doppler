...
zmq::stream_connecter_base_t::stream_connecter_base_t (
    ...
    const zmq::options_t &options_, zmq::address_t *addr_, bool delayed_start_)
    : own_t (io_thread_, options_), io_object_t (io_thread_), _addr (addr_),
      _s (retired_fd), _handle (static_cast<handle_t> (NULL)),
      _socket (session_->get_socket ()), _delayed_start (delayed_start_),
      _reconnect_timer_started (false), _current_reconnect_ivl (-1),
      _session (session_)
{
  zmq_assert (_addr);
...
void
zmq::stream_connecter_base_t::process_term (int linger_)
{
...
{
  // TODO before, this was an assertion for _s != retired_fd, but this does not
  // match usage of close
  if (_s != retired_fd)
    {
...
void
zmq::stream_connecter_base_t::create_engine (fd_t fd_,
...
void
zmq::stream_connecter_base_t::timer_event (int id_)
{
