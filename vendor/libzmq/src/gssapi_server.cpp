...
zmq::gssapi_server_t::gssapi_server_t (session_base_t *session_,
    ...
    : mechanism_base_t (session_, options_),
      ...
      zap_client_t (session_, peer_address_, options_), session (session_),
      peer_address (peer_address_), state (recv_next_token),
...
int
zmq::gssapi_server_t::next_handshake_command (msg_t *msg_)
{
  if (state == send_ready)
    {
...
int
zmq::gssapi_server_t::process_handshake_command (msg_t *msg_)
{
  if (state == recv_ready)
    {
...
void
zmq::gssapi_server_t::send_zap_request ()
{
...
int
zmq::gssapi_server_t::encode (msg_t *msg_)
{
...
int
zmq::gssapi_server_t::decode (msg_t *msg_)
{
  zmq_assert (state == connected);
...
int
zmq::gssapi_server_t::zap_msg_available ()
{
...
zmq::gssapi_server_t::status () const
{
  return state == connected ? mechanism_t::ready : mechanism_t::handshaking;
}

int
zmq::gssapi_server_t::produce_next_token (msg_t *msg_)
{
...
int
zmq::gssapi_server_t::process_next_token (msg_t *msg_)
{
...
void
zmq::gssapi_server_t::accept_context ()
{
