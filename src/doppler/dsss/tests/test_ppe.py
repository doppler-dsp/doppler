...
def test_create_doppler_only():
    ...
    p = PolynomialPhaseEstimator(4096, 0.0)
...
def test_create_rate_search_grid():
    ...
    p = PolynomialPhaseEstimator(512, 5e-5)
...
def test_doppler_only_recovers_tone():
    ...
    n = 2048
    p = PolynomialPhaseEstimator(n, 0.0)
...
def test_joint_search_recovers_chirp(f, r):
    ...
    n = 512
    p = PolynomialPhaseEstimator(n, 5e-5)
...
def test_record_fields():
    e = PolynomialPhaseEstimator(512, 5e-5).estimate(_chirp(512, 0.05, 1e-5))
...
def test_out_of_range_length_is_zeroed():
    p = PolynomialPhaseEstimator(512, 0.0)
