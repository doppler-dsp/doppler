...
The published [Benchmarks](../benchmarks.md) page (`docs/benchmarks.md`) is a
**committed static table** of representative numbers, regenerated by hand — it
is deliberately *not* auto-generated from CI, because GitHub runners are shared
and non-deterministic (their absolute numbers aren't hardware-representative).

On a **representative machine** (a real dev box, not a CI runner), regenerate
and commit it so the release ships current numbers:

```sh
make bench          # run the suite on THIS machine (writes benchmarks/history/)
make bench-docs     # render docs/benchmarks.md, CPU stamped in the header
git add docs/benchmarks.md
git commit -m "docs: refresh benchmarks for vX.Y.Z (<cpu>)"
```

Skip only if no perf-relevant code changed since the last release.
