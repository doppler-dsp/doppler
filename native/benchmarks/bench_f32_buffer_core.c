/* bench_f32_buffer_core.c — the element-typed face of the f32 ring.
 *
...
main (void)
{
  ...
  for (int r = 0; r < ITERATIONS; r++)
    {
      ...
      for (int k = 0; k < BENCH_N / FRAME; k++)
        {
          dp_f32_write_some_view (ab, x, FRAME);
          float _Complex *v = dp_f32_peek_view (ab, FRAME);
          sink += (v != NULL);
          dp_f32_consume (ab, FRAME);
        }
  ...
  jm_bench_write_json (&_bench, "f32_buffer");
  dp_f32_destroy (ab);
  return sink == 0;
}
