...
int get_monitor_event_with_timeout (void *monitor_, int *value_,
                                    char **address_, int timeout_);
...
void print_unexpected_event_stderr (int event_, int err_, int expected_event_,
                                    int expected_err_);
...
//  interrupts, if a ZMQ_EVENT_HANDSHAKE_FAILED_NO_DETAIL with EPIPE,
//  ECONNRESET or ECONNABORTED occurs; in this case, 0 is returned this should
//  be investigated further, see https://github.com/zeromq/libzmq/issues/2644
int expect_monitor_event_multiple (void *server_mon_, int expected_event_,
                                   int expected_err_ = -1,
                                   bool optional_ = false);

int64_t get_monitor_event_v2 (void *monitor_, uint64_t **value_,
