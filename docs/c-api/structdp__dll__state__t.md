...
# Struct dp\_dll\_state\_t



[**ClassList**](annotated.md) **>** [**dp\_dll\_state\_t**](structdp__dll__state__t.md)
...
|  [**dp\_nco\_state\_t**](structdp__nco__state__t.md) | [**code\_nco**](#variable-code_nco)  <br> |
...
```C++
float _Complex dp_dll_state_t::acc_e;
```
...
```C++
float _Complex dp_dll_state_t::acc_l;
```
...
```C++
float _Complex dp_dll_state_t::acc_o;
```
...
```C++
float _Complex dp_dll_state_t::acc_p;
```
...
```C++
double dp_dll_state_t::aid_alpha;
```
...
```C++
size_t dp_dll_state_t::aid_best;
```
...
```C++
uint64_t dp_dll_state_t::aid_count;
```
...
```C++
uint64_t dp_dll_state_t::aid_last_end;
```
...
```C++
size_t dp_dll_state_t::aid_nhyp;
```
...
```C++
double* dp_dll_state_t::aid_power;
```
...
```C++
size_t dp_dll_state_t::aid_ring;
```
...
```C++
float _Complex* dp_dll_state_t::aid_ring_e;
```
...
```C++
float _Complex* dp_dll_state_t::aid_ring_l;
```
...
```C++
float _Complex* dp_dll_state_t::aid_ring_o;
```
...
```C++
float _Complex* dp_dll_state_t::aid_ring_p;
```
...
```C++
double dp_dll_state_t::chip_pos;
```
...
```C++
float _Complex* dp_dll_state_t::chunk_e;
```
...
```C++
float _Complex* dp_dll_state_t::chunk_l;
```
...
```C++
float _Complex* dp_dll_state_t::chunk_p;
```
...
```C++
const uint8_t* dp_dll_state_t::code;
```
...
```C++
dp_nco_state_t dp_dll_state_t::code_nco;
```
...
```C++
double dp_dll_state_t::code_rate;
```
...
```C++
int dp_dll_state_t::have_prev_epoch;
```
...
```C++
uint32_t dp_dll_state_t::held_inc;
```
...
```C++
dp_loop_filter_state_t dp_dll_state_t::held_lf;
```
...
```C++
double dp_dll_state_t::inv_tsamps;
```
...
```C++
double dp_dll_state_t::inv_tsamps2;
```
...
```C++
double dp_dll_state_t::inv_tsamps_sf;
```
...
```C++
float _Complex* dp_dll_state_t::last_backward_p;
```
...
```C++
float _Complex* dp_dll_state_t::last_e;
```
...
```C++
double dp_dll_state_t::last_error;
```
...
```C++
float _Complex* dp_dll_state_t::last_l;
```
...
```C++
dp_loop_filter_state_t dp_dll_state_t::lf;
```
...
```C++
dp_lockdet_state_t dp_dll_state_t::lock;
```
...
```C++
double dp_dll_state_t::lock_alpha;
```
...
```C++
size_t dp_dll_state_t::lock_count;
```
...
```C++
double dp_dll_state_t::lock_stat;
```
...
```C++
double dp_dll_state_t::lock_sum;
```
...
```C++
double dp_dll_state_t::noise_ema;
```
...
```C++
double dp_dll_state_t::noise_guard;
```
...
```C++
double dp_dll_state_t::off_chips;
```
...
```C++
int dp_dll_state_t::owns_code;
```



1 if [**dp\_dll\_destroy()**](dll__core_8h.md#function-dp_dll_destroy) frees `code`. 
...
```C++
double dp_dll_state_t::rate_aid;
```



carrier-aiding code-rate deviation (ratio, 0 = off): a fixed fractional bias summed into the sample-and-hold phase\_inc every epoch, on top of the loop's own ctrl. For physically-coupled Doppler, the caller sets this to carrier\_offset/carrier\_freq so the code NCO rides the code-rate dilation the code discriminator alone can't pull in at low SNR. See [**dp\_dll\_set\_rate\_aid()**](dll__core_8h.md#function-dp_dll_set_rate_aid). 
...
```C++
double dp_dll_state_t::seed_chip;
```
...
```C++
double dp_dll_state_t::seg_chips;
```
...
```C++
double dp_dll_state_t::seg_norm;
```
...
```C++
size_t dp_dll_state_t::segments;
```
...
```C++
float _Complex* dp_dll_state_t::sums;
```
...
```C++
double dp_dll_state_t::sym_period;
```
...
```C++
int dp_dll_state_t::wrap_pending;
```



1: a [**dp\_dll\_set\_code\_phase()**](dll__core_8h.md#function-dp_dll_set_code_phase) moved the phase FORWARD across the period wrap, so the wrap the epoch in progress was waiting for has in effect happened  the kernel closes that epoch on the next sample instead of folding a second period into it (#1287). 
