...
test_stream_2_stream ()
{
  ...
  TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (rconn1, ZMQ_CONNECT_ROUTING_ID,
  rconn1routing_id, strlen(rconn1routing_id))); TEST_ASSERT_SUCCESS_ERRNO
  (zmq_connect (rconn1, bindip));
*/
...
void
test_router_2_router (bool named_)
{
  char buff[256];
  const char msg[] = "hi 1";
  const int zero = 0;
...
*/
  ...
  //  If bound socket identity naming a problem, we'll likely see something
  //  funky here.
...
test_router_2_router_while_receiving ()
{
  ...
  // wait for the X->Z message to be received (so that our non-blocking check
  // will actually fail if the message is routed to Y)
  msleep (SETTLE_TIME);
