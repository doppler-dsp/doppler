...
static const char *const jm_choices_type[]
    = { "tone", "noise", "pn", "bpsk", "qpsk" };
static const char *const jm_choices_snr_mode[]
    = { "auto", "fs", "ebno", "esno" };
static const char *const jm_choices_lfsr[] = { "galois", "fibonacci" };
static const char *const jm_choices_sample_type[]
                ...
                int endian, int ftype, unsigned char *bytes)
{
  ...
  if (endian == 1)
    { /* big-endian: reverse each element's bytes */
  ...
  for (int i = 1; i < argc; i++)
    {
      if (!strcmp (argv[i], "--help") || !strcmp (argv[i], "-h"))
        {
          fputs (
              "usage: wavegen [--type tone|noise|pn|bpsk|qpsk] [--fs V] "
              "[--freq V] [--snr V] [--snr_mode auto|fs|ebno|esno] [--seed V] "
              "[--sps V] [--pn_length V] [--pn_poly V] [--lfsr "
              "galois|fibonacci] [--count V] [--sample_type "
              "cf32|cf64|ci32|ci16|ci8] [--file_type raw|csv] [--endian "
              "le|be] [--output FILE] [--record FILE]\n  --type "
              "tone|noise|pn|bpsk|qpsk\n  --fs V\n  --freq V\n  --snr V\n  "
              "--snr_mode auto|fs|ebno|esno\n  --seed V\n  --sps V\n  "
              "--pn_length V\n  --pn_poly V\n  --lfsr galois|fibonacci\n  "
              "--count V                 number of samples to generate\n  "
              "--sample_type cf32|cf64|ci32|ci16|ci8  output wire sample "
              "type\n  --file_type raw|csv       output container\n  --endian "
              "le|be            byte order (raw only)\n  --output, -o FILE    "
              "     output file (default: stdout)\n  --record FILE            "
              " write a JSON record of the resolved run\n",
              stdout);
          return 0;
        }
          ...
          if (type < 0)
            {
              fprintf (
                  stderr,
                  "error: --type must be one of: tone noise pn bpsk qpsk\n");
              return 2;
            }
        }
          ...
          if (snr_mode < 0)
            {
              fprintf (
                  stderr,
                  "error: --snr_mode must be one of: auto fs ebno esno\n");
              return 2;
            }
        }
      ...
      else if (!strcmp (argv[i], "--lfsr") && i + 1 < argc)
        {
          ...
          if (lfsr < 0)
            {
              fprintf (stderr,
                       "error: --lfsr must be one of: galois fibonacci\n");
              return 2;
            }
        }
      ...
      else if (!strcmp (argv[i], "--sample_type") && i + 1 < argc)
        {
          ...
          if (sample_type < 0)
            {
              fprintf (stderr, "error: --sample_type must be one of: cf32 "
                               "cf64 ci32 ci16 ci8\n");
              return 2;
            }
        }
      ...
      else
        {
          fprintf (stderr,
                   "usage: wavegen [--type tone|noise|pn|bpsk|qpsk] [--fs V] "
                   "[--freq V] [--snr V] [--snr_mode auto|fs|ebno|esno] "
                   "[--seed V] [--sps V] [--pn_length V] [--pn_poly V] "
                   "[--lfsr galois|fibonacci] [--count V] [--sample_type "
                   "cf32|cf64|ci32|ci16|ci8] [--file_type raw|csv] [--endian "
                   "le|be] [--output FILE] [--record FILE]\n");
          return 2;
        }
      ...
      if (rec)
        {
          ...
          fprintf (rec, ",\"sample_type\":\"%s\"",
                   jm_choices_sample_type[sample_type]);
          fprintf (rec, ",\"file_type\":\"%s\"",
                   jm_choices_file_type[file_type]);
