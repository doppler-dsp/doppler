...
int
zmq::tipc_listener_t::set_local_address (const char *addr_)
{
  ...
  // If random Port Identity, update address object to reflect the assigned
  // address
  ...
  if (sock == -1)
    {
      errno_assert (errno == EAGAIN || errno == EWOULDBLOCK || errno == ENOBUFS
                    ...
                    || errno == EPROTO || errno == EMFILE || errno == ENFILE);
      return retired_fd;
    }
