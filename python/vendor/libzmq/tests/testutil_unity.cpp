...
int
test_assert_success_message_errno_helper (int rc_, const char *msg_,
                                          const char *expr_, int line_)
{
  if (rc_ == -1)
    {
      char buffer[512];
...
int
test_assert_success_message_raw_errno_helper (int rc_, const char *msg_,
                                              const char *expr_, int line_,
                                              bool zero)
{
  if (rc_ == -1 || (zero && rc_ != 0))
    {
      ...
      buffer[sizeof (buffer) - 1]
          = 0; // to ensure defined behavior with VC++ <= 2013
      snprintf (buffer, sizeof (buffer) - 1,
                "%s failed%s%s%s with %d, errno = %i/%s", expr_,
                msg_ ? " (additional info: " : "", msg_ ? msg_ : "",
...
int
test_assert_success_message_raw_zero_errno_helper (int rc_, const char *msg_,
                                                   const char *expr_,
                                                   int line_)
{
  return test_assert_success_message_raw_errno_helper (rc_, msg_, expr_, line_,
                                                       true);
}

int
test_assert_failure_message_raw_errno_helper (int rc_, int expected_errno_,
                                              const char *msg_,
                                              const char *expr_, int line_)
{
  char buffer[512];
      ...
      if (current_errno != expected_errno_)
        {
          snprintf (buffer, sizeof (buffer) - 1,
                    ...
                    expr_, msg_ ? " (additional info: " : "", msg_ ? msg_ : "",
                    msg_ ? ")" : "", expected_errno_, current_errno);
          UNITY_TEST_FAIL (line_, buffer);
        }
    }
  return rc_;
...
void
send_string_expect_success (void *socket_, const char *str_, int flags_)
{
...
void
recv_string_expect_success (void *socket_, const char *str_, int flags_)
{
  const size_t len = str_ ? strlen (str_) : 0;
  char buffer[255];
...
static void *
internal_manage_test_context (bool init_, bool clear_)
{
...
static void
internal_manage_test_sockets (void *socket_, bool add_)
{
...
void *
test_context_socket_close (void *socket_)
{
...
void *
test_context_socket_close_zero_linger (void *socket_)
{
  const int linger = 0;
...
void
test_bind (void *socket_, const char *bind_address_, char *my_endpoint_,
           size_t len_)
{
...
void
bind_loopback (void *socket_, int ipv6_, char *my_endpoint_, size_t len_)
{
...
void
bind_loopback_ipv4 (void *socket_, char *my_endpoint_, size_t len_)
{
  bind_loopback (socket_, false, my_endpoint_, len_);
}

void
bind_loopback_ipv6 (void *socket_, char *my_endpoint_, size_t len_)
{
  bind_loopback (socket_, true, my_endpoint_, len_);
}

void
bind_loopback_ipc (void *socket_, char *my_endpoint_, size_t len_)
{
  if (!zmq_has ("ipc"))
    {
...
void
bind_loopback_tipc (void *socket_, char *my_endpoint_, size_t len_)
{
...
void
make_random_ipc_endpoint (char *out_endpoint_)
{
