...
zmq::router_t::router_t (class ctx_t *parent_, uint32_t tid_, int sid_)
    : routing_socket_base_t (parent_, tid_, sid_), _prefetched (false),
      _routing_id_sent (false), _current_in (NULL),
      _terminate_current_in (false), _more_in (false), _current_out (NULL),
...
void
zmq::router_t::xattach_pipe (pipe_t *pipe_, bool subscribe_to_all_,
...
int
zmq::router_t::xsetsockopt (int option_, const void *optval_,
                            size_t optvallen_)
{
...
void
zmq::router_t::xpipe_terminated (pipe_t *pipe_)
{
...
void
zmq::router_t::xread_activated (pipe_t *pipe_)
{
      ...
      if (msg_->flags () & msg_t::more)
        {
          ...
          //  Find the pipe associated with the routing id stored in the
          //  prefix. If there's no such pipe just silently ignore the message,
          //  unless router_mandatory is set.
...
static bool
check_pipe_hwm (const zmq::pipe_t &pipe_)
{
...
int
zmq::router_t::get_peer_state (const void *routing_id_,
...
bool
zmq::router_t::identify_peer (pipe_t *pipe_, bool locally_initiated_)
{
  msg_t msg;
  blob_t routing_id;
  ...
  if (locally_initiated_ && connect_routing_id_is_set ())
    {
      ...
      routing_id.set (reinterpret_cast<const unsigned char *> (
    ...
    {
      //  Pick up handshake cases and also case where next integral routing id
      //  is set
      msg.init ();
