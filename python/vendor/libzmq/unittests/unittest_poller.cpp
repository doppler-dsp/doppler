...
struct test_events_t : zmq::i_poll_events
{
  ...
  void
  out_event () ZMQ_OVERRIDE
  {
    // TODO
  }

  void
  timer_event (int id_) ZMQ_OVERRIDE
  {
    LIBZMQ_UNUSED (id_);
    _poller.rm_fd (_handle);
    _handle = (zmq::poller_t::handle_t)NULL;
  ...
  void
  set_handle (zmq::poller_t::handle_t handle_)
  {
    _handle = handle_;
  }
...
void
wait_in_events (test_events_t &events_)
{
...
void
wait_timer_events (test_events_t &events_)
{
  void *watch = zmq_stopwatch_start ();
...
void
create_nonblocking_fdpair (zmq::fd_t *r_, zmq::fd_t *w_)
{
...
void
close_fdpair (zmq::fd_t w_, zmq::fd_t r_)
{
...
#ifdef _WIN32
void
test_add_fd_with_pending_failing_connect ()
{
  zmq::thread_ctx_t thread_ctx;
  zmq::poller_t poller (thread_ctx);
  ...
  TEST_ASSERT_EQUAL_INT (
      -1, connect (connect_socket, reinterpret_cast<const sockaddr *> (&addr),
                   sizeof (addr)));
  TEST_ASSERT_EQUAL_INT (WSdpWOULDBLOCK, WSAGetLastError ());
  ...
  TEST_ASSERT_EQUAL_INT (0, getsockopt (connect_socket, SOL_SOCKET, SO_ERROR,
                                        ...
                                        &value_len));
  TEST_ASSERT_EQUAL_INT (WSdpCONNREFUSED, value);
