...
int
zmq::msg_t::init (void *data_, size_t size_, msg_free_fn *ffn_, void *hint_,
                  content_t *content_)
{
...
int
zmq::msg_t::init_size (size_t size_)
{
  if (size_ <= max_vsm_size)
    {
      _u.vsm.metadata = NULL;
...
int
zmq::msg_t::init_buffer (const void *buf_, size_t size_)
{
  const int rc = init_size (size_);
  if (unlikely (rc < 0))
    {
...
int
zmq::msg_t::init_external_storage (content_t *content_, void *data_,
...
int
zmq::msg_t::init_data (void *data_, size_t size_, msg_free_fn *ffn_,
                       void *hint_)
{
...
int
zmq::msg_t::init_subscribe (const size_t size_, const unsigned char *topic_)
{
...
int
zmq::msg_t::init_cancel (const size_t size_, const unsigned char *topic_)
{
  int rc = init_size (size_);
  if (rc == 0)
    {
...
void
zmq::msg_t::shrink (size_t new_size_)
{
  //  Check the validity of the message.
  zmq_assert (check ());
...
void
zmq::msg_t::set_flags (unsigned char flags_)
{
  _u.base.flags |= flags_;
}

void
zmq::msg_t::reset_flags (unsigned char flags_)
{
  _u.base.flags &= ~flags_;
}
...
void
zmq::msg_t::set_metadata (zmq::metadata_t *metadata_)
{
...
int
zmq::msg_t::set_routing_id (uint32_t routing_id_)
{
  if (routing_id_)
    {
...
int
zmq::msg_t::set_group (const char *group_)
{
...
int
zmq::msg_t::set_group (const char *group_, size_t length_)
{
