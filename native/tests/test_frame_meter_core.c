...
main (void)
{
  ...
  {
    dp_frame_meter_add (m, 1, 1);  /* found, checked        -> delivered */
    dp_frame_meter_add (m, 1, -1); /* found, no CRC carried -> delivered */
    dp_frame_meter_add (m, 1, 0);  /* found, failed CRC     -> ERROR     */
    dp_frame_meter_add (m, 0, -1); /* never found           -> ERROR     */
    dp_frame_meter_add (m, 0, 1);  /* never found; a CRC verdict here is
                                   meaningless and must not rescue it */

    DP_REQUIRE_MSG (dp_frame_meter_get_frames (m) == 5,
                    "every add is a frame");
    DP_REQUIRE_MSG (dp_frame_meter_get_sync_detected (m) == 3, "3 found");
    DP_REQUIRE_MSG (dp_frame_meter_get_crc_passed (m) == 1, "1 checked");
    DP_REQUIRE_MSG (dp_frame_meter_get_errors (m) == 3,
    ...
    ber_interval_t fer = dp_frame_meter_fer (m);
    DP_REQUIRE_MSG (fer.errors == 3 && fer.symbols == 5, "fer counts");
    ber_interval_t sm = dp_frame_meter_sync_miss (m);
    ...
    for (int i = 0; i < 100; i++)
      dp_frame_meter_add (m, i % 4 != 0, -1);
    DP_REQUIRE_MSG (dp_frame_meter_get_errors (m) == 25,
                    "with no CRC, errors are exactly the misses");
    DP_REQUIRE_MSG (dp_frame_meter_sync_miss (m).errors == 25, "and so is the "
                                                               "miss count");
  }
  ...
  {
    dp_frame_meter_reset (m);
    DP_REQUIRE_MSG (!dp_frame_meter_get_enough (m), "nothing counted yet");
    for (int i = 0; i < 9; i++)
      dp_frame_meter_add (m, 1, 0);
    DP_REQUIRE_MSG (!dp_frame_meter_get_enough (m),
                    "9 of 10 errors is not yet");
    dp_frame_meter_add (m, 1, 0);
  ...
  {
    dp_frame_meter_reset (m);
    DP_REQUIRE_MSG (dp_frame_meter_get_frames (m) == 0
                        && dp_frame_meter_get_errors (m) == 0
                        && dp_frame_meter_get_sync_detected (m) == 0
                        && dp_frame_meter_get_crc_passed (m) == 0,
                    "reset clears every counter");
    dp_frame_meter_add (m, 1, 0);
    DP_REQUIRE_MSG (dp_frame_meter_fer (m).conf == 0.99,
  ...
  /* ── the state triplet: a record resumes across a process boundary ────── */
  {
    dp_frame_meter_reset (m);
    for (int i = 0; i < 37; i++)
      dp_frame_meter_add (m, i % 3 != 0, i % 5 ? 1 : 0);

    size_t   n    = dp_frame_meter_state_bytes (m);
    uint8_t *blob = malloc (n);
    DP_CHECK (blob != NULL);
    dp_frame_meter_get_state (m, blob);

    dp_frame_meter_state_t *r = dp_frame_meter_create (10, 0.99);
    DP_CHECK (r != NULL);
    DP_REQUIRE_MSG (dp_frame_meter_set_state (r, blob) == DP_OK, "restore");
    DP_REQUIRE_MSG (
        dp_frame_meter_get_frames (r) == dp_frame_meter_get_frames (m)
            && dp_frame_meter_get_errors (r) == dp_frame_meter_get_errors (m)
            && dp_frame_meter_get_sync_detected (r)
                   == dp_frame_meter_get_sync_detected (m)
            && dp_frame_meter_get_crc_passed (r)
                   == dp_frame_meter_get_crc_passed (m),
    ...
    dp_frame_meter_add (m, 1, 0);
    dp_frame_meter_add (r, 1, 0);
    DP_REQUIRE_MSG (dp_frame_meter_get_errors (r)
                        == dp_frame_meter_get_errors (m),
                    "and accumulation continues identically");

    ((uint8_t *)blob)[0] ^= 0xFFu;
    DP_REQUIRE_MSG (dp_frame_meter_set_state (r, blob) == DP_ERR_INVALID,
                    "a clobbered envelope is REJECTED, never reinterpreted");
    free (blob);
    dp_frame_meter_destroy (r);
  }
