...
main (void)
{
  ...
  /* Argument validation. */
  DP_CHECK (dp_ppe_create (2, 0.0) == NULL);   /* max_len < 4 */
  DP_CHECK (dp_ppe_create (64, -1.0) == NULL); /* negative rate span */
   ...
   */
  {
    dp_ppe_state_t *p = dp_ppe_create (L, 0.0);
    DP_CHECK (p != NULL && p->n_rate == 1);
    synth_chirp (y, L, 0.1, 0.0);
    ppe_result_t e = dp_ppe_estimate (p, y, L);
   ...
   */
  {
    dp_ppe_state_t *p = dp_ppe_create (L, 5e-5);
    DP_CHECK (p != NULL && p->n_rate > 1);
    ...
    if (ya)
      {
        ...
        for (int i = 0; i < 2; i++)
          {
            dp_ppe_state_t *p = dp_ppe_create (lens[i], 0.0);
            DP_CHECK (p != NULL);
            synth_chirp (ya, lens[i], 0.05, 0.0);
            ppe_result_t e = dp_ppe_estimate (p, ya, lens[i]);
            snr[i]         = e.snr_db;
   ...
   * depends on. */
  {
    ...
    ppe_result_t a = dp_ppe_estimate (p, y, L);
    dp_ppe_reset (p);
    ppe_result_t b = dp_ppe_estimate (p, y, L);
    DP_CHECK (a.freq_norm == b.freq_norm);
    DP_CHECK (a.rate_norm == b.rate_norm);
    DP_CHECK (a.snr_db == b.snr_db);
    dp_ppe_reset (NULL); /* must not crash */
    dp_ppe_destroy (p);
  }
