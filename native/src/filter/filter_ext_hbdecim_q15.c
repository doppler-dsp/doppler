...
HBDecimQ15Obj_init(HBDecimQ15Object *self, PyObject *args, PyObject *kwds)
{
    ...
    self->handle = hbdecim_q15_create(h_len, (const float *)PyArray_DATA(h_arr));
    Py_DECREF(h_arr);
    if (!self->handle) {
        PyErr_SetString(PyExc_MemoryError,
                        "hbdecim_q15_create returned NULL");
        return -1;
    }
    {
...
HBDecimQ15Obj_execute(HBDecimQ15Object *self, PyObject *args)
{
    ...
    x_arr = (PyArrayObject *)PyArray_FROM_OTF(
        x_obj, NPY_INT16, NPY_ARRAY_C_CONTIGUOUS);
    ...
    size_t n_in  = (size_t)PyArray_SIZE(x_arr); /* int16_t count = 2 * complex samples */
    size_t n_out = hbdecim_q15_execute(self->handle,
                       (const int16_t *)PyArray_DATA(x_arr),
                       n_in / 2,            /* complex sample count */
                       self->_execute_buf,
                       self->_execute_buf_cap / 2); /* complex capacity */
    npy_intp dim = (npy_intp)(n_out * 2);   /* back to int16_t element count */
    PyObject *arr = PyArray_SimpleNewFromData(
        1, &dim, NPY_INT16, self->_execute_buf);
    if (!arr) return NULL;
...
HBDecimQ15_getprop_num_taps(HBDecimQ15Object *self, void *Py_UNUSED(closure))
{
    ...
    return PyLong_FromUnsignedLongLong((unsigned long long)hbdecim_q15_get_num_taps(self->handle));
}
static PyObject *
