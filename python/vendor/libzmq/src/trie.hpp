...
private:
  void
  apply_helper (unsigned char **buff_, size_t buffsize_, size_t maxbuffsize_,
...
// lightweight wrapper around trie_t adding tracking of total number of
// prefixes
...
public:
  ...
  bool
  add (unsigned char *prefix_, size_t size_)
  {
  ...
  bool
  rm (unsigned char *prefix_, size_t size_)
  {
  ...
  bool
  check (const unsigned char *data_, size_t size_) const
  {
    return _trie.check (data_, size_);
  }

  void
  apply (void (*func_) (unsigned char *data_, size_t size_, void *arg_),
         void *arg_)
  {
