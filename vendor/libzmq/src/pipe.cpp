...
int
zmq::pipepair (object_t *parents_[2], pipe_t *pipes_[2], const int hwms_[2],
...
void
zmq::send_routing_id (pipe_t *pipe_, const options_t &options_)
{
  zmq::msg_t id;
...
void
zmq::send_hello_msg (pipe_t *pipe_, const options_t &options_)
{
  zmq::msg_t hello;
  const int rc
...
zmq::pipe_t::pipe_t (object_t *parent_, upipe_t *inpipe_, upipe_t *outpipe_,
    ...
    : object_t (parent_), _in_pipe (inpipe_), _out_pipe (outpipe_),
      _in_active (true), _out_active (true), _hwm (outhwm_),
      _lwm (compute_lwm (inhwm_)), _in_hwm_boost (-1), _out_hwm_boost (-1),
      _msgs_read (0), _msgs_written (0), _peers_msgs_read (0), _peer (NULL),
      _sink (NULL), _state (active), _delay (true),
      _server_socket_routing_id (0), _conflate (conflate_)
{
  _disconnect_msg.init ();
}

zmq::pipe_t::~pipe_t () { _disconnect_msg.close (); }

void
zmq::pipe_t::set_peer (pipe_t *peer_)
{
...
void
zmq::pipe_t::set_event_sink (i_pipe_events *sink_)
{
...
void
zmq::pipe_t::set_server_socket_routing_id (uint32_t server_socket_routing_id_)
{
...
void
zmq::pipe_t::set_router_socket_routing_id (
...
bool
zmq::pipe_t::write (const msg_t *msg_)
{
...
void
zmq::pipe_t::process_activate_write (uint64_t msgs_read_)
{
...
void
zmq::pipe_t::process_hiccup (void *pipe_)
{
...
void
zmq::pipe_t::process_pipe_hwm (int inhwm_, int outhwm_)
{
...
void
zmq::pipe_t::terminate (bool delay_)
{
...
bool
zmq::pipe_t::is_delimiter (const msg_t &msg_)
{
  return msg_.is_delimiter ();
}

int
zmq::pipe_t::compute_lwm (int hwm_)
{
...
zmq::pipe_t::hiccup ()
{
  ...
  //  Create new inpipe.
  _in_pipe = _conflate ? static_cast<upipe_t *> (
...
void
zmq::pipe_t::set_hwms (int inhwm_, int outhwm_)
{
  int in = inhwm_ + std::max (_in_hwm_boost, 0);
  int out = outhwm_ + std::max (_out_hwm_boost, 0);

  // if either send or recv side has hwm <= 0 it means infinite so we should
  // set hwms infinite
...
void
zmq::pipe_t::set_hwms_boost (int inhwmboost_, int outhwmboost_)
{
...
void
zmq::pipe_t::send_hwms_to_peer (int inhwm_, int outhwm_)
{
  send_pipe_hwm (_peer, inhwm_, outhwm_);
}

void
zmq::pipe_t::set_endpoint_pair (zmq::endpoint_uri_pair_t endpoint_pair_)
{
...
void
zmq::pipe_t::send_stats_to_peer (own_t *socket_base_)
{
...
void
zmq::pipe_t::process_pipe_peer_stats (uint64_t queue_count_,
                                      own_t *socket_base_,
...
zmq::pipe_t::send_disconnect_msg ()
{
  if (_disconnect_msg.size () > 0 && _out_pipe)
    {
      // Rollback any incomplete message in the pipe, and push the disconnect
      // message.
      rollback ();
...
void
zmq::pipe_t::set_disconnect_msg (const std::vector<unsigned char> &disconnect_)
{
...
void
zmq::pipe_t::send_hiccup_msg (const std::vector<unsigned char> &hiccup_)
{
