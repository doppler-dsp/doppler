...
extern "C"
{
  ...
  static DWORD
  thread_routine (LPVOID arg_)
#else
  static unsigned int __stdcall
  thread_routine (void *arg_)
#endif
  {
...
void
zmq::thread_t::start (thread_fn *tfn_, void *arg_, const char *name_)
{
  _tfn = tfn_;
  _arg = arg_;
  if (name_)
...
#if defined _WIN32_WCE
  _descriptor = (HANDLE)CreateThread (NULL, stack, &::thread_routine, this, 0,
                                      &_thread_id);
#else
...
void
zmq::thread_t::setSchedulingParameters (int priority_, int scheduling_policy_,
...
void
zmq::thread_t::applySchedulingParameters () // to be called in secondary thread
                                            // context
{
  // not implemented
}
...
void
zmq::thread_t::applyThreadName () // to be called in secondary thread context
{
...
void
zmq::thread_t::setSchedulingParameters (int priority_, int schedulingPolicy_,
                                        const std::set<int> &affinity_cpus_)
{
                                            ...
                                            // context
{
  ...
  */
  ...
  if (use_nice_instead_priority)
    param.sched_priority = 0; // this is the only supported priority for most
                              // scheduling policies
  ...
  if (use_nice_instead_priority && _thread_priority != ZMQ_THREAD_PRIORITY_DFLT
      && _thread_priority > 0)
    {
      ...
      // i.e., increase the chance of this thread being scheduled: try setting
      // that to maximum priority.
      rc = nice (-20);

      errno_assert (rc != -1);
      // IMPORTANT: EPERM is typically returned for unprivileged processes:
      // that's because
      //            CAP_SYS_NICE capability is required or RLIMIT_NICE resource
      //            limit should be changed to avoid EPERM!
    }
#endif
