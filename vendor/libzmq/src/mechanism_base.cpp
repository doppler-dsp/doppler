...
int
zmq::mechanism_base_t::check_basic_command_structure (msg_t *msg_) const
{
  if (msg_->size () <= 1
...
void
zmq::mechanism_base_t::handle_error_reason (const char *error_reason_,
                                            size_t error_reason_len_)
{
  ...
  if (error_reason_len_ == status_code_len
    ...
    {
      // it is a ZAP error status code (300, 400 or 500), so emit an
