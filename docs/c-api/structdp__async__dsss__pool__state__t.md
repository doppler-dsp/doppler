...
# Struct dp\_async\_dsss\_pool\_state\_t



[**ClassList**](annotated.md) **>** [**dp\_async\_dsss\_pool\_state\_t**](structdp__async__dsss__pool__state__t.md)
...
|  [**dp\_async\_dsss\_receiver\_state\_t**](structdp__async__dsss__receiver__state__t.md) \*\* | [**rx**](#variable-rx)  <br> |
...
Allocate with [**dp\_async\_dsss\_pool\_create()**](async__dsss__pool__core_8h.md#function-dp_async_dsss_pool_create). 
...
```C++
dp_acq_state_t* dp_async_dsss_pool_state_t::acq;
```
...
```C++
double dp_async_dsss_pool_state_t::carrier_freq_hz;
```
...
```C++
double dp_async_dsss_pool_state_t::chip_rate;
```
...
```C++
uint8_t* dp_async_dsss_pool_state_t::code;
```
...
```C++
size_t dp_async_dsss_pool_state_t::code_len;
```
...
```C++
uint64_t dp_async_dsss_pool_state_t::dropped;
```
...
```C++
uint64_t dp_async_dsss_pool_state_t::events;
```
...
```C++
size_t dp_async_dsss_pool_state_t::feed_n;
```
...
```C++
const float _Complex* dp_async_dsss_pool_state_t::feed_x;
```
...
```C++
double dp_async_dsss_pool_state_t::fs;
```
...
```C++
acq_result_t* dp_async_dsss_pool_state_t::hits;
```
...
```C++
dp_event_log_t* dp_async_dsss_pool_state_t::log;
```
...
```C++
uint64_t dp_async_dsss_pool_state_t::max_on_samples;
```
...
```C++
size_t dp_async_dsss_pool_state_t::max_peaks;
```
...
```C++
size_t dp_async_dsss_pool_state_t::n_assigned;
```
...
```C++
size_t dp_async_dsss_pool_state_t::n_slots;
```
...
```C++
size_t* dp_async_dsss_pool_state_t::n_sym;
```
...
```C++
dp_pool_t* dp_async_dsss_pool_state_t::pool;
```
...
```C++
async_dsss_pool_row_t* dp_async_dsss_pool_state_t::rows;
```
...
```C++
dp_async_dsss_receiver_state_t** dp_async_dsss_pool_state_t::rx;
```
...
```C++
uint64_t dp_async_dsss_pool_state_t::samples_consumed;
```
...
```C++
size_t dp_async_dsss_pool_state_t::spc;
```
...
```C++
float _Complex* dp_async_dsss_pool_state_t::sym_buf;
```
...
```C++
size_t dp_async_dsss_pool_state_t::sym_cap;
```
...
```C++
double dp_async_dsss_pool_state_t::symbol_rate;
```
...
```C++
int dp_async_dsss_pool_state_t::threads;
```
