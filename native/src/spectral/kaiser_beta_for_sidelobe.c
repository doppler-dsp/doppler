 ...
 * window-design companion to dp_kaiser_window()/dp_kaiser_enbw() and is what
 * the measurement suite uses to pick the minimum beta whose sidelobes sit
 * below the required dynamic range — so window leakage never caps SFDR/SNR.
 *
 * NB this is NOT the same as doppler.resample's dp_kaiser_beta(): that uses
 * the Kaiser-Schafer *FIR filter* formula (beta = 0.1102*(A-8.7)), where A is
 * a filter stopband ripple.  A Kaiser window's own peak sidelobe is ~13 dB
...
double
dp_kaiser_beta_for_sidelobe (double atten_db)
{
