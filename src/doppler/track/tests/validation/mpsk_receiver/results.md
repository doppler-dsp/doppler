...
**One object, three faces.** `track.MpskReceiverR` is the same core reached through `mpsk_receiver_create_real()` — a matched DDCR instead of a matched DDC — and is a view rather than a second type as of the collapse ([`docs/design/mpsk-refactor.md`](../../../../../../docs/design/mpsk-refactor.md)). Every loop, discriminator and demapper decision below the front end is one implementation, which is what makes a claim about the loops a claim about all three faces.
