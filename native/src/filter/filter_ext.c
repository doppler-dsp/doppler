...
FIR_init(FIRObject *self, PyObject *args, PyObject *kwds)
{
    ...
    self->_execute_buf = malloc(
        fir_execute_max_out(self->handle) * sizeof(float complex));
...
FIR_execute(FIRObject *self, PyObject *args)
{
    ...
    size_t n_out = fir_execute(self->handle, (const float complex *)PyArray_DATA(in_arr), (size_t)n, self->_execute_buf);
