...
a lent frame is a CONTIGUOUS view of n samples even when those n straddle
...
Which call lends it depends on who is asking. ONE thread doing both jobs
uses `peek(n)`, which answers at once -- the frame, or None for "not yet".
A consumer on its OWN thread uses `wait(n)`, which blocks until a producer
delivers. A is the first, B is the second.
...
Two directions, and they are the two that actually occur:
  ...
  A. LARGE IN, FIXED OUT — irregular multi-thousand-sample blocks in, exact
     ...
     every append. Here the re-blocking IS the ring -- five lines, and the
     blocks may be LARGER than the ring.
...
**The ring need only hold one FRAME, not one block.** A frame larger than
`capacity` can never be lent — the ring holds at most that many — so `peek`
and `wait` raise `ValueError` rather than wait for something that cannot
arrive. What the producer hands over has no such limit: `write_some` takes
what fits and says how much.
...
assert max(DRIVER_BLOCKS) > ring.capacity, "a block larger than the ring"
assert ring.dropped == 0, "write_some never refuses, so nothing is counted"
assert all(len(f) == FFT_N and f.flags["C_CONTIGUOUS"] for f in frames)
assert len(frames) == produced // FFT_N, "every whole frame was taken"
assert ring.available == produced % FFT_N, "and only the remainder is left"

# The frames are the RIGHT samples, not merely the right size: joined, they
# are the input stream, in order, across every wrap.
...
assert len(frames) > 10 * ring.capacity // FFT_N, "many wraps, not one"
assert len(spectra) == len(frames) and len(spectra[0]) == FFT_N
print(
    f"A. {produced:,} samples in as {len(DRIVER_BLOCKS) * 6} irregular "
    f"blocks ({min(DRIVER_BLOCKS)}..{max(DRIVER_BLOCKS)}) through a "
    f"{ring.capacity:,}-sample ring -> {len(frames):,} FFT frames of exactly "
    f"{FFT_N}, identical to the input, {ring.available} left over"
)
